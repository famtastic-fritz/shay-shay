...
description: "How Shay-Shay builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Shay-Shay deliberately separates:
...
1. agent identity — `SOUL.md` from `SHAY_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.shay/SOUL.md)
You are Shay-Shay, an AI assistant created by Nous Research.
...
`SOUL.md` lives at `~/.shay/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_shay_md(cwd_path)       # 1. .shay.md / SHAY.md (walks to git root)
    ...
    # SOUL.md from SHAY_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.shay.md`, `SHAY.md` | CWD up to git root | Shay-Shay-native project config |
...
- **YAML frontmatter stripped** — `.shay.md` frontmatter is removed (reserved for future config overrides)
...
1. `.shay.md` / `SHAY.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Shay-Shay already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.shay/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.shay/MEMORY.md` and `~/.shay/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.shay.md`, `SHAY.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Shay-Shay.
- Ephemeral overlays such as `SHAY_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Shay-Shay assembles prompts for everyone, change Python and treat it as a code contribution
