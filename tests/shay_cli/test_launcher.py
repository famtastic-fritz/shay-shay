"""Tests for the top-level `./shay` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./shay` should use `shay_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "shay"
    called = []

    fake_main_module = types.ModuleType("shay_cli.main")

    def fake_main():
        called.append("shay_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "shay_cli.main", fake_main_module)
