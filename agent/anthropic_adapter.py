"""Anthropic Messages API adapter for Shay-Shay.

Translates between Shay-Shay's internal OpenAI-style message format and
...
# Shay-Shay effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Shay-Shay historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Shay-Shay)
    ...
    # 1. Shay-Shay-managed OAuth/setup token env var
...
# ── Shay-Shay-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.shay/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_SHAY_OAUTH_FILE = get_shay_home() / ".anthropic_oauth.json"
...
def run_shay_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Shay-Shay-native OAuth PKCE flow and return credential state."""
    ...
    print()
    print("Authorize Shay-Shay with your Claude Pro/Max subscription.")
    print()
    ...
    # Validate state to prevent CSRF (RFC 6749 §10.12). The OAuth state
    # is intentionally distinct from the PKCE verifier: state is visible in
    # the authorization URL, while the verifier must stay secret for token
    # exchange.
...
def read_shay_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Shay-Shay-managed OAuth credentials from ~/.shay/.anthropic_oauth.json."""
    if _SHAY_OAUTH_FILE.exists():
        try:
            data = json.loads(_SHAY_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Shay-Shay OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See shay-shay#13848.
            #
    ...
    # are preserved.  See shay-shay#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Shay-Shay", "Claude Code")
                text = text.replace("Shay-Shay", "Claude Code")
                text = text.replace("shay-shay", "claude-code")
    ...
    # silently hides reasoning text that Shay-Shay surfaces in its CLI. We
