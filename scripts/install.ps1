# ============================================================================
# Shay-Shay Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$Shay-ShayHome = "$env:LOCALAPPDATA\shay",
    [string]$InstallDir = "$env:LOCALAPPDATA\shay\shay-shay"
)
...
$RepoUrlSsh = "git@github.com:NousResearch/shay-shay.git"
$RepoUrlHttps = "https://github.com/NousResearch/shay-shay.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "│             ⚕ Shay-Shay Installer                    │" -ForegroundColor Magenta
    Write-Host "├─────────────────────────────────────────────────────────┤" -ForegroundColor Magenta
...
function Install-Git {
    ...
    which Shay-Shay uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\shay\git`` — never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Shay-Shay needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Shay-Shay copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\shay\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``SHAY_GIT_BASH_PATH`` (User scope) so Shay-Shay can find it in a fresh
    ...
    # Download PortableGit into $Shay-ShayHome\git.  Always works as long as
    # we can reach github.com — no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found — downloading PortableGit to $Shay-ShayHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "shay-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected — PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Shay-Shay features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Shay-Shay needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``SHAY_GIT_BASH_PATH`` (User env scope) so Shay-Shay can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $Shay-ShayHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $Shay-ShayHome\git\usr\bin\bash.exe
    $candidates += "$Shay-ShayHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$Shay-ShayHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("SHAY_GIT_BASH_PATH", $candidate, "User")
            $env:SHAY_GIT_BASH_PATH = $candidate
            Write-Info "Set SHAY_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe — Shay-Shay may not find Git Bash."
    Write-Info "If needed, set SHAY_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$Shay-ShayHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        $env:Path = "$Shay-ShayHome\node;$env:Path"
        Write-Success "Node.js $version found (Shay-Shay-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    # Fallback: download binary zip to ~/.shay/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\shay-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$Shay-ShayHome\node") { Remove-Item -Recurse -Force "$Shay-ShayHome\node" }
                Move-Item $extractedDir.FullName "$Shay-ShayHome\node"
                $env:Path = "$Shay-ShayHome\node;$env:Path"

                $version = & "$Shay-ShayHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.shay/node/"
                $script:HasNode = $true
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running shay processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/NousResearch/shay-shay/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\shay-shay-$Branch.zip"
                $extractPath = "$env:TEMP\shay-shay-extract"
...
function Install-Dependencies {
    ...
    #         believe a user expects `shay dashboard` / slash commands /
    ...
    if (-not $installed) {
        throw "Failed to install shay-shay package even with no extras. Inspect the uv pip install output above."
    }

    # Verify the dashboard deps specifically — they're the most common thing
    # users hit and lazy-import errors from `shay dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable — `shay dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `shay dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up shay command..."
    
    if ($NoVenv) {
        $shayBin = "$InstallDir"
    } else {
        $shayBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so shay is globally available
    # On Windows, the shay.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$shayBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$shayBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $shayBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set SHAY_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\shay instead
    # of the Unix default ~/.shay
    $currentShay-ShayHome = [Environment]::GetEnvironmentVariable("SHAY_HOME", "User")
    if (-not $currentShay-ShayHome -or $currentShay-ShayHome -ne $Shay-ShayHome) {
        [Environment]::SetEnvironmentVariable("SHAY_HOME", $Shay-ShayHome, "User")
        Write-Success "Set SHAY_HOME=$Shay-ShayHome"
    }
    $env:SHAY_HOME = $Shay-ShayHome
    
    # Update current session
    $env:Path = "$shayBin;$env:Path"
    
    Write-Success "shay command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.shay directory structure
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$Shay-ShayHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$Shay-ShayHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.shay/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.shay/.env"
        }
    } else {
        Write-Info "~/.shay/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$Shay-ShayHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.shay/config.yaml from template"
        }
    } else {
        Write-Info "~/.shay/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Shay-Shay's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$Shay-ShayHome\SOUL.md"
...
"@
        ...
        Write-Success "Created ~/.shay/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.shay/"
    
    # Seed bundled skills into ~/.shay/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.shay/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.shay/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$Shay-ShayHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.shay/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'shay setup tools' later."
        return
    }
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\shay-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\shay-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
                try {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\shay-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.shay/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m shay_cli.main setup
    } else {
        python -m shay_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $shayCmd = "$InstallDir\venv\Scripts\shay.exe"
    if (-not (Test-Path $shayCmd)) {
        $shayCmd = "shay"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$Shay-ShayHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'shay whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$Shay-ShayHome\logs\gateway.log"
            Start-Process -FilePath $shayCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$Shay-ShayHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: shay gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: shay gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$Shay-ShayHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$Shay-ShayHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$Shay-ShayHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$Shay-ShayHome\shay-shay\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   shay              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   shay setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   shay config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   shay config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   shay gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   shay update       " -NoNewline -ForegroundColor Green
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/NousResearch/shay-shay/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
