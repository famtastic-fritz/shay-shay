...
"""
    ...
    assert "Tyler Williams > Active Projects: Shay-Shay" in entries
...
def test_cron_store_is_archived_without_config_cron_section(tmp_path: Path):
    ...
    assert "Run `shay cron` to recreate scheduled tasks" in notes_text
...
def test_skill_installs_cleanly_under_skills_guard():
    ...
    # shay_config_mod  — print statements in the post-migration summary that
    #                      tell the user to *review* ~/.shay/config.yaml;
    ...
    KNOWN_FALSE_POSITIVES = {"agent_config_mod", "python_os_environ", "shay_config_mod"}
...
def test_rebrand_text_replaces_openclaw_variants():
    mod = load_module()
    # Mixed-case / capitalized matches → capital-H ``Shay-Shay``.
    assert mod.rebrand_text("OpenClaw prefers Python 3.11") == "Shay-Shay prefers Python 3.11"
    assert mod.rebrand_text("I told Open Claw to use dark mode") == "I told Shay-Shay to use dark mode"
    assert mod.rebrand_text("Open-Claw config is great") == "Shay-Shay config is great"
    assert mod.rebrand_text("OPENCLAW uses tools well") == "Shay-Shay uses tools well"
    # All-lowercase matches → lowercase ``shay``; this preserves the
    # real filesystem path ``~/.shay`` (Shay-Shay home) when rebranding
    # memory entries that reference ``~/.openclaw`` or ``openclaw`` prose.
    assert mod.rebrand_text("openclaw should always respond concisely") == "shay should always respond concisely"


def test_rebrand_text_replaces_legacy_bot_names():
    mod = load_module()
    # Same case-preservation rule as above.
    assert mod.rebrand_text("ClawdBot remembers my timezone") == "Shay-Shay remembers my timezone"
    assert mod.rebrand_text("clawdbot prefers tabs") == "shay prefers tabs"
    assert mod.rebrand_text("MoltBot was configured for Spanish") == "Shay-Shay was configured for Spanish"
    assert mod.rebrand_text("moltbot uses Python") == "shay uses Python"
...
def test_rebrand_text_handles_multiple_replacements():
    ...
    assert mod.rebrand_text(text) == "Shay-Shay said to ask Shay-Shay about Shay-Shay settings"


def test_rebrand_text_preserves_filesystem_path_casing():
    """Lowercase matches — especially ``.openclaw`` filesystem paths — must
    rewrite to lowercase ``.shay`` (the real Shay-Shay home), not the broken
    ``.Shay-Shay``.
    ...
    rewritten to ``~/.Shay-Shay/config.yaml`` — a path that doesn't exist —
    and the agent kept trying to read it.
    """
    mod = load_module()
    assert mod.rebrand_text("config is at ~/.openclaw/config.yaml") == \
        "config is at ~/.shay/config.yaml"
    assert mod.rebrand_text("use .openclaw directory") == "use .shay directory"
    assert mod.rebrand_text("Path.home() / '.openclaw'") == "Path.home() / '.shay'"
    # Sentence with both lowercase path and capitalized prose.
    assert mod.rebrand_text("openclaw config path: ~/.openclaw/") == \
        "shay config path: ~/.shay/"
...
def test_migrate_soul_rebrands_content(tmp_path):
    ...
    assert "OpenClaw" not in result
    assert "You are Shay-Shay" in result
