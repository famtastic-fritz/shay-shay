"""Tests for the Shay-Shay plugin system (shay_cli.plugins)."""
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<shay_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<shay_home>/plugins/``; we derive
    ``<shay_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <SHAY_HOME>/config.yaml.
        # Config is always read from SHAY_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        shay_home_str = os.environ.get("SHAY_HOME")
        if shay_home_str:
            shay_home = Path(shay_home_str)
        else:
            shay_home = base.parent
        shay_home.mkdir(parents=True, exist_ok=True)
        cfg_path = shay_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.shay/plugins/ are discovered."""
        plugins_dir = tmp_path / "shay_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("SHAY_HOME", str(tmp_path / "shay_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHAY_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".shay" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under shay_plugins.<name>."""
        plugins_dir = tmp_path / "shay_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("SHAY_HOME", str(tmp_path / "shay_test"))

        # Clean up any prior namespace module
        sys.modules.pop("shay_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "shay_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="shay_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import shay_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("shay_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "shay_test" / "plugins"
        # Set SHAY_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("shay_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("shay_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("shay_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("shay_cli.plugins.PluginContext.dispatch_tool.__module__", "shay_cli.plugins"):
...
class TestPluginDebugLogging:
    """SHAY_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("SHAY_PLUGINS_DEBUG", raising=False)
        from shay_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With SHAY_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("SHAY_PLUGINS_DEBUG", "1")
        from shay_cli import plugins as plugins_mod
