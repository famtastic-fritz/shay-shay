...
Handles: shay honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "shay" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Shay-Shay profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'shay honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'shay honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `shay update` -- no output, no exceptions.
    """
    try:
        from shay_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Shay-Shay profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $SHAY_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_shay_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Shay-Shay persistent cross-session memory.")
    ...
    shay_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'shay honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = shay_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        shay_host["peerName"] = new_peer

    current_ai = shay_host.get("aiPeer") or cfg.get("aiPeer", "shay")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        shay_host["aiPeer"] = new_ai

    current_workspace = shay_host.get("workspace") or cfg.get("workspace", "shay")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        shay_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = shay_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        shay_host["observationMode"] = new_obs
    else:
        shay_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(shay_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        shay_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        shay_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = shay_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        shay_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = shay_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in ("none", "uncapped", "no limit"):
        shay_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                shay_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(shay_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            shay_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        shay_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        shay_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in ("minimal", "low", "medium", "high", "max"):
        shay_host["dialecticReasoningLevel"] = new_reasoning
    else:
        shay_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = shay_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        shay_host["sessionStrategy"] = new_strat

    shay_host["enabled"] = True
    shay_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        shay_config = load_config()
        shay_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(shay_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: shay config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    shay honcho status     -- show full config")
    print("    shay honcho mode       -- change recall/observation mode")
    print("    shay honcho tokens     -- tune context and dialectic budgets")
    print("    shay honcho peer       -- update peer names")
    print("    shay honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Shay-Shay profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: shay honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'shay honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: shay honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        shay = hosts.get(_host_key(), {})
        user = shay.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = shay.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = shay.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = shay.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Shay-Shay' identity in Honcho. Seed with 'shay honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: shay honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: shay honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = shay.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = shay.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = shay.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Shay-Shay asks Honcho's AI peer a question")
        ...
        print("\n  Set with: shay honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'shay honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'shay honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    shay honcho identity --show        — show both peer representations")
        print("    shay honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Shay-Shay + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Shay-Shay\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Shay-Shay persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  shay honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'shay honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'shay honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  shay honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'shay'")
        print("  with Honcho configured and no prior session history.")
        print("  (Shay-Shay calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    shay honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'shay honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Shay-Shay, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'shay honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    shay honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  shay honcho identity <path/to/SOUL.md>")
    ...
    print("    Shay-Shay:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Shay-Shay:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Shay-Shay:   Honcho observes every message and updates representations")
    ...
    print("    Shay-Shay:   per-session by default — each run gets its own session")
    print("              Map a custom name:  shay honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. shay honcho setup              — configure API key (required)")
        print("  2. shay honcho migrate            — re-run this walkthrough")
    else:
        print("  1. shay honcho status             — verify Honcho connection")
        print("  2. shay                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. shay honcho identity --show    — verify AI peer representation")
        print("  4. shay honcho tokens             — tune context and dialectic budgets")
        print("  5. shay honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'shay memory setup'...\n")
        from shay_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``shay honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``shay honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to shay memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Shay-Shay Honcho",
    )
