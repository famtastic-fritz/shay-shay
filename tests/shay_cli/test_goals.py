"""Tests for shay_cli/goals.py — persistent cross-turn goals."""
...
def shay_home(tmp_path, monkeypatch):
    """Isolated SHAY_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves SHAY_HOME.
    from shay_cli import goals
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from shay_cli.goals import _parse_judge_response
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from shay_cli.goals import judge_goal
...
class TestGoalManager:
    def test_no_goal_initial(self, shay_home):
    ...
    def test_set_then_status(self, shay_home):
        from shay_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, shay_home):
        from shay_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, shay_home):
        from shay_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, shay_home):
    ...
    def test_evaluate_after_turn_done(self, shay_home):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, shay_home):
        from shay_cli import goals
        from shay_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, shay_home):
    ...
    def test_evaluate_after_turn_inactive(self, shay_home):
    ...
    def test_continuation_prompt_shape(self, shay_home):
        ...
        mgr.set("port goal command to shay")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to shay" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from shay_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, shay_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from shay_cli import goals
        from shay_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, shay_home):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, shay_home):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from shay_cli import goals
        from shay_cli.goals import GoalManager, load_goal
