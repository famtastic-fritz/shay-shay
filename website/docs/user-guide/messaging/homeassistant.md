...
description: Control your smart home with Shay-Shay via Home Assistant integration.
...
Shay-Shay integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Shay-Shay"
5. Copy the token
...
Shay-Shay registers four tools for smart home control:
...
Configure which events the agent sees in `~/.shay/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Shay-Shay".
