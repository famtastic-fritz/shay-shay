...
import pytest
from shay_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from shay_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("shay_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `shay setup` should reach setup.py, which prints headless guidance."""
        from shay_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("shay_cli.setup.ensure_shay_home"),
            patch("shay_cli.setup.load_config", return_value={}),
            patch("shay_cli.setup.get_shay_home", return_value="/tmp/.shay"),
            patch("shay_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "shay config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `shay` should not prompt for input when no provider exists and stdin is headless."""
        from shay_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("shay_cli.main._has_any_provider_configured", return_value=False),
            patch("shay_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`shay setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["shay", "setup", "tts"])
