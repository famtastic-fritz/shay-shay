...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Shay-Shay starts and finds ``~/.openclaw/``.

    Points users at ``shay claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``shay claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Shay-Shay, run "
        "`shay claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `shay claw cleanup` (renames it to ~/.openclaw.pre-migration — "
