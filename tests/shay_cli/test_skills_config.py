"""Tests for shay_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestSaveDisabledSkills:
    @patch("shay_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from shay_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("shay_cli.config.load_config")
    ...
    @patch("shay_cli.config.load_config")
    @patch.dict("os.environ", {"SHAY_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """SHAY_SESSION_PLATFORM should be used when SHAY_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
    ...
    def test_shay_platform_takes_precedence(self, tmp_path, monkeypatch):
        """SHAY_PLATFORM should win over SHAY_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("SHAY_HOME", str(tmp_path))
        monkeypatch.setenv("SHAY_PLATFORM", "telegram")
        monkeypatch.setenv("SHAY_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from shay_cli.skills_config import _get_categories
        skills = [
