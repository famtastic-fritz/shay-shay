...
This module provides the glue:
    - ``ShayTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Shay-Shay"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses SHAY_HOME so each profile gets its own OAuth tokens.
    Layout: ``SHAY_HOME/mcp-tokens/``
    """
    try:
        from shay_constants import get_shay_home
        base = Path(get_shay_home())
    except ImportError:
        base = Path(os.environ.get("SHAY_HOME", str(Path.home() / ".shay")))
...
# ---------------------------------------------------------------------------
# ShayTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ShayTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        SHAY_HOME/mcp-tokens/<server_name>.json         -- tokens
        SHAY_HOME/mcp-tokens/<server_name>.client.json   -- client info
        SHAY_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Shay-Shay records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Shay-Shay.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Shay-Shay")
    scope = cfg.get("scope")
