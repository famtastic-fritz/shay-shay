...
# 5. Symlinks the 'shay' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Shay-Shay Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink shay into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up shay command..."

SHAY_BIN="$SCRIPT_DIR/venv/bin/shay"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$SHAY_BIN" "$COMMAND_LINK_DIR/shay"
echo -e "${GREEN}✓${NC} Symlinked shay → $COMMAND_LINK_DISPLAY_DIR/shay"
...
# ============================================================================
# Seed bundled skills into ~/.shay/skills/
# ============================================================================

SHAY_SKILLS_DIR="${SHAY_HOME:-$HOME/.shay}/skills"
mkdir -p "$SHAY_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.shay/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$SHAY_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  shay status        # Check configuration"
if is_termux; then
    echo "  shay gateway       # Run gateway in foreground"
else
    echo "  shay gateway install # Install gateway service (messaging + cron)"
fi
echo "  shay cron list     # View scheduled jobs"
echo "  shay doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m shay_cli.main setup
fi
