...
# Shay-Shay-internal env vars that should NOT leak into terminal subprocesses.
_SHAY_PROVIDER_ENV_FORCE_PREFIX = "_SHAY_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from shay_cli.config import OPTIONAL_ENV_VARS
...
_SHAY_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Shay-Shay-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_SHAY_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _SHAY_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_SHAY_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_SHAY_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _SHAY_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("SHAY_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\shay\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\shay\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _shay_portable_git = os.path.join(_local_appdata, "shay", "git") if _local_appdata else ""
    if _shay_portable_git:
        for candidate in (
            os.path.join(_shay_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_shay_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Shay-Shay requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set SHAY_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_SHAY_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_SHAY_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _SHAY_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # Shay-Shay is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    # npm …) into {SHAY_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Shay-Shay trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``SHAY_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under SHAY_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_shay_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "shay_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._shay_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_shay_pgid", None)
                    if pgid is None:
                        raise
