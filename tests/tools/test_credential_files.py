...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        shay_home.mkdir()
        (shay_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"SHAY_HOME": str(shay_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(shay_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.shay/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        (shay_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        (shay_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        (shay_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.shay/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        shay_home = tmp_path / ".shay"
        (shay_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"SHAY_HOME": str(shay_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.shay")

        assert mounts[0]["container_path"] == "/home/user/.shay/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.shay/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.shay/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape SHAY_HOME."""
        monkeypatch.setenv("SHAY_HOME", str(tmp_path / ".shay"))
        (tmp_path / ".shay").mkdir()

        # Create a sensitive file one level above shay_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside shay_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside SHAY_HOME must still be registered."""
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        monkeypatch.setenv("SHAY_HOME", str(shay_home))
        (shay_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_shay_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of SHAY_HOME must be allowed."""
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        subdir = shay_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside SHAY_HOME pointing outside must be rejected."""
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        monkeypatch.setenv("SHAY_HOME", str(shay_home))

        # Create a sensitive file outside shay_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside shay_home pointing outside
        symlink = shay_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes SHAY_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, shay_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape SHAY_HOME."""
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        monkeypatch.setenv("SHAY_HOME", str(shay_home))
        ...
        sensitive.write_text("{}")
        self._write_config(shay_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(shay_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside SHAY_HOME via config must still mount."""
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        monkeypatch.setenv("SHAY_HOME", str(shay_home))

        (shay_home / "oauth.json").write_text("{}")
        self._write_config(shay_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        (shay_home / "cache" / "documents").mkdir(parents=True)
        (shay_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("SHAY_HOME", str(shay_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.shay/cache/documents" in paths
        assert "/root/.shay/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.shay/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        # Use legacy dir name — get_shay_dir prefers old if it exists
        (shay_home / "document_cache").mkdir()
        (shay_home / "image_cache").mkdir()
        monkeypatch.setenv("SHAY_HOME", str(shay_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(shay_home / "document_cache") in host_paths
        assert str(shay_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.shay/cache/documents" in container_paths
        assert "/root/.shay/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        shay_home = tmp_path / ".shay"
        doc_dir = shay_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        shay_home = tmp_path / ".shay"
        ss_dir = shay_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.shay/cache/screenshots/session_abc/screen1.png"
