#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Shay-Shay.
...
# Ensure sibling modules (_shay_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _shay_home import display_shay_home, get_shay_home

SHAY_HOME = get_shay_home()
TOKEN_PATH = SHAY_HOME / "google_token.json"
CLIENT_SECRET_PATH = SHAY_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = SHAY_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Shay-Shay profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'shay-shay[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Shay-Shay home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_shay_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Shay-Shay")
