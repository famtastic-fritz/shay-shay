...
Usage (via cron with --no-agent):
    ...
    shay cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$SHAY_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Shay-Shay-Watcher/1.0"})
