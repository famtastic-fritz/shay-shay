...
description: "在 Windows 上通过 WSL2 安装 uv、Shay-Shay 与 Tool Gateway 的推荐路径与常见坑"
...
上游开发与 CI 以 **Linux / macOS** 为主；在 Windows 上，**官方推荐路径是 WSL2**，而不是在「旧版原生 CMD/PowerShell」里直接跑完整 Shay-Shay 栈。本页给出从 0 到可跑 `shay` + Tool Gateway 的最短闭环。
...
:::tip 路径与权限
Shay-Shay 默认配置目录为 `~/.shay/`（在 WSL 内即 Linux 家目录）。请勿把 WSL 项目放在会被 Windows 杀毒实时深度扫描的极慢盘符上；推荐放在 WSL 文件系统（例如 `~/projects/...`）而非 `/mnt/c/...` 下的重度 IO 路径。
:::

## 4. 模型与 Tool Gateway

1. 在 WSL 内运行 `shay model`，按提示绑定 **Nous Portal**（或其他提供商）。  
...
| `shay` 找不到 | 确认 `uv run shay` 或按安装文档将 CLI 暴露到 `PATH`；命令应在 **WSL** 内执行。 |
