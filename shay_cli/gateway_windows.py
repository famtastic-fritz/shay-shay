...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m shay_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Shay_Gateway"
_TASK_DESCRIPTION = "Shay-Shay Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: ``Shay_Gateway``
    Named profile X: ``Shay_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during shay_cli boot.
    from shay_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\shay\\gateway-service\\<task_name>.cmd``
    (or ``<SHAY_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Shay-Shay installs stay self-contained).
    """
    _assert_windows()
    from shay_cli.config import get_shay_home

    script_dir = Path(get_shay_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports SHAY_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m shay_cli.main [--profile X] gateway run --replace``
    ...
    lines.append(f'set "SHAY_HOME={shay_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "SHAY_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports shay_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    prog_args = [python_path, "-m", "shay_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    shay_home = str(Path(get_shay_home()).resolve())
    profile_arg = _profile_arg(shay_home)

    content = _build_gateway_cmd_script(python_path, working_dir, shay_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "shay_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m shay_cli.main gateway run --replace``
    ...
    log_dir = Path(get_shay_home()) / "logs"
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from shay_cli.config import get_shay_home
        print(f"    type {Path(get_shay_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_shay_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from shay_cli.config import get_shay_home

    shay_home = Path(get_shay_home()).resolve()
    print()
    print("Next steps:")
    print("  shay gateway status                      # Check status")
    print(f"  type {shay_home}\\logs\\gateway.log       # View logs")
...
def stop() -> None:
    ...
    _assert_windows()
    from shay_cli.gateway import kill_gateway_processes
