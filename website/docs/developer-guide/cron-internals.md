...
description: "How Shay-Shay stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `shay_cli/cron.py` | CLI `shay cron` subcommands |
...
Jobs are stored in `~/.shay/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `shay cron` commands are run or during active CLI sessions.
...
```python
# ~/.shay/scripts/check_competitors.py
...
2. **Environment variable** — `SHAY_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.shay/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `shay cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `shay cron` CLI provides direct job management:

```bash
shay cron list                    # Show all jobs
shay cron create                  # Interactive job creation (alias: add)
shay cron edit <job_id>           # Edit job configuration
shay cron pause <job_id>          # Pause a running job
shay cron resume <job_id>         # Resume a paused job
shay cron run <job_id>            # Trigger immediate execution
shay cron remove <job_id>         # Delete a job
```
