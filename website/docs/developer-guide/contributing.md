...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Shay-Shay — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Shay-Shay! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Shay-Shay core? Start with [Build a Shay-Shay Plugin](../guides/build-a-shay-plugin.md)
- Building a new built-in core tool for Shay-Shay itself? Start with [Adding Tools](./adding-tools.md)
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.shay/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.shay` — use `get_shay_home()` from `shay_constants` for code paths and `display_shay_home()` for user-facing messages. See [AGENTS.md](https://github.com/NousResearch/shay-shay/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

Shay-Shay officially supports **Linux, macOS, WSL2, and native Windows (early beta — via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. The native-Windows path is new and moves fast — if you're doing Windows-heavy dev, expect to hit and fix rough edges.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/NousResearch/shay-shay/blob/main/LICENSE).
