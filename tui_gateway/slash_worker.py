"""Persistent slash-command worker — one ShayCLI per TUI session.
...
def _run(cli: ShayCLI, command: str) -> str:
...
def main():
    ...
    os.environ["SHAY_SESSION_KEY"] = args.session_key
    os.environ["SHAY_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ShayCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
