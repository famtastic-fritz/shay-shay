...
sidebar_position: 5
title: "WhatsApp"
description: "Set up Shay-Shay as a WhatsApp bot via the built-in Baileys bridge"
---

# WhatsApp Setup

Shay-Shay connects to WhatsApp through a built-in bridge based on **Baileys**. This works by emulating a WhatsApp Web session — **not** through the official WhatsApp Business API. No Meta developer account or Business verification is required.
...
with third-party bridges. When this happens, Shay-Shay will update the bridge dependency. If the
bot stops working after a WhatsApp update, pull the latest Shay-Shay version and re-pair.
:::
...
3. Run `shay whatsapp` and scan the QR code from that WhatsApp account
...
The Baileys bridge saves its session under `~/.shay/platforms/whatsapp/session`. This means:
...
| **QR code expires** | QR codes refresh every ~20 seconds. If it times out, restart `shay whatsapp`. |
| **Session not persisting** | Check that `~/.shay/platforms/whatsapp/session` exists and is writable. If containerized, mount it as a persistent volume. |
| **Logged out unexpectedly** | WhatsApp unlinks devices after long inactivity. Keep the phone on and connected to the network, then re-pair with `shay whatsapp` if needed. |
| **Bridge crashes or reconnect loops** | Restart the gateway, update Shay-Shay, and re-pair if the session was invalidated by a WhatsApp protocol change. |
| **Bot stops working after WhatsApp update** | Update Shay-Shay to get the latest bridge version, then re-pair. |
| **macOS: "Node.js not installed" but node works in terminal** | launchd services don't inherit your shell PATH. Run `shay gateway install` to re-snapshot your current PATH into the plist, then `shay gateway start`. See the [Gateway Service docs](./index.md#macos-launchd) for details. |
...
| **Bot replies to strangers with a pairing code** | Set `whatsapp.unauthorized_dm_behavior: ignore` in `~/.shay/config.yaml` if you want unauthorized DMs to be silently ignored instead. |
...
- The `~/.shay/platforms/whatsapp/session` directory contains full session credentials — protect it like a password
- Set file permissions: `chmod 700 ~/.shay/platforms/whatsapp/session`
