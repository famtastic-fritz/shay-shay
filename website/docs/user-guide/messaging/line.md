...
sidebar_position: 17
title: "LINE"
description: "Set up Shay-Shay as a LINE Messaging API bot"
---

# LINE Setup

Run Shay-Shay as a [LINE](https://line.me/) bot via the official LINE Messaging API. The adapter lives as a bundled platform plugin under `plugins/platforms/line/` — no core edits, just enable it like any other platform.
...
# devtunnel
devtunnel create shay-line --allow-anonymous
devtunnel port create shay-line -p 8646 --protocol https
devtunnel host shay-line
```
...
**Bot receives nothing in groups.** Check `LINE_ALLOWED_GROUPS` includes the `C...` group ID. To find a group ID, send a test message and grep `~/.shay/logs/gateway.log` for `LINE: rejecting unauthorized source` — the rejected source dict has the IDs.
...
**"already in use by another profile".** The same channel access token is bound to another running Shay-Shay profile. Stop the other gateway or use a separate channel.
