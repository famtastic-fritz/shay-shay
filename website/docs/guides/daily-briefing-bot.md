...
2. **Shay-Shay spins up** a fresh agent session with your prompt
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.shay/cron/output/` and you can read them anytime.
:::
...
Shay-Shay will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Shay-Shay
```
...
Before creating cron jobs, ensure Shay-Shay has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Shay-Shay what you want:
...
Shay-Shay will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Shay-Shay to delegate each topic to a sub-agent:
...
Shay-Shay will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
