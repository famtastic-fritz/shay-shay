...
CHECKPOINT_PATH = get_shay_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/shay_bg_{session.id}.log"
        pid_path = f"{temp_dir}/shay_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/shay_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `shay update` restarting the gateway) is still
