"""
IRC Platform Adapter for Shay-Shay.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Shay-Shay.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "shay-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Shay-Shay")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Shay-Shay shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: shay_, shay_1, shay_2...
...
def check_requirements() -> bool:
    ...
    # shay setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `shay gateway setup` flow for the IRC platform.

    Lazy-imports ``shay_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Shay-Shay to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #shay — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.shay/.env")
    print_info("Restart the gateway for changes to take effect: shay gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "shay-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "shay-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Shay-Shay (cron)")
