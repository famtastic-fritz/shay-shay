...
Ensures ``shay update`` correctly detects running gateways managed by
...
from shay_cli.main import cmd_update
...
def _no_restart_verify_sleep(monkeypatch):
    """shay_cli/main.py uses time.sleep(3) after systemctl restart to
...
def _make_run_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        # systemctl list-units shay-gateway* — discover all gateway services
        if "systemctl" in joined and "list-units" in joined:
            if "--user" in joined and systemd_active:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="shay-gateway.service loaded active running Shay-Shay Gateway\n",
                    stderr="",
                )
...
class TestLaunchdPlistPath:
    def test_plist_contains_environment_variables(self):
        ...
        assert "<key>SHAY_HOME</key>" in plist
...
class TestCmdUpdateLaunchdRestart:
    ...
    def test_update_detects_launchd_and_skips_manual_restart_message(
        ...
        'auto-restart via launchd' instead of 'Restart it with: shay gateway run'."""
        ...
        assert "Restart manually: shay gateway run" not in captured
    ...
    def test_update_without_launchd_shows_manual_restart(
        ...
        captured = capsys.readouterr().out
        assert "Restart manually: shay gateway run" in captured
    ...
    def test_update_restarts_profile_manual_gateways(
        ...
        monkeypatch.setattr(
            ...
            lambda: tmp_path / "ai.shay.gateway.plist",
        )
        ...
        process = gateway_cli.ProfileGatewayProcess(
            profile="coder",
            path=tmp_path / ".shay" / "profiles" / "coder",
            pid=12345,
        )
    ...
    def test_update_with_systemd_still_restarts_via_systemd(
        ...
        captured = capsys.readouterr().out
        assert "Restarted shay-gateway" in captured
        ...
        def side_effect(cmd, **kwargs):
            ...
            if "systemctl" in joined and "list-units" in joined:
                if "--user" in joined:
                    return subprocess.CompletedProcess(
                        cmd, 0,
                        stdout="shay-gateway.service loaded active running\n",
                        stderr="",
                    )
                return subprocess.CompletedProcess(cmd, 0, stdout="", stderr="")
    ...
    def test_update_falls_back_to_systemctl_restart_when_sigusr1_times_out(
        ...
        monkeypatch.setattr(
            "shay_cli.gateway._graceful_restart_via_sigusr1",
    ...
    def test_update_bypasses_restartsec_after_graceful_drain(
        ...
        # Must have called ``reset-failed shay-gateway`` AND ``start
        # shay-gateway`` explicitly so systemd bypasses RestartSec.
        reset_calls = [c for c in calls if "reset-failed" in c and "shay-gateway" in c]
        start_calls = [
            c for c in calls
            if "start" in c and "shay-gateway" in c and "restart" not in c
        ]
        assert reset_calls, (
            f"Expected explicit `reset-failed shay-gateway` after graceful drain; "
            f"systemctl calls were: {calls}"
        )
        assert start_calls, (
            f"Expected explicit `start shay-gateway` after graceful drain to "
...
class TestServicePidExclusion:
    ...
    where ``shay update`` would restart the gateway and immediately kill it.
    """
...
class TestGetServicePids:
    ...
    def test_returns_launchd_pid(self, monkeypatch):
        monkeypatch.setattr(gateway_cli, "is_linux", lambda: False)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: True)
        monkeypatch.setattr(gateway_cli, "get_launchd_label", lambda: "ai.shay.gateway")
    ...
    def test_excludes_zero_pid(self, monkeypatch):
        ...
        def fake_run(cmd, **kwargs):
            ...
            if "list-units" in joined:
                return subprocess.CompletedProcess(
                    cmd, 0,
                    stdout="shay-gateway.service loaded inactive dead Shay-Shay Gateway\n",
                    stderr="",
                )
            if "show" in joined and "MainPID" in joined:
    ...
    def test_filters_to_current_profile(self, monkeypatch, tmp_path):
        ...
        def fake_run(cmd, **kwargs):
            return subprocess.CompletedProcess(
                cmd, 0,
                stdout=(
                    "100 /Users/dgrieco/.shay/shay-shay/venv/bin/python -m shay_cli.main --profile orcha gateway run --replace\n"
                    "200 /Users/dgrieco/.shay/shay-shay/venv/bin/python -m shay_cli.main --profile other gateway run --replace\n"
                ),
                stderr="",
            )
        ...
        monkeypatch.setattr(gateway_cli, "_profile_arg", lambda shay_home=None: "--profile orcha")
...
class TestGatewayModeWritesExitCodeEarly:
    """When running as ``shay update --gateway``, the exit code marker must be
    ...
    def test_exit_code_written_in_gateway_mode(
        ...
        # Point SHAY_HOME at a temp dir so the marker file lands there
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        monkeypatch.setenv("SHAY_HOME", str(shay_home))
        import shay_cli.config as _cfg
        monkeypatch.setattr(_cfg, "get_shay_home", lambda: shay_home)
        # Also patch the module-level ref used by cmd_update
        import shay_cli.main as _main_mod
        monkeypatch.setattr(_main_mod, "get_shay_home", lambda: shay_home)
        ...
        exit_code_path = shay_home / ".update_exit_code"
...
class TestCmdUpdateLegacyGatewayWarning:
    """Tests for the legacy shay.service warning printed by `shay update`.

    Users who installed Shay-Shay before the service rename often have a
    dormant ``shay.service`` that starts flap-fighting the current
    ``shay-gateway.service`` after PR #5646. Every ``shay update``
    should remind them to run ``shay gateway migrate-legacy`` until
    they do.
    """
    ...
    def test_update_prints_legacy_warning_when_detected(
        ...
        user_dir.mkdir()
        system_dir.mkdir()
        legacy_path = user_dir / "shay.service"
        ...
        captured = capsys.readouterr().out
        assert "Legacy Shay-Shay gateway unit(s) detected" in captured
        assert "shay.service" in captured
        assert "shay gateway migrate-legacy" in captured
    ...
    def test_update_silent_when_no_legacy_units(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Shay-Shay gateway" not in captured
    ...
    def test_update_does_not_flag_profile_units(
        ...
        """Profile units (shay-gateway-coder.service) must not trigger the warning.

        This is the core safety invariant: the legacy allowlist is
        ``shay.service`` only, no globs.
        """
        user_dir = tmp_path / "user"
        system_dir = tmp_path / "system"
        user_dir.mkdir()
        ...
        (user_dir / "shay-gateway-coder.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        (user_dir / "shay-gateway.service").write_text(
            self._OUR_UNIT_TEXT, encoding="utf-8"
        )
        ...
        captured = capsys.readouterr().out
        assert "Legacy Shay-Shay gateway" not in captured
        assert "shay-gateway-coder.service" not in captured  # not flagged
    ...
    def test_update_lists_system_scope_unit_with_sudo_hint(
        ...
        captured = capsys.readouterr().out
        assert "Legacy Shay-Shay gateway" in captured
...
class TestCmdUpdateResetFailedBeforeRestart:
    """`shay update` must call `systemctl reset-failed` before every
    ...
    Mirrors the recovery pattern `shay gateway restart` (systemd_restart)
    ...
    def test_reset_failed_runs_before_fallback_restart(
        ...
        assert any(
            "shay-gateway" in " ".join(str(c) for c in call)
            for call in reset_calls
        ), (
            "Expected `systemctl reset-failed shay-gateway` before the "
        ...
        for idx, call in enumerate(mock_run.call_args_list):
            ...
            if "systemctl" in joined and "reset-failed" in joined and first_reset_idx is None:
                first_reset_idx = idx
            if "systemctl" in joined and "restart" in joined and "shay-gateway" in joined:
    ...
    def test_reset_failed_also_runs_before_retry_restart(
        ...
        gateway_restarts = [
            ...
            if "shay-gateway" in " ".join(str(a) for a in c)
        ]
        gateway_resets = [
    ...
    def test_final_failure_message_tells_user_to_reset_failed(
        ...
        assert "shay-gateway" in captured
