...
from shay_constants import OPENROUTER_BASE_URL, get_shay_home
...
if tinker_atropos_dir.exists():
    ...
    os.environ['SHAY_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to shay-shay directory if submodule not found
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.shay/config.yaml if not provided)
    ...
    """
    # Load config from ~/.shay/config.yaml
    config = load_shay_config()
