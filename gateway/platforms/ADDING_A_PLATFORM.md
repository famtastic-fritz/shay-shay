...
There are two ways to add a platform to the Shay-Shay gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.shay/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Shay-Shay code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `shay gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `shay_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Shay-Shay core.
...
And add it to the `shay-gateway` composite:

```python
"shay-gateway": {
    "includes": [..., "shay-your-platform"]
}
```
...
## 12. Status Display (`shay_cli/status.py`)
...
## 13. Gateway Setup Wizard (`shay_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ shay_cli/ toolsets.py \
  --include="*.py" -l | sort -u
