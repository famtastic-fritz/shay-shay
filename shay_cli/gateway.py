"""
Gateway subcommand for shay CLI.

Handles: shay gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_shay_home is imported lazily at call sites to avoid ImportError
# when shay_constants is cached from a pre-update version during `shay update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "shay-gateway*",
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "shay CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``shay gateway status`` from falsely counting the ``shay`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``shay gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    exclude_pids = exclude_pids | _get_ancestor_pids()
    pids: list[int] = []
    patterns = [
        "shay_cli.main gateway",
        "shay_cli.main --profile",
        "shay_cli.main -p",
        "shay_cli/main.py gateway",
        "shay_cli/main.py --profile",
        "shay_cli/main.py -p",
        "shay gateway",
        "gateway/run.py",
    ]
    current_home = str(get_shay_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"SHAY_HOME={current_home}" in command
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. SHAY_HOME
        ...
        # SHAY_HOME= in argv is.
        if "--profile " in command or " -p " in command:
            return False
        if "SHAY_HOME=" in command and f"SHAY_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``shay update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Shay-Shay profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Shay-Shay profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from shay_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "shay_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``shay update`` users
    # with no running gateway until they re-invoke ``shay gateway``
...
def _sync_shay_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``SHAY_HOME``.

    Under ``sudo``, ``SHAY_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_shay_home` falls back to ``/root/.shay`` — the wrong
    profile. The unit file pins ``SHAY_HOME`` for the actual gateway
    ...
    unit_home = env.get("SHAY_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("SHAY_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["SHAY_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}shay gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}shay gateway restart{scope_flag}")
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from shay_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `shay gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``shay gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from shay_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (SHAY_HOME-scoped).
...
from shay_constants import is_container, is_termux, is_wsl
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``shay gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``SHAY_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("SHAY_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "shay-gateway"
SERVICE_DESCRIPTION = "Shay-Shay Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current SHAY_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (SHAY_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from shay_constants import get_default_shay_root
    home = get_shay_home().resolve()
    default = get_default_shay_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary SHAY_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(shay_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when SHAY_HOME is a named profile.

    For ``~/.shay/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        shay_home: Optional explicit SHAY_HOME path. Defaults to the current
            ``get_shay_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from shay_constants import get_default_shay_root
    home = Path(shay_home or str(get_shay_home())).resolve()
    default = get_default_shay_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this SHAY_HOME.

    Default ``~/.shay`` returns ``shay-gateway`` (backward compatible).
    Profile ``~/.shay/profiles/coder`` returns ``shay-gateway-coder``.
    Any other SHAY_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Shay-Shay installs that predate the
# shay-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (shay-gateway-*.service) and unrelated third-party
# "shay" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("shay.service",)
...
def _find_legacy_shay_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Shay-Shay gateway units.

    Detects unit files installed by older Shay-Shay versions that used a
    different service name (e.g. ``shay.service`` before the rename to
    ``shay-gateway.service``). When both a legacy unit and the current
    ``shay-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``shay-gateway-coder.service`` and unrelated third-party
      ``shay-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``shay.service`` running an unrelated
...
def has_legacy_shay_units() -> bool:
    """Return True when any legacy Shay-Shay gateway unit files exist."""
    return bool(_find_legacy_shay_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Shay-Shay gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_shay_units()
    if not legacy:
        return
    print_warning("Legacy Shay-Shay gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current shay-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    shay gateway migrate-legacy")


def remove_legacy_shay_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Shay-Shay gateway unit files.

    Iterates over whatever ``_find_legacy_shay_units()`` returns — which is
    ...
    legacy = _find_legacy_shay_units()
    if not legacy:
        print("No legacy Shay-Shay gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Shay-Shay gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: shay gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo shay gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    shay gateway uninstall")
    print_info("    sudo shay gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            print_warning("  System service install requires sudo, so Shay-Shay can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo shay gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo shay gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo shay gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Shay-Shay often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.shay`` → ``ai.shay.gateway.plist`` (backward compatible).
    Profile ``~/.shay/profiles/coder`` → ``ai.shay.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.shay.gateway-{suffix}" if suffix else "ai.shay.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.shay/shay-shay  -> /home/alice/.shay/shay-shay
      /opt/shay                 -> /opt/shay  (kept as-is)
...
def _shay_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current SHAY_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_shay_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.shay                    → /home/alice/.shay
      /root/.shay/profiles/coder     → /home/alice/.shay/profiles/coder
      /opt/custom-shay               → /opt/custom-shay  (kept as-is)
    """
    current_shay = get_shay_home().resolve()
    current_default = (Path.home() / ".shay").resolve()
    target_default = Path(target_home_dir) / ".shay"

    # Default ~/.shay → remap to target user's default
    if current_shay == current_default:
        return str(target_default)

    # Profile or subdir of ~/.shay → preserve the relative structure
    try:
        relative = current_shay.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.shay) — keep as-is
        return str(current_shay)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        shay_home = _shay_home_for_target_user(home_dir)
        profile_arg = _profile_arg(shay_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m shay_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="SHAY_HOME={shay_home}"
Restart=always
RestartSec=60
...
"""
    ...
    shay_home = str(get_shay_home().resolve())
    profile_arg = _profile_arg(shay_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # sandboxed by the test conftest (only SHAY_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp SHAY_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../shay_test`` path into
    # ``Environment="SHAY_HOME=..."``. Writing that to the developer's
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Shay-Shay install")
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo shay gateway uninstall --system")
    print_info("         shay gateway install")
    print_info("         shay gateway start")
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (shay.service from pre-rename installs)
    # before installing the new shay-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}shay gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}shay gateway status{scope_flag}             # Check status")
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}shay gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_stop(system: bool = False):
    ...
    _sync_shay_home_from_systemd_unit(system=system)
    try:
    ...
    try:
        ...
        print(
            ...
            "check `shay gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `shay gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}shay gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}shay gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run after the start-limit window expires: {'sudo ' if system else ''}shay gateway restart{scope_flag}")
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}shay gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.shay.gateway-{suffix}" if suffix else "ai.shay.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Shay-Shay install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  shay gateway status             # Check status")
...
def launchd_stop():
    ...
    # `shay gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Shay-Shay install")
    else:
        print("⚠ Service definition is stale relative to the current Shay-Shay install")
        print("  Run: shay gateway start")
    ...
    if deep:
        log_file = get_shay_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("SHAY_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Shay-Shay gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'shay' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/shay/docker/entrypoint.sh before the Shay-Shay command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$SHAY_HOME and break later non-root dashboard/gateway runs."
    )
    print("  Set SHAY_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk.")
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `shay gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set SHAY_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `shay gateway restart` which already
    ...
    # `shay gateway start/restart` — leaving the gateway vulnerable to
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           ⚕ Shay-Shay Gateway Starting...                 │")
    print("├─────────────────────────────────────────────────────────┤")
    ...
    # SHAY_GATEWAY_EXIT_DIAG env var (default: on while we're still
    ...
    def _exit_diag(tag: str, **extra: object) -> None:
        if os.environ.get("SHAY_GATEWAY_EXIT_DIAG", "1") != "1":
            return
        try:
            from shay_constants import get_shay_home as _ghh
            log_dir = _ghh() / "logs"
            log_dir.mkdir(parents=True, exist_ok=True)
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Shay-Shay will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @shay:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. shay) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Shay-Shay delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Shay-Shay",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Shay-Shay will use (e.g., shay@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Shay-Shay connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: shay pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Shay-Shay will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``shay setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.shay/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_shay_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'shay pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: shay pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'shay gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from shay_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from shay_cli.dingtalk_auth import dingtalk_qr_auth
...
def _is_service_installed() -> bool:
    ...
    if supports_systemd_services():
        ...
        from shay_cli import gateway_windows
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Shay-Shay will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. Shay-Shay will store the returned account_id/token in ~/.shay/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `shay gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_shay_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `shay pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `shay pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"ShayAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``shay_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from shay_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.shay/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.shay/.env: {', '.join(required)}")
    else:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: shay gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: shay gateway install")
                else:
                    print_info("  You can install later: shay gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo shay gateway install --system")
                    print_info("  Or run in foreground:  shay gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: shay gateway run")
                print_info("  For persistence:   tmux new -s shay 'shay gateway run'")
                print_info("  To enable systemd: add systemd=true to /etc/wsl.conf, then 'wsl --shutdown'")
            elif is_termux():
                from shay_constants import display_shay_home as _dhh
                print_info("  Termux does not use systemd/launchd services.")
                print_info("  Run in foreground: shay gateway run")
                print_info(f"  Or start it manually in the background (best effort): nohup shay gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
            else:
                print_info("  Service install not supported on this platform.")
                print_info("  Run in foreground: shay gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'shay gateway setup' when ready.")
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``shay gateway install|uninstall|start|stop|restart``
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: shay gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: shay gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s shay 'shay gateway run'")
                print()
            ...
            print()
            print("  shay gateway run                              # direct foreground")
            print("  tmux new -s shay 'shay gateway run'         # persistent via tmux")
            print("  nohup shay gateway run > ~/.shay/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: shay gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: shay gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: shay gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: shay gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: shay gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  shay gateway install")
                else:
                    print("To install as a service:")
                    print("  shay gateway install")
                    print("  sudo shay gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  shay gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup shay gateway run > ~/.shay/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s shay 'shay gateway run'         # persistent via tmux")
                    print("  nohup shay gateway run > ~/.shay/logs/gateway.log 2>&1 &  # background")
                elif is_windows():
                    print("  shay gateway install  # Install as Windows Scheduled Task (auto-start on login)")
                else:
                    print("  shay gateway install  # Install as user service")
                    print("  sudo shay gateway install --system  # Install as boot-time system service")
        ...
        # Stop, disable, and remove legacy Shay-Shay gateway unit files from
        # pre-rename installs (e.g. shay.service). Profile units and
        # unrelated third-party services are never touched.
        dry_run = getattr(args, 'dry_run', False)
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_shay_units(interactive=not yes, dry_run=dry_run)
