...
- Give it a title like "shay-shay-&lt;machine-name>"
...
elif [ -f ~/.shay/.env ] && grep -q "^GITHUB_TOKEN=" ~/.shay/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.shay/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
