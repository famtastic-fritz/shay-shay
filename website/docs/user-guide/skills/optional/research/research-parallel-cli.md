...
| | |
|---|---|
| Source | Optional — install with `shay skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Shay-Shay core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Shay-Shay native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Shay-Shay native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Shay-Shay native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Shay-Shay workflow:
...
## Recommended Shay-Shay usage patterns
...
- Do not silently choose Parallel when Shay-Shay native tools are already sufficient.
