"""``shay slack ...`` CLI subcommands.

Today only ``shay slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ shay slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Shay-Shay. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Shay-Shay deployment — users can tweak them in the Slack UI after
    pasting.
    """
    from shay_cli.commands import slack_app_manifest
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Shay-Shay on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Shay-Shay in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``shay_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$SHAY_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Shay-Shay")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Shay-Shay"
    description = getattr(args, "description", None) or "Your Shay-Shay on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("SHAY_HOME") or str(Path.home() / ".shay")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Shay-Shay app\n"
