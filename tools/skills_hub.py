#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Shay-Shay Skills Hub.
...
Used by shay_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            shay_meta = metadata.get("shay", {})
            if isinstance(shay_meta, dict):
                tags = shay_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.shay/skills/ during setup.  They are discoverable via the Skills Hub
...
# ---------------------------------------------------------------------------
# Shay-Shay centralized index source
# ---------------------------------------------------------------------------

SHAY_INDEX_URL = "https://shay-shay.nousresearch.com/docs/api/skills-index.json"
SHAY_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "shay-index.json"
SHAY_INDEX_TTL = 6 * 3600  # 6 hours


def _load_shay_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for SHAY_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if SHAY_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - SHAY_INDEX_CACHE_FILE.stat().st_mtime
            if age < SHAY_INDEX_TTL:
                return json.loads(SHAY_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(SHAY_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("Shay-Shay index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("Shay-Shay index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        SHAY_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        SHAY_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class ShayIndexSource(SkillSource):
    """Skill source backed by the centralized Shay-Shay Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "shay-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "shay-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        ShayIndexSource(auth=auth), # Centralized index (search + resolved install paths)
