"""
LINE Messaging API platform adapter for Shay-Shay.
...
to Shay-Shay. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``SHAY_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            shay_home = Path.home().joinpath(".shay").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``shay status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``shay status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``shay setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.shay/.env`` via ``shay_cli.config``.
    """
    print()
    ...
    try:
        from shay_cli.config import get_env_var, set_env_var
    except ImportError:
        print("shay_cli.config not available; set LINE_* vars manually in ~/.shay/.env")
        return
