...
``<root>`` is the **shared Shay-Shay root** (the parent of any active
...
``shay -p <profile>`` joins the same board as the dispatcher that
...
the profile's Shay-Shay home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``SHAY_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``SHAY_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``shay kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.shay``. In Docker / custom
deployments where ``SHAY_HOME`` points outside ``~/.shay`` (e.g.
``/opt/shay``), ``<root>`` is ``SHAY_HOME``. Legacy env-var
overrides still work:

* ``SHAY_KANBAN_DB`` — pin the database file path directly.
* ``SHAY_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``SHAY_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``SHAY_KANBAN_DB``,
``SHAY_KANBAN_WORKSPACES_ROOT``, and ``SHAY_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/shay-kanban-v1-spec.pdf`` for the full
...
# enough that kebab-case names like ``atm10-server`` or ``shay-shay``
...
def kanban_home() -> Path:
    """Return the shared Shay-Shay root that anchors the kanban board.

    Resolution order:

    1. ``SHAY_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_shay_root()``, which already returns ``<root>``
       when ``SHAY_HOME`` is ``<root>/profiles/<name>``, and returns
       ``SHAY_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``SHAY_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("SHAY_KANBAN_HOME", "").strip()
    if override:
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``shay kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``SHAY_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``shay kanban boards
    ...
    """
    env = os.environ.get("SHAY_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``shay kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``SHAY_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("SHAY_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``SHAY_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("SHAY_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``shay kanban log`` unambiguous even when multiple
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``SHAY_KANBAN_DB`` env → ``SHAY_KANBAN_BOARD`` env →
    ...
    # See shay_state._WAL_INCOMPAT_MARKERS for detection logic.
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``shay kanban init`` and the
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from shay_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``shay --skills ...`` alongside the built-in
    ...
    # `shay --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
    ...
    for attempt in range(2):
        ...
        try:
            with write_txn(conn):
                # The lookup must share the IMMEDIATE transaction with the
                # insert. BEGIN IMMEDIATE serializes concurrent creators, so
                # only the first active task for a key emits ``created``;
                # archived rows remain reusable. Returning from this context
                # still commits normally through the context manager.
                if idempotency_key:
                    row = conn.execute(
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `shay kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``shay kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``shay kanban complete <ready-task>
...
def recompute_ready(conn: sqlite3.Connection) -> int:
    """Promote ``todo`` tasks to ``ready`` when every parent is ``done``.
...
def complete_task(
    ...
    *,
    ...
    completion (``shay kanban complete <id>``) works without requiring
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Shay-Shay
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``shay kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from shay_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        # Skip ready tasks whose assignee is not a real Shay-Shay profile.
        # `_default_spawn` invokes ``shay -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Shay-Shay
...
def _resolve_shay_argv() -> list[str]:
    """Resolve the ``shay`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``shutil.which("shay")`` — the console-script shim, the same form
       that shows up in ``ps`` output and existing logs. Preferred so live
       systems' diagnostics stay familiar.
    2. ``sys.executable -m shay_cli.main`` — fallback for setups where
       Shay-Shay is launched from a venv and the ``shay`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_shay_bin`` for the same reason. Kept
    local (not imported from gateway) because ``shay_cli`` sits below
    ...
    if shay_bin:
        return [shay_bin]
    # Fallback to the module form. ``shay_cli.main`` is the actual
    # console-script target declared in pyproject.toml, NOT a top-level
    # ``shay`` package — there is no ``shay`` package to import.
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``shay -p <profile> chat -q ...`` subprocess.
    ...
    ``SHAY_KANBAN_DB`` / ``SHAY_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject SHAY_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `shay -p <name>` the
    # _apply_profile_override() runs *before* shay_constants is imported.
    # If SHAY_HOME is absent from the child's env, get_shay_home() falls
    # back to Path.home() / ".shay" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from shay_cli.profiles import resolve_profile_env
    try:
        env["SHAY_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via SHAY_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # SHAY_HOME never had profiles created.
        pass
    if task.tenant:
        env["SHAY_TENANT"] = task.tenant
    env["SHAY_KANBAN_TASK"] = task.id
    env["SHAY_KANBAN_WORKSPACE"] = workspace
    if task.current_run_id is not None:
        env["SHAY_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["SHAY_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Pin the shared board + workspaces root the dispatcher resolved, so
    # that even when the worker activates a profile (`shay -p <name>`
    # rewrites SHAY_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_shay_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["SHAY_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["SHAY_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["SHAY_KANBAN_BOARD"] = resolved_board
    # SHAY_PROFILE is the author the kanban_comment tool defaults to.
    # `shay -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["SHAY_PROFILE"] = profile_arg
    ...
    # `shay kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`shay` executable not found on PATH. "
            "Install Shay-Shay or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``shay kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled SHAY_PROFILE values like "shay-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Shay-Shay root exists

    Reads profile paths directly so this module has no import dependency on
    ``shay_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from shay_constants import get_default_shay_root
        default_root = get_default_shay_root()
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``shay kanban assignees`` for the terminal.
