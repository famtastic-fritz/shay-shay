...
def _make_socket_dir(tmpdir, session_name, pid=None, owner_pid=None):
    ...
    Args:
        ...
        owner_pid: owning shay PID to write to <session>.owner_pid
...
class TestReapOrphanedBrowserSessions:
    ...
    def test_non_shay_dirs_are_ignored(self, fake_tmpdir):
...
class TestOwnerPidCrossProcess:
    ...
    The owner_pid file records which shay process owns a daemon so that
    concurrent shay processes don't reap each other's active browser
