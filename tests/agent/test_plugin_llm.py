...
""",
        ...
        from shay_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        shay_home = tmp_path / ".shay"
        shay_home.mkdir()
        (shay_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("SHAY_HOME", str(shay_home))
        from shay_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from shay_cli.plugins import PluginContext, PluginManifest, PluginManager
