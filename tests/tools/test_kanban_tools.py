...
Verifies:
  - Tools are gated on SHAY_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `shay chat` sessions (no SHAY_KANBAN_TASK) must have
    zero kanban_* tools in their schema."""
    monkeypatch.delenv("SHAY_KANBAN_TASK", raising=False)
    home = tmp_path / ".shay"
    home.mkdir()
    ...
    schema = registry.get_definitions(set(resolve_toolset("shay-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SHAY_KANBAN_TASK", "t_fake")
    home = tmp_path / ".shay"
    home.mkdir()
    monkeypatch.setenv("SHAY_HOME", str(home))
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the SHAY_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: SHAY_HOME isolated, SHAY_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".shay"
    home.mkdir()
    monkeypatch.setenv("SHAY_HOME", str(home))
    monkeypatch.setenv("SHAY_PROFILE", "test-worker")
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("SHAY_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["shay_cli/kanban.py"],
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to SHAY_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``SHAY_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``shay-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "shay-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from SHAY_PROFILE in the fixture, not the
        # caller-supplied "shay-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no SHAY_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (SHAY_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has SHAY_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no SHAY_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # SHAY_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("SHAY_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("SHAY_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no SHAY_KANBAN_TASK) can still complete
