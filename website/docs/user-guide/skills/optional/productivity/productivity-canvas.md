...
| | |
|---|---|
| Source | Optional — install with `shay skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Shay-Shay"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `~/.shay/.env`:
...
```bash
CANVAS="python $SHAY_HOME/skills/productivity/canvas/scripts/canvas_api.py"
