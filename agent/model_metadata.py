...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Shay-Shay also honours SHAY_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("SHAY_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
# Minimum context length required to run Shay-Shay.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # so these hardcoded fallbacks prevent Shay-Shay from probing-down to
...
def _get_context_cache_path() -> Path:
    ...
    return get_shay_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Shay-Shay grow conversations past the runtime limit and Ollama
...
def get_model_context_length(
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from shay_cli.models import get_copilot_model_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Shay-Shay sends to providers:
