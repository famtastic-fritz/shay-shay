...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.shay."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /shay, /btw, /stop,
        ...
        for expected in ("/shay", "/btw", "/stop", "/model", "/help"):
...
class TestSlashCommands:
    ...
    # instead of as /shay subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /shay btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_shay_prefix_still_works(self, adapter):
        """Backward compat: /shay btw foo must still route to /btw foo.

        Old workspace manifests only declared /shay as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_shay_freeform_question(self, adapter):
        """/shay <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/shay",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_shay_slash_stashes_context(self, adapter):
        """Legacy /shay <subcommand> also stashes context."""
        command = {
            "command": "/shay",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_shay_question_does_not_stash_context(self, adapter):
        """Free-form /shay <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/shay",
