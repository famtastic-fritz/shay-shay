"""
Shay-Shay — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m shay_cli.main web          # Start on http://127.0.0.1:9119
    python -m shay_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["SHAY_WEB_DIST"]) if "SHAY_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Shay-Shay", version=__version__)
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "tts.provider": {
        ...
        "options": [
            "macos", "edge", "elevenlabs", "openai", "minimax", "mistral",
            "gemini", "xai", "neutts", "kittentts", "piper",
        ],
    },
    "stt.provider": {
        "type": "select",
        "description": "Speech-to-text provider",
        "options": ["local", "local_command", "groq", "openai", "mistral", "xai"],
    },
    "display.skin": {
        "type": "select",
...
# streamed to a per-action log file under ``~/.shay/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_shay_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "shay-update": "shay-update.log",
}
...
def _spawn_shay_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``shay <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``shay_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "shay_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "SHAY_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``shay gateway restart`` in the background."""
    try:
        proc = _spawn_shay_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/shay/update")
async def update_shay():
    """Kick off ``shay update`` in the background."""
    try:
        proc = _spawn_shay_action(["update"], "shay-update")
    except Exception as exc:
        _log.exception("Failed to spawn shay update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Shay-Shay supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in shay_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.shay/config.yaml`` — applies to **new** sessions only.
...
# the canonical ``shay auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Shay-Shay resolves Anthropic creds in this order at runtime:
    1. ``~/.shay/.anthropic_oauth.json`` — Shay-Shay-managed PKCE flow
    ...
    try:
        ...
        read_shay_oauth_credentials = None  # type: ignore
        _SHAY_OAUTH_FILE = None  # type: ignore

    shay_creds = None
    if read_shay_oauth_credentials:
        try:
            shay_creds = read_shay_oauth_credentials()
        except Exception:
            shay_creds = None
    if shay_creds and shay_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "shay_pkce",
            "source_label": f"Shay-Shay PKCE ({_SHAY_OAUTH_FILE})",
            "token_preview": _truncate_token(shay_creds.get("accessToken")),
            "expires_at": shay_creds.get("expiresAt"),
            "has_refresh_token": bool(shay_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Shay-Shay even
    when they also have a separate Shay-Shay-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "shay auth add anthropic",
    ...
    },
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "shay auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "shay auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "shay auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "shay auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from shay_cli import auth as hauth
        if provider_id == "nous":
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("shay_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Shay-Shay-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _SHAY_OAUTH_FILE
            if _SHAY_OAUTH_FILE.exists():
                _SHAY_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from shay_cli.auth import clear_provider_auth
...
#          → persists to ~/.shay/.anthropic_oauth.json AND credential pool
...
# Guarded so shay web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Shay-Shay file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``shay auth add anthropic``.
    """
    from agent.anthropic_adapter import _SHAY_OAUTH_FILE
    payload = {
    ...
    _SHAY_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _SHAY_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from shay_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("SHAY_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from shay_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from shay_cli.auth import persist_nous_credentials
...
async def get_logs(
    ...
    from shay_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_shay_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from shay_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_shay_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from shay_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "shay setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from shay_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from shay_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``shay --tui`` binary the CLI uses, behind
...
try:
    from shay_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``shay --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``SHAY_TUI_RESUME`` env var —
    matching what ``shay_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``SHAY_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from shay_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("SHAY_TUI_DISABLE_MOUSE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["SHAY_TUI_RESUME"] = resume

    if sidecar_url:
        env["SHAY_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Shay-Shay inside WSL2 to use the dashboard's /chat "
...
# SHAY_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/shay"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/shay/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /shay`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__SHAY_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/shay``)
    ...
    # under ``/shay`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Shay-Shay backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Shay-Shay Teal",         "description": "Classic dark teal — the canonical Shay-Shay look"},
    {"name": "default-large", "label": "Shay-Shay Teal (Large)", "description": "Shay-Shay Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.shay/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.shay/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_shay_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.shay/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as shay_cli.plugins):
    1. User plugins:    ~/.shay/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.shay/plugins/  (only if SHAY_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()
    ...
    search_dirs = [
        (get_shay_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("SHAY_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".shay" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from shay_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_shay_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from shay_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from shay_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from shay_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from shay_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"shay_dashboard_plugin_{plugin['name']}"
