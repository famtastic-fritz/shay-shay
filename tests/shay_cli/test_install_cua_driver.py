...
* Be macOS-only — no-op silently on Linux/Windows so ``shay update`` can
...
class TestInstallCuaDriverUpgrade:
    def test_upgrade_on_non_macos_is_silent_noop(self):
        """``shay update`` calls install_cua_driver(upgrade=True) for every
        ...
        from shay_cli import tools_config
    ...
    def test_upgrade_on_macos_with_binary_runs_installer(self):
        ...
        with patch("platform.system", return_value="Darwin"), \
            ...
            # "grant macOS permissions" block on every `shay update`.
    ...
    def test_non_upgrade_on_macos_with_binary_skips_install(self):
        ...
        This is the behaviour that ``shay tools`` (re)enable depends on,
