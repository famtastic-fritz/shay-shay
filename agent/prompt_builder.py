...
from shay_constants import get_shay_home, get_skills_dir, is_wsl
...
_SHAY_MD_NAMES = (".shay.md", "SHAY.md")


def _find_shay_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.shay.md`` or ``SHAY.md``.
...
SHAY_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Shay-Shay "
    "itself, load the `shay-shay` skill with skill_view(name='shay-shay') "
    "before answering. Docs: https://shay-shay.nousresearch.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.shay/kanban.db`. Your task id is in "
    "`$SHAY_KANBAN_TASK`; your workspace is `$SHAY_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $SHAY_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `shay kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Shay-Shay WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Shay-Shay itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Shay-Shay runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Shay-Shay itself is running. The host OS, home, and cwd "
                f"of the Shay-Shay process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Shay-Shay "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_shay_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    *lane* selects which scoping lane this prompt is for:
    ...
    scanned alongside the local ``~/.shay/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("SHAY_PLATFORM")
        or get_session_env("SHAY_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Shay-Shay itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `shay-shay` skill "
            "first. It has the actual commands (e.g. `shay config set …`, `shay tools`, "
            "`shay setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: shay setup, shay setup tools, shay setup terminal, shay status.",
        ]
    )
    return "\n".join(lines)
...
def _load_shay_md(cwd_path: Path) -> str:
    ...
    if not shay_md_path:
        return ""
    try:
        content = shay_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = shay_md_path.name
        try:
            rel = str(shay_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".shay.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", shay_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    SOUL.md from SHAY_HOME is independent and always included when present.
    ...
    # SOUL.md from SHAY_HOME only — skip when already loaded as identity
    if not skip_soul:
