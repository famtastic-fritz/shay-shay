...
def container_env(tmp_path, monkeypatch):
    """Set up a fake SHAY_HOME with .container-mode file."""
    shay_home = tmp_path / ".shay"
    shay_home.mkdir()
    monkeypatch.setenv("SHAY_HOME", str(shay_home))
    monkeypatch.delenv("SHAY_DEV", raising=False)

    container_mode = shay_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=shay-shay\n"
        "exec_user=shay\n"
        "shay_bin=/data/current-package/bin/shay\n"
    )
    return shay_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("shay_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "shay-shay"
    assert info["exec_user"] == "shay"
    assert info["shay_bin"] == "/data/current-package/bin/shay"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("shay_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_shay_dev(container_env, monkeypatch):
    """Returns None when SHAY_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("SHAY_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_shay_dev_zero(container_env, monkeypatch):
    """SHAY_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("SHAY_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        shay_home = Path(tmpdir) / ".shay"
        shay_home.mkdir()
        (shay_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("shay_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_shay_home": lambda: shay_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHAY_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=shay-custom\n"
        "exec_user=myuser\n"
        "shay_bin=/opt/shay/bin/shay\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "shay-custom"
    assert info["exec_user"] == "myuser"
    assert info["shay_bin"] == "/opt/shay/bin/shay"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from shay_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "shay"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "shay-shay" in cmd
    assert "/data/current-package/bin/shay" in cmd
    assert "chat" in cmd
