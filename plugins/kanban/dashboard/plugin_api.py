...
``shay_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__SHAY_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``shay_cli/web_server.py``.

This means ``shay dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from shay_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from shay_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``shay_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from shay_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``SHAY_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from shay_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``shay kanban
...
# spawn failures, stuck-blocked). See shay_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``shay kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``shay kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``shay kanban specify <task_id>``.
    ...
    prev_env = os.environ.get("SHAY_KANBAN_BOARD")
    try:
        os.environ["SHAY_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from shay_cli import kanban_specify  # noqa: WPS433 (intentional)
        ...
        if prev_env is None:
            ...
            os.environ["SHAY_KANBAN_BOARD"] = prev_env
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``shay kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.shay/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.shay/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in shay_cli/web_server.py.
