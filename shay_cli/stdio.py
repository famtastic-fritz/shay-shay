...
on Japanese locales, etc.).  Shay-Shay's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``shay_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Shay-Shay is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``SHAY_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("SHAY_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Shay-Shay-managed Git install directories so
    ...
    # already-running shells don't see the change — which means shay
    ...
    # install without Shay-Shay-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Shay-Shay's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Shay-Shay's
    ``shay config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Shay-Shay (User env var in Windows Settings, or
    export in a PowerShell profile) and Shay-Shay picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Shay-Shay-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\shay\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``shay`` from right after install) retain their old PATH.

    Any subprocess Shay-Shay spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Shay-Shay-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "shay", "git", "cmd"),
        os.path.join(local_appdata, "shay", "git", "bin"),
        os.path.join(local_appdata, "shay", "git", "usr", "bin"),
        # Shay-Shay venv Scripts directory — host of the shay.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes shay, but harmless to include.
        os.path.join(local_appdata, "shay", "shay-shay", "venv", "Scripts"),
