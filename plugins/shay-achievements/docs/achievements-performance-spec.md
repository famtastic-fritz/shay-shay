# Shay-Shay Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: shay-achievements plugin
...
### `GET /api/plugins/shay-achievements/achievements`
...
### `POST /api/plugins/shay-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/shay-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/shay-achievements/overview`
...
- Persist plugin data under:
  - `~/.shay/plugins/shay-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.shay/plugins/shay-achievements/scan_checkpoint.json`
...
- `~/.shay/plugins/shay-achievements/`
