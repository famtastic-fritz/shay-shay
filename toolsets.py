...
_SHAY_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (SHAY_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (SHAY_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Shay-Shay toolsets (CLI + messaging platforms)
    #
    ...
    "shay-cli": {
        ...
        "tools": _SHAY_CORE_TOOLS,
        "includes": []
    },

    "shay-cron": {
        # Mirrors shay-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `shay tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant, rl) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as shay-cli; gated by `shay tools`",
        "tools": _SHAY_CORE_TOOLS,
        "includes": []
    },
    ...
    "shay-email": {
        "description": "Email bot toolset - interact with Shay-Shay via email (IMAP/SMTP)",
        "tools": _SHAY_CORE_TOOLS,
        "includes": []
    },
    ...
    "shay-sms": {
        "description": "SMS bot toolset - interact with Shay-Shay via SMS (Twilio)",
        "tools": _SHAY_CORE_TOOLS,
        "includes": []
    },
    ...
    "shay-gateway": {
        ...
        "tools": [],
        "includes": ["shay-telegram", "shay-discord", "shay-whatsapp", "shay-slack", "shay-signal", "shay-bluebubbles", "shay-homeassistant", "shay-email", "shay-sms", "shay-mattermost", "shay-matrix", "shay-dingtalk", "shay-feishu", "shay-wecom", "shay-wecom-callback", "shay-weixin", "shay-qqbot", "shay-webhook", "shay-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (shay-<name>).
        # Gives them _SHAY_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("shay-"):
            platform_name = name[len("shay-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_SHAY_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
