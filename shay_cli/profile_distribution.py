"""Profile distributions — shareable, packaged Shay-Shay profiles via git.

A distribution is a Shay-Shay profile published as a git repository (or
...
* ``shay profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``shay skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``shay profile``, not a parallel tree):

    shay profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "shay-shay", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            shay_requires=str(data.get("shay_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.shay_requires:
            out["shay_requires"] = self.shay_requires
        if self.author:
...
def check_shay_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Shay-Shay {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``shay profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Shay-Shay profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Shay-Shay distribution."
        )

    # Version check up-front so we fail fast
    check_shay_requires(manifest.shay_requires, shay_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.shay).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="shay_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `shay profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`shay profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="shay_dist_update_") as tmp:
        plan = plan_install(
