"""CLI entry point for the shay-shay ACP adapter.

Loads environment variables from ``~/.shay/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from SHAY_HOME (default ``~/.shay``)."""
    from shay_cli.env_loader import load_shay_dotenv

    shay_home = get_shay_home()
    loaded = load_shay_dotenv(shay_home=shay_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", shay_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting shay-shay ACP adapter")
    ...
    import acp
    from .server import ShayACPAgent
