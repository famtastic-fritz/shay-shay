...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_shay_home() / "sessions")
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.shay/config.yaml (primary user-facing config)
    3. ~/.shay/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_shay_home()
    gw_data: dict = {}
