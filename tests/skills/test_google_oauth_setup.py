...
class TestShayConstantsFallback:
    """Tests for _shay_home.py fallback when shay_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_shay_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _shay_home.py with shay_constants blocked."""
        monkeypatch.setitem(sys.modules, "shay_constants", None)
        spec = importlib.util.spec_from_file_location("_shay_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_shay_home_env_var(self, monkeypatch, tmp_path):
        """When shay_constants is missing, SHAY_HOME comes from env var."""
        monkeypatch.setenv("SHAY_HOME", str(tmp_path / "custom-shay"))
        module = self._load_helper(monkeypatch)
        assert module.get_shay_home() == tmp_path / "custom-shay"

    def test_fallback_defaults_to_dot_shay(self, monkeypatch):
        """When shay_constants is missing and SHAY_HOME unset, default to ~/.shay."""
        monkeypatch.delenv("SHAY_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_shay_home() == Path.home() / ".shay"

    def test_fallback_ignores_empty_shay_home(self, monkeypatch):
        """Empty/whitespace SHAY_HOME is treated as unset."""
        monkeypatch.setenv("SHAY_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_shay_home() == Path.home() / ".shay"

    def test_fallback_display_shay_home_shortens_path(self, monkeypatch):
        """Fallback display_shay_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("SHAY_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_shay_home() == "~/.shay"

    def test_fallback_display_shay_home_profile_path(self, monkeypatch):
        """Fallback display_shay_home() handles profile paths under ~/."""
        monkeypatch.setenv("SHAY_HOME", str(Path.home() / ".shay/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_shay_home() == "~/.shay/profiles/coder"

    def test_fallback_display_shay_home_custom_path(self, monkeypatch):
        """Fallback display_shay_home() returns full path for non-home locations."""
        monkeypatch.setenv("SHAY_HOME", "/opt/shay-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_shay_home() == "/opt/shay-custom"

    def test_delegates_to_shay_constants_when_available(self):
        """When shay_constants IS importable, _shay_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_shay_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import shay_constants
        assert module.get_shay_home is shay_constants.get_shay_home
        assert module.display_shay_home is shay_constants.display_shay_home
