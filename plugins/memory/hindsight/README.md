...
Shay-Shay spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.shay/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p shay ui start
```
...
Config file: `~/.shay/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `shay` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `shay-{profile}` isolates memory per active Shay-Shay profile. Empty placeholders collapse cleanly (e.g. `shay-{user}` with no user becomes `shay`). |
...
The LLM API key is stored in `~/.shay/.env` as `HINDSIGHT_LLM_API_KEY`.
