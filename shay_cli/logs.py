"""``shay logs`` — view and filter Shay-Shay log files.
...
Usage examples::
    ...
    shay logs                    # last 50 lines of agent.log
    shay logs -f                 # follow agent.log in real time
    shay logs errors             # last 50 lines of errors.log
    shay logs gateway -n 100    # last 100 lines of gateway.log
    shay logs --level WARNING    # only WARNING+ lines
    shay logs --session abc123   # filter by session ID substring
    shay logs --component tools  # only tool-related lines
    shay logs --since 1h         # lines from the last hour
    shay logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_shay_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Shay-Shay runs — try 'shay chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_shay_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_shay_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_shay_home()}/logs/")
        return

    print(f"Log files in {display_shay_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'shay chat' to generate logs)")
