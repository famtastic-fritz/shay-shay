...
def _clean_state():
    ...
    saved = {}
    for k in ("SHAY_INTERACTIVE", "SHAY_GATEWAY_SESSION", "SHAY_EXEC_ASK", "SHAY_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["SHAY_INTERACTIVE"] = "1"
        session_key = os.getenv("SHAY_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No SHAY_INTERACTIVE or SHAY_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
