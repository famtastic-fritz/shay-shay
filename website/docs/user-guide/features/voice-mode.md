...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Shay-Shay — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Shay-Shay supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Shay-Shay](/docs/guides/use-voice-mode-with-shay).

## Prerequisites

Before using voice features, make sure you have:

1. **Shay-Shay installed** — `pip install shay-shay` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `shay model` or set your preferred provider credentials in `~/.shay/.env`
3. **A working base setup** — run `shay` to verify the agent responds to text before enabling voice

:::tip
The `~/.shay/` directory and default `config.yaml` are created automatically the first time you run `shay`. You only need to create `~/.shay/.env` manually for API keys.
:::
...
pip install "shay-shay[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "shay-shay[messaging]"

# Premium TTS (ElevenLabs)
pip install "shay-shay[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "shay-shay[all]"
```
...
Voice mode is available in both the **classic CLI** (`shay chat`) and the **TUI** (`shay --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.shay/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
shay                # Start the interactive CLI
```
...
1. Start the CLI with `shay` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.shay/config.yaml` (default: `ctrl+b`).
:::
...
```bash
shay gateway        # Start the gateway (connects to configured platforms)
shay gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@shaybyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.shay/.env`:
```bash
...
```bash
shay gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.shay/.env`
