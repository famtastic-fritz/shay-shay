...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``SHAY_PLATFORM`` env var and
    ``SHAY_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("SHAY_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.shay.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_shay_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.shay/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.shay/skills/`` and any ``skills.external_dirs`` so the
