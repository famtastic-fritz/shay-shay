---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Shay-Shay"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Shay-Shay"
---
...
Migrate a user's OpenClaw customization footprint into Shay-Shay. Imports Shay-Shay-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shay skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Shay-Shay (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Shay-Shay`, `Memory`, `Persona`, `Import` |
| Related skills | [`shay-shay`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-shay-shay) |
...
# OpenClaw -> Shay-Shay Migration

Use this skill when a user wants to move their OpenClaw setup into Shay-Shay with minimal manual cleanup.
...
```bash
shay claw migrate              # Full interactive migration
...
shay claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `shay setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_shay.py` to:

- import `SOUL.md` into the Shay-Shay home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Shay-Shay memory entries
- merge OpenClaw command approval patterns into Shay-Shay `command_allowlist`
- migrate Shay-Shay-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.shay/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Shay-Shay workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.shay/tts/`
- archive non-secret docs that do not have a direct Shay-Shay destination
...
- `scripts/openclaw_to_shay.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.shay/skills/migration/openclaw-migration/scripts/openclaw_to_shay.py`

Do not guess a shorter path like `~/.shay/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.shay/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Shay-Shay-compatible secrets, currently:
...
Shay-Shay CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Shay-Shay. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Shay-Shay workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Shay-Shay skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.shay/skills/migration/openclaw-migration/scripts/openclaw_to_shay.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.shay/skills/migration/openclaw-migration/scripts/openclaw_to_shay.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.shay/skills/migration/openclaw-migration/scripts/openclaw_to_shay.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.shay/skills/migration/openclaw-migration/scripts/openclaw_to_shay.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.shay/skills/migration/openclaw-migration/scripts/openclaw_to_shay.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.shay/skills/migration/openclaw-migration/scripts/openclaw_to_shay.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Shay-Shay unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Shay-Shay targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Shay-Shay destination. Unsupported auth blobs must still be reported as skipped.
...
- Shay-Shay persona state imported
- Shay-Shay memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.shay/skills/openclaw-imports/`
