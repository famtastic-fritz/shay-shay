"""
Email platform adapter for the Shay-Shay gateway.

Allows users to interact with Shay-Shay by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "shay-shay" "version" "{_shay_version}" '
            '"vendor" "NousResearch" '
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Shay-Shay")
        ...
        msg_id = f"<shay-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
