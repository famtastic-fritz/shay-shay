...
class TestToolsetConsistency:
    ...
    def test_shay_platforms_share_core_tools(self):
        """All shay-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on shay-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["shay-cli", "shay-telegram", "shay-discord", "shay-whatsapp", "shay-slack", "shay-signal", "shay-homeassistant"]
