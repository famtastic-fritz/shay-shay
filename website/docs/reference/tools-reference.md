...
description: "Authoritative reference for Shay-Shay built-in tools, grouped by toolset"
---

# Built-in Tools Reference

This page documents Shay-Shay' built-in tools, grouped by toolset. Availability varies by platform, credentials, and enabled toolsets.
...
:::tip MCP Tools
In addition to built-in tools, Shay-Shay can load tools dynamically from MCP servers. MCP tools appear with the prefix `mcp_<server>_` (e.g., `mcp_github_create_issue` for the `github` MCP server). See [MCP Integration](/docs/user-guide/features/mcp) for configuration.
:::
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `execute_code` | Run a Python script that can call Shay-Shay tools programmatically. Use this when you need 3+ tool calls with processing logic between them, need to filter/reduce large tool outputs before they enter your context, need conditional branching (… | — |
...
Scoped to the Feishu document-comment intelligent-reply handler (`gateway/platforms/feishu_comment.py`). Not exposed on `shay-cli` or the regular Feishu chat adapter.
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `computer_use` | Background macOS desktop control via cua-driver — screenshots (SOM / vision / AX), click / drag / scroll / type / key / wait, list_apps, focus_app. Does NOT steal the user's cursor or keyboard focus. Works with any tool-capable model. macOS only. | `cua-driver` on `$PATH` (install via `shay tools`). |
...
Registered only when the agent is spawned by the kanban dispatcher (`SHAY_KANBAN_TASK` env set). Lets workers mark tasks done with structured handoffs, block for human input, heartbeat during long ops, comment on threads, and (for orchestrators) fan out into child tasks. See [Kanban Multi-Agent](/docs/user-guide/features/kanban) for the full workflow.

| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `kanban_show` | Show the active kanban task assigned to this worker (title, description, comments, dependencies). | `SHAY_KANBAN_TASK` |
| `kanban_complete` | Mark the current task done with a structured handoff payload (results, artifacts, follow-ups). | `SHAY_KANBAN_TASK` |
| `kanban_block` | Block the current task on a question for the user — the dispatcher pauses, surfaces the question, and resumes once a human replies. | `SHAY_KANBAN_TASK` |
| `kanban_heartbeat` | Send a progress heartbeat during a long-running operation so the dispatcher knows the worker is still alive. | `SHAY_KANBAN_TASK` |
| `kanban_comment` | Add a comment to the task thread without changing its state — useful for surfacing intermediate findings. | `SHAY_KANBAN_TASK` |
| `kanban_create` | (Orchestrator only) Fan out child tasks from the current task. | `SHAY_KANBAN_TASK` + orchestrator role |
| `kanban_link` | (Orchestrator only) Link related tasks together (blocks/blocked-by/related). | `SHAY_KANBAN_TASK` + orchestrator role |
...
| Tool | Description | Requires environment |
|------|-------------|----------------------|
| `skill_manage` | Manage skills (create, update, delete). Skills are your procedural memory — reusable approaches for recurring task types. New skills go to ~/.shay/skills/; existing skills can be modified wherever they live. Actions: create (full SKILL.m… | — |
...
Opt-in toolset (not loaded in the default `shay-cli` set). Add via `--toolsets video` or include `video` in your `toolsets:` config.
...
Registered on the `shay-discord` platform toolset (gateway only). Uses the same bot token as the messaging adapter.
...
Registered on the `shay-discord` platform toolset. Moderation actions require the bot to hold the matching Discord permissions.
...
Registered by the bundled `spotify` plugin. Requires an OAuth token — run `shay spotify setup` once to authorize.
...
## `shay-yuanbao` toolset

Registered only on the `shay-yuanbao` platform toolset. Yuanbao is Tencent's chat app; these tools drive its DM/group/sticker APIs.
