...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_shay_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /shay prefix). A single regex
            ...
            # manifest (see `shay slack manifest`). In Socket Mode, Slack
            ...
            from shay_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/shay$")

            @self._app.command(_slash_pattern)
            async def handle_shay_command(ack, command):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = f":thread: Shay-Shay handoff — *{(name or 'session').strip()[:80]}*"
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Shay-Shay session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "shay_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "shay_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "shay_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "shay_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "shay_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "shay_confirm_once": "once",
            "shay_confirm_always": "always",
            "shay_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "shay_approve_once": "once",
            "shay_approve_session": "session",
            "shay_approve_always": "always",
            "shay_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/shay <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/shay
        ...
        if slash_name in {"shay", ""}:
            # Legacy /shay <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from shay_cli.commands import slack_subcommand_map
        ...
        # "Running /cmd…" ack shown by handle_shay_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/shay <question>" must produce public replies so
