...
| | |
|---|---|
| Source | Optional — install with `shay skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${SHAY_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${SHAY_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${SHAY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client shay
```
...
```bash
PUBLISH="${SHAY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client shay
```
...
```bash
DRIVE="${SHAY_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `shay`)    |
