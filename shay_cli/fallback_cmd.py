"""
shay fallback — manage the fallback provider chain.
...
Subcommands:
  shay fallback [list]   Show the current fallback chain (default when no subcommand)
  shay fallback add      Pick provider + model via the same picker as `shay model`,
                           then append the selection to the chain
  shay fallback remove   Pick an entry to delete from the chain
  shay fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.shay/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from shay_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `shay model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  shay fallback add")
        print()
        return
    ...
    print("  Docs: https://shay-shay.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `shay model`, then append the selection to the chain."""
    from shay_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `shay model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `shay fallback list` to view, or `shay fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``shay fallback [subcommand]``."""
