...
Auto-tracks and cleans up ephemeral files created during Shay-Shay
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$SHAY_HOME` and `/tmp/shay-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `SHAY_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `SHAY_HOME` or `/tmp/shay-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$SHAY_HOME/disk-cleanup/` is itself excluded
- `$SHAY_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
