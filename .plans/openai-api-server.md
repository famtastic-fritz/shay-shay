# OpenAI-Compatible API Server for Shay-Shay
...
SSE streaming. By exposing this endpoint, shay-shay becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  shay-shay   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.shay/.env`
2. Run `shay gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with shay-shay through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns shay-shay as a model) |
...
Authorization: Bearer shay-api-key-here
...
But shay-shay sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "shay-shay"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured shay-shay model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of shay-shay's core prompt
...
Once implemented, shay-shay works as a drop-in backend for:
