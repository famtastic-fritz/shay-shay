...
description: "How to add a new inference provider to Shay-Shay — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Shay-Shay can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `shay model` menu entries
...
1. `shay_cli/auth.py` decides how credentials are found.
2. `shay_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `shay_cli/models.py` and `shay_cli/main.py` make the provider show up in the CLI. (`shay_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `shay_cli/auth.py`
2. `shay_cli/models.py`
3. `shay_cli/runtime_provider.py`
4. `shay_cli/main.py`
...
:::tip
`shay_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `shay setup`.
:::
...
2. That's it. Provider plugins auto-load the first time anything calls `get_provider_profile()` or `list_providers()` — bundled plugins (this repo) and user plugins at `$SHAY_HOME/plugins/model-providers/` both get picked up.
...
7. `shay model` menu includes the provider
8. `shay setup` wizard delegates to `main.py` automatically
9. `provider:model` alias syntax works
10. Runtime resolver returns the correct `base_url` and `api_key`
11. `SHAY_INFERENCE_PROVIDER` env-var override accepts the provider id
12. Fallback model activation can switch into the provider cleanly

User plugins at `$SHAY_HOME/plugins/model-providers/<name>/` override bundled plugins of the same name (last-writer-wins in `register_provider()`) — so third parties can monkey-patch or replace any built-in profile without editing the repo.
...
- Provider-specific `shay model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `shay_cli/auth.py`
- `_PROVIDER_LABELS` in `shay_cli/models.py`
- `_PROVIDER_ALIASES` in both `shay_cli/auth.py` and `shay_cli/models.py`
- CLI `--provider` choices in `shay_cli/main.py`
...
## Step 2: Add auth metadata in `shay_cli/auth.py`
...
- What env vars should Shay-Shay check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `shay_cli/models.py`
...
## Step 4: Resolve runtime data in `shay_cli/runtime_provider.py`
...
Be careful with API-key precedence. Shay-Shay already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `shay_cli/main.py`

A provider is not discoverable until it shows up in the interactive `shay model` flow.

Update these in `shay_cli/main.py`:
...
:::tip
`shay_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `shay model` and `shay setup` automatically.
:::
...
When you add a native provider, double-check that Shay-Shay is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m shay_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `shay_cli/auth.py`
- [ ] aliases added in `shay_cli/auth.py` and `shay_cli/models.py`
- [ ] model catalog added in `shay_cli/models.py`
- [ ] runtime branch added in `shay_cli/runtime_provider.py`
- [ ] CLI wiring added in `shay_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `shay model` but not `shay setup`
