...
AlexKucera reported on Discord (2026-04-16) that after ``shay update`` pulled
...
Opt-in — not part of default CI:
    SHAY_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.shay/.env).
"""
...
# Load ~/.shay/.env so live runs pick up OPENROUTER_API_KEY without
...
LIVE = os.environ.get("SHAY_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set SHAY_LIVE_TESTS=1"),
