...
title: "SMS (Twilio)"
description: "Set up Shay-Shay as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Shay-Shay connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `pip install 'shay-shay[sms]'`
...
If you're running Shay-Shay locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Shay-Shay will respond via SMS.
...
Shay-Shay validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Shay-Shay gateway logs for Twilio API errors
