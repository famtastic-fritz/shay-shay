...
it is automatically downloaded from GitHub releases to $SHAY_HOME/bin/tirith.
...
def _get_shay_home() -> str:
    """Return the Shay-Shay home directory, respecting SHAY_HOME env var."""
    return str(get_shay_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_shay_home(), ".tirith-install-failed")
...
def _shay_bin_dir() -> str:
    """Return $SHAY_HOME/bin, creating it if needed."""
    d = os.path.join(_get_shay_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $SHAY_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_shay_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $SHAY_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $SHAY_HOME/bin/tirith
    ...
    shay_bin = os.path.join(_shay_bin_dir(), "tirith")
    if os.path.isfile(shay_bin) and os.access(shay_bin, os.X_OK):
        _resolved_path = shay_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return shay_bin
