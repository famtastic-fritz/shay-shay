...
from shay_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``SHAY_PLATFORM`` or
            ``SHAY_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``shay`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.shay/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``shay`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against SHAY_HOME, not cwd
        if not p.is_absolute():
            p = (shay_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.shay/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    shay = metadata.get("shay") or {}
    if not isinstance(shay, dict):
        shay = {}
    return {
        "fallback_for_toolsets": shay.get("fallback_for_toolsets", []),
        "requires_toolsets": shay.get("requires_toolsets", []),
        "fallback_for_tools": shay.get("fallback_for_tools", []),
        "requires_tools": shay.get("requires_tools", []),
    }
