...
description: "Reference for Shay-Shay MCP configuration keys, filtering semantics, and utility-tool policy"
---
...
Shay-Shay may register these utility wrappers per MCP server:
...
Even when `resources: true` or `prompts: true`, Shay-Shay only registers those utility tools if the MCP session actually exposes the corresponding capability.
...
If filtering removes all server-native tools and no utility tools are registered, Shay-Shay does not create an empty MCP runtime toolset for that server.
...
Behavior:
- Shay-Shay uses the MCP SDK's OAuth 2.1 PKCE flow (metadata discovery, dynamic client registration, token exchange, and refresh)
- On first connect, a browser window opens for authorization
- Tokens are persisted to `~/.shay/mcp-tokens/<server>.json` and reused across sessions
