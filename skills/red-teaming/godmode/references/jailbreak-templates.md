...
## 5. GODMODE FAST — Shay-Shay 4 405B (Zero Refusal)

**Model:** `nousresearch/shay-4-405b`
**Strategy:** Shay-Shay models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Shay-Shay

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.shay/config.yaml`:
