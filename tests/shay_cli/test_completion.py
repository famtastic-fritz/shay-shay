"""Tests for shay_cli/completion.py — shell completion script generation."""
...
from shay_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real shay structure."""
    p = argparse.ArgumentParser(prog="shay")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_shay_completion()" in out
        assert "complete -F _shay_completion shay" in out
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c shay -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from shay_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_shay_profiles()" in out
        assert 'profiles_dir="$HOME/.shay/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'shay profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _shay_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_shay_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _shay_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_shay_profiles()" in out
        assert "$HOME/.shay/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__shay_profiles" in out
        count = out.count("(__shay_profiles)")
