...
This page covers all commands related to [Shay-Shay profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `shay profile` without a subcommand shows help.
...
```bash
shay profile use <name>
```

Sets `<name>` as the active profile. All subsequent `shay` commands (without `-p`) will use this profile.
...
shay profile use default
```

## `shay profile create`

```bash
shay profile create <name> [options]
```
...
shay profile create work2 --clone --clone-from work
```

## `shay profile delete`

```bash
shay profile delete <name> [options]
```
...
```bash
shay profile delete mybot
shay profile delete mybot --yes
```
...
```bash
shay profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Shay-Shay home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ shay profile show work
Profile: work
Path:    ~/.shay/profiles/work
...
```bash
shay profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Shay-Shay installation.
...
```bash
shay profile alias work
# Creates/updates ~/.local/bin/work

shay profile alias work --name mywork
# Creates ~/.local/bin/mywork

shay profile alias work --remove
...
```bash
shay profile rename <old-name> <new-name>
```
...
```bash
shay profile rename mybot assistant
# ~/.shay/profiles/mybot → ~/.shay/profiles/assistant
...
```bash
shay profile export <name> [options]
```
...
```bash
shay profile export work
# Creates work.tar.gz in the current directory

shay profile export work -o ./work-2026-03-29.tar.gz
```

## `shay profile import`

```bash
shay profile import <archive> [options]
```
...
```bash
shay profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

shay profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`shay profile export` / `import` are still the right commands for
...
### `shay profile install`

```bash
shay profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `shay -p telemetry`). |
...
shay profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
shay profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
shay profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
shay profile install ./telemetry/
```

### `shay profile update`

```bash
shay profile update <name> [--force-config] [--yes]
```
...
```bash
shay profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Shay-Shay version, author, env var requirements, the source URL/path, and
...
`shay profile list` also shows the distribution name and version in a
`Distribution` column, and `shay profile show <name>` / `delete <name>`
...
shay profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
shay profile install https://github.com/your-org/internal-assistant.git
```
...
`shay_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Shay-Shay version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Shay-Shay can clone from.
3. Tell recipients to run `shay profile install <your-repo-url>`.
...
```bash
shay -p <name> <command> [options]
shay --profile <name> <command> [options]
```

Global flag to run any Shay-Shay command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
shay -p work chat -q "Check the server status"
shay --profile dev gateway start
shay -p personal skills list
shay -p work config edit
```

## `shay completion`

```bash
shay completion <shell>
```
...
- `shay profile <TAB>` — subcommands (list, use, create, etc.)
- `shay profile use <TAB>` — profile names
- `shay -p <TAB>` — profile names
