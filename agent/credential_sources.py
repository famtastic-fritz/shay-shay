"""Unified removal contract for every credential source Shay-Shay reads from.

Shay-Shay seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.shay/.env
    claude_code   — ~/.claude/.credentials.json
    shay_pkce   — ~/.shay/.anthropic_oauth.json
...
unify here is **removal**:
    ...
    ``shay auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), shay_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.shay/.env  → clear it
    ...
    """
    from shay_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Shay-Shay.",
            f"  The pool entry is now suppressed — Shay-Shay will ignore "
            f"{env_var} until you run `shay auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Shay-Shay stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `shay auth add anthropic` to re-enable if needed.",
    ])


def _remove_shay_pkce(provider: str, removed) -> RemovalResult:
    """~/.shay/.anthropic_oauth.json is ours — delete it outright."""
    from shay_constants import get_shay_home

    result = RemovalResult()
    oauth_file = get_shay_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Shay-Shay Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `shay login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `shay auth add nous` to re-engage, which is the documented re-add
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Shay-Shay auth store is not enough — _seed_from_singletons() would
    ...
    via ``shay auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from shay_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `shay auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `shay auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Shay-Shay stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `shay auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="shay_pkce",
        remove_fn=_remove_shay_pkce,
        description="~/.shay/.anthropic_oauth.json",
    ))
    register(RemovalStep(
