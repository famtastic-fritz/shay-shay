...
description: "Connect Shay-Shay to external tool servers via MCP — and control exactly which MCP tools Shay-Shay loads"
---

# MCP (Model Context Protocol)

MCP lets Shay-Shay connect to external tool servers so the agent can use tools that live outside Shay-Shay itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Shay-Shay to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Shay-Shay tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Shay-Shay to see
...
2. Add an MCP server to `~/.shay/config.yaml`:
...
4. Ask Shay-Shay to use the MCP-backed capability.
...
Shay-Shay will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Shay-Shay connects to directly.
...
- you do not want Shay-Shay spawning a local subprocess for that integration

## Basic configuration reference

Shay-Shay reads MCP config from `~/.shay/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Shay-Shay skips the server entirely |
...
## How Shay-Shay registers MCP tools

Shay-Shay prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Shay-Shay sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Shay-Shay also registers utility tools around MCP resources and prompts:
...
- Shay-Shay only registers resource utilities if the MCP session actually supports resource operations
- Shay-Shay only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Shay-Shay, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Shay-Shay skips the server completely and does not even attempt a connection.
...
You can also separately disable Shay-Shay-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Shay-Shay does not create an empty runtime MCP toolset for that server.
...
Shay-Shay discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Shay-Shay when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Shay-Shay receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Shay-Shay does not blindly pass your full shell environment.
...
Then verify your config and restart Shay-Shay.
...
Because Shay-Shay now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Shay-Shay via the `sampling/createMessage` protocol. This allows an MCP server to ask Shay-Shay to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Shay-Shay as an MCP server

In addition to connecting **to** MCP servers, Shay-Shay can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Shay-Shay's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Shay-Shay
- You want a single MCP server that bridges to all of Shay-Shay's connected messaging platforms at once
- You already have a running Shay-Shay gateway with connected platforms
...
Add Shay-Shay to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Shay-Shay in a specific location:

```json
{
  "mcpServers": {
    "shay": {
      "command": "/home/user/.shay/shay-shay/venv/bin/shay",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Shay-Shay-specific channel browser:
...
The MCP server includes a live event bridge that polls Shay-Shay's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
shay mcp serve              # Normal mode
shay mcp serve --verbose    # Debug logging on stderr
```
