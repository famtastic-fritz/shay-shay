...
description: "Frequently asked questions and solutions to common issues with Shay-Shay"
---
...
### What LLM providers work with Shay-Shay?

Shay-Shay works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** — Nous Research's own inference endpoint
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `shay login anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `shay model` or by editing `~/.shay/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Shay-Shay requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Shay-Shay from inside it. The standard install command works perfectly in WSL2:

```bash
curl -fsSL https://raw.githubusercontent.com/NousResearch/shay-shay/main/scripts/install.sh | bash
```

### I run Shay-Shay in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Shay-Shay inside WSL2
...
- let Shay-Shay use the resulting MCP browser tools

This is more reliable than trying to force Shay-Shay core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Shay-Shay](../guides/use-mcp-with-shay.md#wsl2-bridge-shay-in-wsl-to-windows-chrome)
...
Yes — Shay-Shay now has a tested Termux install path for Android phones.
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Shay-Shay does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.shay/`.

### Can I use it offline / with local models?

Yes. Run `shay model`, select **Custom endpoint**, and enter your server's URL:
...
Shay-Shay persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Shay-Shay — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Shay-Shay auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `SHAY_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Shay-Shay itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Shay-Shay instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Shay-Shay programmatically:
...
#### `shay: command not found` after installation
...
**Cause:** Shay-Shay requires Python 3.11 or newer.
...
**Cause:** Shay-Shay builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Shay-Shay runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Shay-Shay auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.shay/config.yaml`:
...
**Solution:** Exit your session and use `shay model` from your terminal to add new providers:

```bash
# Exit the Shay-Shay chat session first (Ctrl+C or /quit)
...
After adding a new provider via `shay model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `shay model` (from terminal) |
| Enter/change API keys | `shay model` (from terminal) |
...
# Or set directly
shay config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.shay/.env` for conflicting entries.
:::
...
# Set a valid model
shay config set SHAY_MODEL anthropic/claude-opus-4.7

# Or specify per-session
shay chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `shay chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Shay-Shay detected the wrong context length for your model.
...
shay chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Shay-Shay may have the wrong context length for your model. Check what it detected:
...
**Cause:** Shay-Shay detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Shay-Shay never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `shay chat`
...
cat ~/.shay/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `shay gateway setup`
- Check gateway logs: `cat ~/.shay/logs/gateway.log | tail -50`
...
Configure in `~/.shay/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "shay-shay[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `shay gateway start` fails
...
tmux new -s shay 'shay gateway run'
# Reattach later: tmux attach -t shay

# Option 3: Background via nohup
nohup shay gateway run > ~/.shay/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'shay gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `shay gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
shay gateway install    # Re-snapshots your current PATH
shay gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.shay.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `shay chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `shay chat -t "terminal"`
...
cd ~/.shay/shay-shay && uv pip install -e ".[mcp]"
...
Verify your `~/.shay/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
shay config show | grep -A 12 mcp_servers

# Restart Shay-Shay or reload MCP after config changes
shay chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Shay-Shay](/docs/guides/use-mcp-with-shay)
...
:::warning
If an MCP server crashes mid-request, Shay-Shay will report a timeout. Check the server's own logs (not just Shay-Shay logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `shay profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `shay update`?

`shay update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `shay update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.shay/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Shay-Shay can route subagents to a different model automatically. Set this in `~/.shay/config.yaml`:
...
Now when you tell Shay-Shay "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Shay-Shay do this?

**Current limitation:** Shay-Shay profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Shay-Shay reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `shay skills config` settings don't seem to take effect.

**Solution:** Use `shay skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`shay gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Shay-Shay creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Shay-Shay to another machine
...
1. Install Shay-Shay on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/NousResearch/shay-shay/main/scripts/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   shay backup
   ```
   This creates a zip of your entire `~/.shay/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/shay-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/shay-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   shay import ~/shay-backup-<timestamp>.zip
   ```

4. On the new machine, run `shay setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
shay profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
shay profile import ./work-backup.tar.gz work
```
...
### `shay backup` vs `shay profile export`

| Feature | `shay backup` | `shay profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.shay` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='shay-shay' ~/.shay/ newmachine:~/.shay/
```

:::tip
`shay backup` produces a consistent snapshot even while Shay-Shay is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Shay-Shay installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Shay-Shay-specific issue — it's a shell config permissions problem.
...
shay config show | head -20

# Re-run model selection
shay model

# Or test with a known-good model
shay chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/NousResearch/shay-shay/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Shay-Shay version (`shay --version`), and the full error message
