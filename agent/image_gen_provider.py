...
as ``kind: backend``) or ``~/.shay/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``shay tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``shay tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``shay tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$SHAY_HOME/cache/images/``, creating parents as needed."""
    from shay_constants import get_shay_home

    path = get_shay_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$SHAY_HOME/cache/images/``.
