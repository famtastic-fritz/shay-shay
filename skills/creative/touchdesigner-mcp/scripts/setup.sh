...
SHAY_HOME_DIR="${SHAY_HOME:-$HOME/.shay}"
SHAY_CFG="${SHAY_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Shay-Shay config has twozero_td MCP entry ──
if [[ ! -f "$SHAY_CFG" ]]; then
    echo -e " ${FAIL} Shay-Shay config not found at ${SHAY_CFG}"
    manual_steps+=("Create ${SHAY_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$SHAY_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Shay-Shay config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Shay-Shay config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${SHAY_CFG} manually"); }
    manual_steps+=("Restart Shay-Shay session to pick up config change")
fi
