"""
Dump command for shay CLI.

Outputs a compact, plain-text summary of the user's Shay-Shay setup
...
from shay_cli.config import get_shay_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``shay dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from shay_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(shay_home: Path) -> int:
...
def _cron_summary(shay_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = shay_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"shay_home:      {display_shay_home()}")
    ...
    toolsets = config.get("toolsets", ["shay-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(shay_home)}")
    lines.append(f"  skills:             {_count_skills(shay_home)}")
