"""Tests for cli.ShayCLI._confirm_destructive_slash.

Drives the helper directly via __get__ on a SimpleNamespace stand-in so we
don't have to construct a full ShayCLI (which requires extensive setup).
"""
...
def _make_self(prompt_response):
    ...
    self_._normalize_slash_confirm_choice = _bound(
        ShayCLI._normalize_slash_confirm_choice, self_,
    )
    return self_
...
def test_gate_off_returns_once_without_prompting():
    ...
    with patch(
        ...
        result = _bound(ShayCLI._confirm_destructive_slash, self_)(
            "clear", "detail",
        )
...
def test_slash_confirm_modal_number_selection_submits_without_raw_input():
    ...
    _bound(ShayCLI._submit_slash_confirm_response, self_)("always")
...
def test_slash_confirm_display_fragments_include_choice_mapping():
    ...
    fragments = _bound(ShayCLI._get_slash_confirm_display_fragments, self_)()
