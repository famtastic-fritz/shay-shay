...
Shay-Shay is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Shay-Shay checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.shay/config.yaml`:
...
1. **CLI flag**: Start a session with `shay --yolo` or `shay chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `SHAY_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `SHAY_YOLO_MODE` environment variable which is checked before every command execution.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Shay-Shay refuses to run them **regardless** of:
...
Configure the timeout in `~/.shay/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.shay/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.shay/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` shay/gateway | Self-termination prevention |
...
The `SHAY_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.shay/config.yaml`:
...
:::tip
Use `shay config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Shay-Shay controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.shay/.env`:
...
For more flexible authorization, Shay-Shay includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `shay pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.shay/config.yaml`:
...
shay pairing approve telegram ABC12DEF

# Revoke a user's access
shay pairing revoke telegram 123456789

# Clear all pending codes
shay pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.shay/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Shay-Shay applies strict security hardening to every container.
...
Container resources are configurable in `~/.shay/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.shay/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `SHAY_HOME`. Skills declare these in frontmatter:
...
When loaded, Shay-Shay checks if these files exist in the active profile's `SHAY_HOME` and registers them for mounting:
...
Paths are relative to `~/.shay/`. Files are mounted to `/root/.shay/` inside the container.
...
| **terminal** (local) | Blocks explicit Shay-Shay infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Shay-Shay infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Shay-Shay integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.shay/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.shay/logs/` for unauthorized access attempts
10. **Keep updated** — run `shay update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.shay/.env`:
...
TERMINAL_SSH_USER=shay
TERMINAL_SSH_KEY=~/.ssh/shay_agent_key
```
