"""Tests for the `shay memory reset` CLI command.
...
- Profile-scoped reset (uses SHAY_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake SHAY_HOME with memory files."""
    shay_home = tmp_path / ".shay"
    memories = shay_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("SHAY_HOME", str(shay_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nShay repo is at ~/.shay/shay-shay\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `shay memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        shay_home = tmp_path / ".shay"
        (shay_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("SHAY_HOME", str(shay_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's SHAY_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        shay_home = tmp_path / ".shay"
        shay_home.mkdir(parents=True)
        # No memories dir
        monkeypatch.setenv("SHAY_HOME", str(shay_home))

        # The memories dir won't exist; get_shay_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
