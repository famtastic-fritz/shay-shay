...
title: "Git Worktrees"
description: "Run multiple Shay-Shays safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Shay-Shay is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Shay-Shay so each session has a clean, isolated working directory.

## Why Use Worktrees with Shay-Shay?

Shay-Shay treats the **current working directory** as the project root:

- CLI: the directory where you run `shay` or `shay chat`
...
git worktree add ../repo-feature feature/shay-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/shay-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Shay-Shay there:

```bash
cd ../repo-feature

# Start Shay-Shay in the worktree
shay
```
...
git worktree add ../repo-experiment-a feature/shay-a
git worktree add ../repo-experiment-b feature/shay-b
```
...
Each Shay-Shay process:

- Works on its own branch (`feature/shay-a` vs `feature/shay-b`).
...
- Shay-Shay checkpoint data under `~/.shay/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Shay-Shay experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/shay-checkpoints-docs`, `feature/shay-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Shay-Shay from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `shay -w` (Automatic Worktree Mode)

Shay-Shay has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `shay/shay-<hash>`).
...
```bash
shay -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `shay -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Shay-Shay session its own clean checkout.
