...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Shay-Shay automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Shay-Shay auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
shay auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
shay auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
shay auth add anthropic --type oauth
...
Run `shay auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `shay auth` | Interactive pool management wizard |
| `shay auth list` | Show all pools and credentials |
| `shay auth list <provider>` | Show a specific provider's pool |
| `shay auth add <provider>` | Add a credential (prompts for type and key) |
| `shay auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `shay auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `shay auth remove <provider> <index>` | Remove credential by 1-based index |
| `shay auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `shay auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `shay model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via shay model:
shay auth list
# Shows:
...
shay auth add Together.ai --api-key sk-together-second-key
```
...
Shay-Shay automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Shay-Shay PKCE OAuth | `~/.shay/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `shay auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `shay auth add`) are never auto-pruned.
...
2. **`shay_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`shay_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.shay/auth.json` under the `credential_pool` key:
