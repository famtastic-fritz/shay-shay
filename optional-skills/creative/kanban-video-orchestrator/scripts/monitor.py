#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `shay kanban list` and
...
def shay_available() -> bool:
    return shutil.which("shay") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed shay CLI."""
    try:
        out = subprocess.run(
            ["shay", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `shay kanban list`
    out = subprocess.run(
        ["shay", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def main():
    ...
    if not shay_available():
        print("ERROR: 'shay' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
