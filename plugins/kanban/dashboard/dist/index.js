/**
 * Shay-Shay Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.shay/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__SHAY_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__SHAY_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "shay-kanban-dot-triage",
    todo: "shay-kanban-dot-todo",
    ready: "shay-kanban-dot-ready",
    running: "shay-kanban-dot-running",
    blocked: "shay-kanban-dot-blocked",
    done: "shay-kanban-dot-done",
    archived: "shay-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-shay-task";
  ...
  const DOCS_URL = "https://shay-shay.nousresearch.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://shay-shay.nousresearch.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "shay.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="shay-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "shay-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "shay-kanban-md",
  ...
  // standard `drop` event and our `shay-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("shay-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (col !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("shay-kanban-column--drop");
          if (col) col.classList.add("shay-kanban-column--drop");
          lastTarget = col;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("shay-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          lastTarget.dispatchEvent(new CustomEvent("shay-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__SHAY_SESSION_TOKEN__ || "";
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "shay-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "shay-kanban-attention",
        "shay-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "shay-kanban-attention-bar" },
        h("span", { className: "shay-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "shay-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "shay-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "shay-kanban-attention-row",
                  "shay-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "shay-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "shay-kanban-attention-row-id" }, task.id),
                h("span", { className: "shay-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "shay-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "shay-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "shay-kanban-diag-action-btn",
      action.suggested ? "shay-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " shay-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".shay-kanban-drawer-comment-row input, .shay-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "shay-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("shay-kanban-diag", sevClass) },
      h("div", { className: "shay-kanban-diag-header" },
        h("span", { className: "shay-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "shay-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "shay-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "shay-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "shay-kanban-diag-data-row" },
                  h("span", { className: "shay-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "shay-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "shay-kanban-diag-data-row" },
                h("span", { className: "shay-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "shay-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "shay-kanban-diag-reassign-row" },
            h("span", { className: "shay-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "shay-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "shay-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "shay-kanban-diag-msg",
              msg.ok ? "shay-kanban-diag-msg--ok" : "shay-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "shay-kanban-section" },
      h("div", { className: "shay-kanban-section-head-row" },
        h("span", { className: "shay-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "shay-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "shay-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "shay-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "shay-kanban-docs-link",
      title: "Open Shay-Shay Kanban docs in a new tab",
      "aria-label": "Shay-Shay Kanban documentation",
    }, "?");
  }
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "shay-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "shay-kanban-boardswitcher" },
      h("div", { className: "shay-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "shay-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "shay-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "shay-kanban-dialog-title" },
        ...
        h("div", { className: "shay-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Shay-Shay profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "shay-kanban-bulk" },
      h("span", { className: "shay-kanban-bulk-count" },
      ...
      h("div", { className: "shay-kanban-bulk-priority",
      ...
      h("div", { className: "shay-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Shay-Shay profile. Pick a profile (or unassign) and click Apply." },
        h(Select, {
          value: assignee,
      ...
      h("label", { className: "shay-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".shay-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "shay-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("shay-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("shay-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "shay-kanban-column",
        dragOver ? "shay-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "shay-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "shay-kanban-col-check",
        ...
        h("span", { className: cn("shay-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "shay-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "shay-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "shay-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "shay-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "shay-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "shay-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "shay-kanban-lane" },
                  h("div", { className: "shay-kanban-lane-head" },
                    h("span", { className: "shay-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "shay-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "shay-kanban-card--stale-red";
    if (age >= tier.amber) return "shay-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".shay-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "shay-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "shay-kanban-card",
        props.selected ? "shay-kanban-card--selected" : "",
        props.failed ? "shay-kanban-card--failed" : "",
        props.draggingSource ? "shay-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "shay-kanban-card-content" },
          h("div", { className: "shay-kanban-card-row" },
            h("label", {
              className: "shay-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "shay-kanban-card-check",
            ...
            h("span", { className: "shay-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or shay kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "shay-kanban-warning-badge",
                    "shay-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
              ...
              : null,
            t.priority > 0
              ? h(Badge, { className: "shay-kanban-priority",
                           title: `Priority ${t.priority}. Higher-priority tasks are claimed first by the dispatcher.` }, `P${t.priority}`)
              : null,
            t.tenant
              ? h(Badge, { variant: "outline", className: "shay-kanban-tag",
                           title: `Tenant: ${t.tenant}. Free-form tag for grouping tasks (customer, project, team).` }, t.tenant)
              : null,
            progress
              ? h("span", {
                  className: cn(
                    "shay-kanban-progress",
                    progress.done === progress.total ? "shay-kanban-progress--full" : "",
                  ),
                  title: `${progress.done} of ${progress.total} child tasks done`,
                }, `${progress.done}/${progress.total}`)
              : null,
          ),
          h("div", { className: "shay-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "shay-kanban-card-row shay-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "shay-kanban-assignee",
                            title: `Assigned to Shay-Shay profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "shay-kanban-unassigned",
            ...
            t.comment_count > 0
              ? h("span", { className: "shay-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "shay-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "shay-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Shay-Shay profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Shay-Shay profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "shay-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "shay-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "shay-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "shay-kanban-drawer-close",
        ...
        data ? h("div", { className: "shay-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "shay-kanban-drawer-body" },
      h("div", { className: "shay-kanban-drawer-title" },
        h("span", { className: cn("shay-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "shay-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "shay-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "shay-kanban-section" },
        h("div", { className: "shay-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "shay-kanban-section" },
        h("div", { className: "shay-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "shay-kanban-comment" },
                h("div", { className: "shay-kanban-comment-head" },
                  h("span", { className: "shay-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "shay-kanban-comment-ago" },
      ...
      h("div", { className: "shay-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "shay-kanban-event",
              isDiag ? "shay-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "shay-kanban-event-header" },
                  h("span", { className: "shay-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "shay-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "shay-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "shay-kanban-event-header-plain" },
                  h("span", { className: "shay-kanban-event-kind" }, e.kind),
                  h("span", { className: "shay-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "shay-kanban-event-phantom-row" },
                  h("span", { className: "shay-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "shay-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "shay-kanban-event-payload" },
    ...
    return h("div", { className: "shay-kanban-section" },
      h("div", { className: "shay-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              ...
              className: "shay-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `shay-kanban-run--${r.outcome || r.status || "ended"}`
          : "shay-kanban-run--active";
        return h("div", { key: r.id, className: cn("shay-kanban-run", outcomeClass) },
          h("div", { className: "shay-kanban-run-head" },
            h("span", { className: "shay-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "shay-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "shay-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "shay-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "shay-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "shay-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "shay-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "shay-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "shay-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "shay-kanban-pre shay-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "shay-kanban-meta-row" },
      h("span", { className: "shay-kanban-meta-label" }, props.label),
      h("span", { className: "shay-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "shay-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "shay-kanban-meta-row" },
        h("span", { className: "shay-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "shay-kanban-meta-value shay-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "shay-kanban-meta-row" },
        h("span", { className: "shay-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "shay-kanban-meta-value shay-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "shay-kanban-section" },
      h("div", { className: "shay-kanban-section-head-row" },
        h("span", { className: "shay-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "shay-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "shay-kanban-section" },
      h("div", { className: "shay-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "shay-kanban-deps-row" },
        h("span", { className: "shay-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "shay-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "shay-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "shay-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "shay-kanban-dep-chip-x",
      ...
      h("div", { className: "shay-kanban-deps-row" },
        h("span", { className: "shay-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "shay-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "shay-kanban-actions" },
        specifyButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "shay-kanban-section" },
      ...
      h("div", { className: "shay-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "shay-kanban-home-sub shay-kanban-home-sub--on"
              : "shay-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__SHAY_PLUGINS__ && typeof window.__SHAY_PLUGINS__.register === "function") {
    window.__SHAY_PLUGINS__.register("kanban", KanbanPage);
  }
})();
