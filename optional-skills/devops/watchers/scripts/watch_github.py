...
Usage (via cron with --no-agent):
    ...
    shay cron create shay-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$SHAY_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name shay-issues --repo NousResearch/shay-shay --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.shay/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Shay-Shay-Watcher/1.0",
    }
