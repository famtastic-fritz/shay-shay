...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Shay-Shay's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Shay-Shay instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
shay memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$SHAY_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `shay` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Shay-Shay syncs them back at session init.
...
The `shay honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `shay memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
shay honcho status          # Connection status, config, and key settings
shay honcho setup           # Redirects to `shay memory setup`
shay honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
shay honcho peer            # Show or update peer names + dialectic reasoning level
shay honcho mode            # Show or set recall mode (hybrid/context/tools)
shay honcho tokens          # Show or set token budget for context and dialectic
shay honcho identity        # Seed or show the AI peer's Honcho identity
shay honcho sync            # Sync Honcho config to all existing profiles
shay honcho peers           # Show peer identities across all profiles
shay honcho sessions        # List known Honcho session mappings
shay honcho map             # Map current directory to a Honcho session name
shay honcho enable          # Enable Honcho for the active profile
shay honcho disable         # Disable Honcho for the active profile
shay honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `shay honcho`

If you previously used the standalone `shay honcho setup`:
...
No re-login or re-setup needed. Run `shay memory setup` and select "honcho" — the wizard detects your existing config.
