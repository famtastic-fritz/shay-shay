...
class TestManifest:
    ...
    def test_manifest_fields(self):
        ...
        # Required env vars are the user-facing SHAY_ prefixed keys.
        assert "SHAY_LANGFUSE_PUBLIC_KEY" in data["requires_env"]
        assert "SHAY_LANGFUSE_SECRET_KEY" in data["requires_env"]
...
class TestDiscovery:
    def test_plugin_is_discovered_as_standalone_opt_in(self, tmp_path, monkeypatch):
        ...
        # Isolated SHAY_HOME so we don't read the developer's config.yaml.
        home = tmp_path / ".shay"
        home.mkdir()
...
class TestRuntimeGate:
    ...
    def test_get_langfuse_returns_none_without_credentials(self, monkeypatch):
        for k in (
            "SHAY_LANGFUSE_PUBLIC_KEY", "SHAY_LANGFUSE_SECRET_KEY",
    ...
    def test_get_langfuse_caches_failure_no_config_load(self, monkeypatch):
        ...
        def tracking_get(key, default=None):
            if key.startswith(("SHAY_LANGFUSE_", "LANGFUSE_")):
                called["n"] += 1
    ...
    def test_get_langfuse_does_not_import_shay_config(self, monkeypatch):
        ...
        # Drop any cached import of shay_cli.config.
        sys.modules.pop("shay_cli.config", None)

        langfuse_plugin = self._fresh_plugin()
        for _ in range(20):
            langfuse_plugin._get_langfuse()

        assert "shay_cli.config" not in sys.modules, (
            "langfuse plugin imported shay_cli.config — regression toward "
