#!/usr/bin/env python3
"""
Shay-Shay CLI - Main entry point.

Usage:
    shay                     # Interactive chat (default)
    shay chat                # Interactive chat
    shay gateway             # Run gateway in foreground
    shay gateway start       # Start gateway as service
    shay gateway stop        # Stop gateway service
    shay gateway status      # Show gateway status
    shay gateway install     # Install gateway service
    shay gateway uninstall   # Uninstall gateway service
    shay setup               # Interactive setup wizard
    shay logout              # Clear stored authentication
    shay status              # Show status of all components
    shay cron                # Manage cron jobs
    shay cron list           # List cron jobs
    shay cron status         # Check if cron scheduler is running
    shay doctor              # Check configuration and dependencies
    shay honcho setup                    # Configure Honcho AI memory integration
    shay honcho status                   # Show Honcho config and connection status
    shay honcho sessions                 # List directory → session name mappings
    shay honcho map <name>               # Map current directory to a session name
    shay honcho peer                     # Show peer names and dialectic settings
    shay honcho peer --user NAME         # Set user peer name
    shay honcho peer --ai NAME           # Set AI peer name
    shay honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    shay honcho mode                     # Show current memory mode
    shay honcho mode [hybrid|honcho|local]  # Set memory mode
    shay honcho tokens                   # Show token budget settings
    shay honcho tokens --context N       # Set session.context() token cap
    shay honcho tokens --dialectic N     # Set dialectic result char cap
    shay honcho identity                 # Show AI peer identity representation
    shay honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    shay honcho migrate                  # Step-by-step migration guide: OpenClaw native → Shay-Shay + Honcho
    shay version             Show version
    shay update              Update to latest version
    shay uninstall           Uninstall Shay-Shay
    shay acp                 Run as an ACP server for editor integration
    shay sessions browse     Interactive session picker with search

    shay claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: shay_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``shay_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``shay update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``shay_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, shay crashes on import and the user can't run
# ``shay update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import shay_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to SHAY_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (shay tools, shay setup, shay model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'shay {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any shay module import.
#
# Many modules cache SHAY_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("SHAY_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.shay/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set SHAY_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors shay_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If SHAY_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.shay/profiles/coder or
    # /opt/data/profiles/coder).  If SHAY_HOME points to the shay root
    # instead (e.g. systemd hardcodes SHAY_HOME=/root/.shay), we must
    # still read active_profile — the user may have switched profiles via
    # `shay profile use` and the gateway should honour that choice.
    # See issue #22502.
    shay_home_env = os.environ.get("SHAY_HOME", "")
    if profile_name is None and shay_home_env:
        if Path(shay_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the shay root
    if profile_name is None:
        try:
            from shay_constants import get_default_shay_root

            active_path = get_default_shay_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set SHAY_HOME
    if profile_name is not None:
        try:
            from shay_cli.profiles import resolve_profile_env

            shay_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent shay from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["SHAY_HOME"] = shay_home
...
# Load .env from ~/.shay/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from shay_cli.config import get_shay_home
from shay_cli.env_loader import load_shay_dotenv
...
try:
    if "SHAY_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_shay_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["SHAY_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `shay` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from shay_logging import setup_logging as _setup_logging
...
try:
    from shay_cli.config import load_config as _load_config_early
    from shay_constants import apply_ipv4_preference as _apply_ipv4
...
from shay_cli import __version__, __release_date__
from shay_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from shay_cli.config import get_env_path, get_shay_home, load_config
    from shay_cli.auth import get_auth_status

    # Determine whether Shay-Shay itself has been explicitly configured (model
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_shay_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_shay_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Shay-Shay has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Shay-Shay to use their tokens.
    if _has_shay_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, shay_bin
        cli_args: the original CLI arguments (everything after 'shay')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    shay_bin = container_info["shay_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo shay {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo shay {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  shay --tui --resume {target}")
    if title:
        print(f'  shay --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @shay/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@shay" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``SHAY_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("SHAY_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    shay_home = os.environ.get("SHAY_HOME") or str(Path.home() / ".shay")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "SHAY_HOME": shay_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(shay_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (SHAY_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("SHAY_NODE")
    ...
    ext_dir = os.environ.get("SHAY_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with SHAY_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset SHAY_TUI_DIR (e.g. `unset SHAY_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("SHAY_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from shay_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="shay-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["SHAY_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["SHAY_PYTHON_SRC_ROOT"] = os.environ.get(
        "SHAY_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("SHAY_PYTHON", sys.executable)
    env.setdefault("SHAY_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["SHAY_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["SHAY_MODEL"] = model
        env["SHAY_INFERENCE_MODEL"] = model
    if provider:
        env["SHAY_TUI_PROVIDER"] = provider
        env["SHAY_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["SHAY_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["SHAY_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["SHAY_TUI_SKILLS"] = value
    if query:
        env["SHAY_TUI_QUERY"] = query
    if image:
        env["SHAY_TUI_IMAGE"] = image
    if checkpoints:
        env["SHAY_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["SHAY_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["SHAY_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["SHAY_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["SHAY_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["SHAY_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["SHAY_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``SHAY_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``shay kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``shay kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("SHAY_KANBAN_BOARD"):
        return
    try:
        from shay_cli.kanban_db import get_current_board

        os.environ["SHAY_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("SHAY_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'shay sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Shay-Shay isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  shay setup")
        print()

        from shay_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'shay setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from shay_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["SHAY_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.shay/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["SHAY_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["SHAY_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["SHAY_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from shay_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from shay_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Shay-Shay?")
        print()
    ...
    session_dir = get_shay_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: shay gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  shay gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Shay-Shay'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  shay gateway")
        ...
        print()
        print("  Or install as a service: shay gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'shay whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from shay_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``shay model``) and the setup wizard
    ...
    from shay_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("SHAY_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'shay model' for "
                "available providers, or run 'shay doctor' to diagnose config "
                "issues."
            )
    ...
    from shay_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from shay_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.shay/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.shay/.env if the active provider is not 'custom'.
    ...
    """
    from shay_cli.config import get_env_value, save_env_value, load_config
...
#
# Shay-Shay uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `shay model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `shay model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Shay-Shay only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``shay model`` flow, then route aux tasks to them here.
    """
    from shay_cli.config import load_config
    from shay_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from shay_cli.auth import _prompt_model_selection
    from shay_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from shay_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from shay_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from shay_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from shay_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from shay_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from shay_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `shay auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from shay_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from shay_cli.auth import _save_model_choice, deactivate_provider
    from shay_cli.config import get_env_value, load_config, save_config
    ...
    from shay_cli.models import probe_api_models
    ...
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        ...
        print(
            ...
            f"Shay-Shay will still save it."
        )
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `shay model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from shay_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from shay_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from shay_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Shay-Shay will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from shay_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from shay_cli.config import save_env_value, load_config, save_config
    from shay_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from shay_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from shay_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Shay-Shay turns to `copilot --acp`.")
    print("  Shay-Shay currently starts its own ACP subprocess for each request.")
    print("  Shay-Shay uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set SHAY_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``shay setup`` / ``shay model``.
    ...
    recover from a malformed paste without editing ``~/.shay/.env`` by hand.
    ...
    from shay_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from shay_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `shay setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so shay knows where to find it
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Shay-Shay typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Shay-Shay, enable billing on your "
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from shay_constants import display_shay_home as _dhh_fn

            print(
                f"    Shay-Shay will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from shay_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'shay model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Shay-Shay CLI with a provider."""
    from shay_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from shay_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from shay_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from shay_cli.status import show_status
...
def cmd_webhook(args):
    ...
    from shay_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``shay slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: shay slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `shay slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from shay_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from shay_cli.kanban import kanban_command
...
def cmd_dump(args):
    ...
    from shay_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from shay_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from shay_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Shay-Shay home directory to a zip file."""
    if getattr(args, "quick", False):
        from shay_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from shay_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Shay-Shay backup from a zip file."""
    from shay_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Shay-Shay v{__version__} ({__release_date__})")
    ...
    try:
        from shay_cli.banner import check_for_updates
        from shay_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from shay_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``shay update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``shay_cli/web_dist/`` (per vite.config.ts
    outDir: "../shay_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "shay_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``shay update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``shay web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (shay web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``shay dashboard`` processes other than ourselves.

    ``shay dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``shay update`` replaces files on disk, the running
    ...
    patterns = [
        "shay dashboard",
        "shay_cli.main dashboard",
        "shay_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "shay.*dashboard"`) keeps us consistent
            # with `shay_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `shay update`.
    ...
    print("  Preview now:  shay curator run --dry-run")
    print("  Pause it:     shay curator pause")
    print(
        "  Docs:         https://shay-shay.nousresearch.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``shay update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``shay update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: shay curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``shay dashboard`` processes.

    Called at the end of ``shay update`` (default ``reason``) and also
    from ``shay dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    shay dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Shay-Shay by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NousResearch/shay-shay/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="shay-update-")
        zip_path = os.path.join(tmp_dir, f"shay-shay-{branch}.zip")
        ...
        # GitHub ZIPs extract to shay-shay-<branch>/
        extracted = os.path.join(tmp_dir, f"shay-shay-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "shay-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Shay-Shay behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes shay completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Shay-Shay couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Shay-Shay couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Shay-Shay behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `shay update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NousResearch/shay-shay.git",
    "git@github.com:NousResearch/shay-shay.git",
    "https://github.com/NousResearch/shay-shay",
    "git@github.com:NousResearch/shay-shay",
}
OFFICIAL_REPO_URL = "https://github.com/NousResearch/shay-shay.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_shay_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_shay_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Shay-Shay repository.")
        print("  This means you may miss updates from NousResearch/shay-shay.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NousResearch/shay-shay.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NousResearch/shay-shay.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``shay update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from shay_constants import get_default_shay_root

    default_home = get_default_shay_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``shay update`` is still progressing even if pip/uv itself is silent.
    """
...
def _shay_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "shay.exe",
        scripts_dir / "shay-gateway.exe",
    ]


def _quarantine_running_shay_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``shay.exe``.
    ...
    when ``shay update`` runs, ``shay.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``shay.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next shay invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _shay_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_shay_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``shay.exe.old.*`` left by prior updates.

    Called early on every shay invocation. The .old files are unlocked once
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``shay.exe`` / ``shay-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_shay_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_shay_exe(scripts_dir)
        try:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``shay update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.shay/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``shay update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``shay update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.shay/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``shay update --gateway``) the update is already
    ...
    try:
        ...
        # shay_cli.config.get_shay_home to simulate setup failure.
        from shay_cli.config import get_shay_home as _get_shay_home

        logs_dir = _get_shay_home() / "logs"
...
def _cmd_update_check():
    """Implement ``shay update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``shay update`` without requiring a reinstall.  The
    ...
    ``shay`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/shay.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v shay'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/shay, code at /usr/local/lib/shay-shay).
    fhs_link = Path("/usr/local/bin/shay")
    ...
    path_comment = (
        "# Shay-Shay — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of SHAY_HOME before running the update.
    ...
    SHAY_HOME directories.  The ``--backup`` flag on ``shay update``
    ...
    try:
        from shay_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_shay_home so the user sees ~/.shay/...
    try:
        from shay_constants import get_shay_home, display_shay_home

        home = get_shay_home()
        try:
            display_path = f"{display_shay_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  shay import {out_path}")
...
def cmd_update(args):
    """Update Shay-Shay to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from shay_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Shay-Shay")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print(
                "  curl -fsSL https://raw.githubusercontent.com/NousResearch/shay-shay/main/scripts/install.sh | bash"
            )
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_shay_home added to shay_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload shay_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_shay_home() added since the last release.
        try:
            import importlib
            import shay_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit SHAY_HOME so
        # it is not affected by sync_skills()'s module-level SHAY_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's SHAY_HOME env var points at the default or a named profile.
        try:
            from shay_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: shay config migrate")
            else:
                print()
                print("Skipped. Run 'shay config migrate' later to configure.")
        else:
        ...
        # check `shay curator status`. Self-stamps after printing so it
        ...
        # Tying the refresh to ``shay update`` gives users a predictable
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                from shay_cli.tools_config import install_cua_driver
        ...
        # When running as ``shay update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_shay_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all shay-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. shay-gateway.service or shay-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: shay gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: shay -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Shay-Shay gateway unit files are still installed.
        # When both shay.service (from a pre-rename install) and the
        # current shay-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `shay update` surfaces the issue until the user migrates.
        try:
            from shay_cli.gateway import (
                has_legacy_shay_units,
                _find_legacy_shay_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_shay_units():
                print()
                print("⚠ Legacy Shay-Shay gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_shay_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (shay.service) fight the current")
                print("  shay-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    shay gateway migrate-legacy")
                print()
        ...
        print("  shay model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``shay -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `shay profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_shay_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → shay -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.shay)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  shay profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     shay -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `shay profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from shay_cli.profiles import profile_exists
        ...
        from shay_cli.profiles import rename_profile
        ...
        from shay_cli.profiles import export_profile
        ...
        from shay_cli.profiles import import_profile
        ...
        import tempfile
        from shay_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="shay_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  shay -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      shay -p {plan.manifest.name} chat")
        ...
        from shay_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `shay profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"shay -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from shay_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("shay_requires"):
            print(f"Requires:     Shay-Shay {data['shay_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from shay_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.shay_requires:
        print(f"  Requires: Shay-Shay {mf.shay_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``shay dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``shay dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No shay dashboard processes running.")
        return 0

    print(f"{len(pids)} shay dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No shay dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `shay update`.
    ...
    if "SHAY_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["SHAY_WEB_DIST"])
            if "SHAY_WEB_DIST" in os.environ
            else PROJECT_ROOT / "shay_cli" / "web_dist"
        )
    ...
    from shay_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("SHAY_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from shay_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Shay-Shay log files."""
    from shay_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from shay_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``shay -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``shay_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``shay -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``shay`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for shay CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from shay_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``shay.exe.old.*`` quarantine files left by previous
    # ``shay update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_shay_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from shay_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: shay-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from shay_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://shay-shay.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `shay model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    control_plane_export = control_plane_subparsers.add_parser("export", help="Write the universal intelligence-route truth surface for other runtimes")
    control_plane_export.add_argument("--path", help="Optional output path override")
    control_plane_show = control_plane_subparsers.add_parser("show-universal", help="Print the universal intelligence-route truth surface as JSON")
    control_plane_show.add_argument("--path", help="Optional input path override")
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy shay.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Shay-Shay gateway unit files "
            "(e.g. shay.service) left over from older installs. Profile "
            "units (shay-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Shay-Shay with an interactive wizard. "
        "Run a specific section: shay setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'shay setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Shay-Shay.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $SHAY_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Shay-Shay")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Shay-Shay CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: shay-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Shay-Shay with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set SHAY_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Shay-Shay components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.shay/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.shay/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from shay_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.shay/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.shay/shell-hooks-allowlist.json."
        ),
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Shay-Shay setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Shay-Shay. Use 'shay debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    shay debug share              Upload debug report and print URL
    shay debug share --lines 500  Include more log lines
    shay debug share --expire 30  Keep paste for 30 days
    shay debug share --local      Print report locally (no upload)
    shay debug share --no-redact  Disable upload-time secret redaction
    shay debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'shay debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Shay-Shay home directory to a zip file",
        description="Create a zip archive of your entire Shay-Shay configuration, "
        "skills, sessions, and data (excludes the shay-shay codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/shay-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.shay/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo shay uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from shay_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Shay-Shay backup from a zip file",
        description="Extract a previously created Shay-Shay backup into your "
        "Shay-Shay home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Shay-Shay configuration",
    )
    ...
    def cmd_pairing(args):
        from shay_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.shay/skills/.bundled_manifest) "
            "so future 'shay update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from shay_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from shay_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/shay-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `shay plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from shay_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``shay --help``, ``shay version``, ``shay logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from shay_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from shay_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_shay_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_shay_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_shay_home()}/memories/\n")
        else:
            from shay_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'shay tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # shay tools disable <name...> [--platform cli]
    ...
    # shay tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from shay_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from shay_cli.tools_config import tools_command
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use `shay computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `shay tools` runs when you first\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: shay computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: shay computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Shay-Shay as an MCP server",
        description=(
            "Manage MCP server connections and run Shay-Shay as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'shay mcp add' to connect to a new server, or\n"
            "'shay mcp serve' to expose Shay-Shay conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Shay-Shay as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `shay mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from shay_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_shay_home() / "sessions"
            ...
            # Launch shay --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from shay_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Shay-Shay",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Shay-Shay",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.shay/ (by default a "
        "single restore-point archive is written to ~/.shay/backups/ "
        "before apply; restorable with 'shay import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from shay_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Shay-Shay to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'shay config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Shay-Shay",
        description="Remove Shay-Shay from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Shay-Shay as an ACP (Agent Client Protocol) server",
        description="Start Shay-Shay in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Shay-Shay as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Shay-Shay instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `shay update` skill sync)",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Shay-Shay profile distribution. SOURCE can be a git URL "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Shay-Shay web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `shay --tui` via PTY/WebSocket). "
            "Alternatively set SHAY_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `shay dashboard` cmdlines and SIGTERM them directly — the same
    # path `shay update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running shay dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running shay dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Shay-Shay log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    shay logs                    Show last 50 lines of agent.log
    shay logs -f                 Follow agent.log in real time
    shay logs errors             Show last 50 lines of errors.log
    shay logs gateway -n 100     Show last 100 lines of gateway.log
    shay logs --level WARNING    Only show WARNING and above
    shay logs --session abc123   Filter by session ID
    shay logs --component tools  Only show tool-related lines
    shay logs --since 1h         Lines from the last hour
    shay logs --since 30m -f     Follow, starting from 30 min ago
    shay logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``shay -c Pokemon Agent Dev`` → ``shay -c 'Pokemon Agent Dev'``
    ...
    from shay_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'shay -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (shay hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from shay_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from shay_cli.oneshot import run_oneshot
