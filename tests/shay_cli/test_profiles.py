"""Comprehensive tests for shay_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and SHAY_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.shay/profiles)
    * SHAY_HOME  -> tmp_path/.shay  (so get_shay_home() agrees)
    * Creates the bare-minimum ~/.shay directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["shay", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Shay-Shay install itself or with
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.shay must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".shay"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes shay-shay, .worktrees,
        ...
        (default_home / "shay-shay" / ".git").mkdir(parents=True)
        (default_home / "shay-shay" / "venv" / "bin").mkdir(parents=True)
        (default_home / "shay-shay" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "shay-shay").exists()
...
class TestNoSkillsOptOut:
    """Tests for `shay profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `shay update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("shay_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".shay" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_shay_home_returns_default(self, profile_env):
        # SHAY_HOME points to tmp_path/.shay which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".shay" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom SHAY_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("SHAY_HOME", str(custom))
        # With Docker-aware roots, a custom SHAY_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("shay")
        assert result is not None
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "shay.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["shay.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["shay.heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "shay.ssi_health": {"workspace": "shay", "enabled": True}
            }
        }))
        ...
        assert "shay.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["shay.heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["shay.heimdall"]["workspace"] == "shay"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "shay.ssi_health": {"aiPeer": "ssi_health"},
                "shay.heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["shay.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["shay.heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("shay-shay", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".shay_history",
        ...
        excluded_prefixes = [
            "default/shay-shay", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.shay_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_shay_profiles_function(self):
        script = generate_bash_completion()
        assert "_shay_profiles" in script

    def test_zsh_completion_has_shay_function(self):
        script = generate_zsh_completion()
        assert "_shay" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultShayHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_shay_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".shay" / "profiles"

    def test_default_shay_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_shay_home()
        assert home == tmp_path / ".shay"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (SHAY_HOME outside ~/.shay), profiles go under SHAY_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_shay_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_shay_home() returns SHAY_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (SHAY_HOME under ~/.shay), profiles root is still ~/.shay/profiles."""
        native = tmp_path / ".shay"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under SHAY_HOME."""
        from shay_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under SHAY_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".shay" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from shay_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".shay"
