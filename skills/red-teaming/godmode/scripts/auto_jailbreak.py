...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("SHAY_HOME", os.path.expanduser("~/.shay")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("SHAY_HOME", Path.home() / ".shay")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Shay-Shay config paths
# ═══════════════════════════════════════════════════════════════════

SHAY_HOME = Path(os.getenv("SHAY_HOME", Path.home() / ".shay"))
CONFIG_PATH = SHAY_HOME / "config.yaml"
PREFILL_PATH = SHAY_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Shay-Shay models — already uncensored, just needs clean prompt
    "shay": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "shay" in model_lower or "nous" in model_lower:
        return "shay"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Shay-Shay config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.shay/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Shay-Shay for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Shay-Shay for changes to take effect.")
