...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.shay/.env` at the end.
...
4. Fill in:
   - **Name:** `Shay-Shay Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `shay-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
```powershell
# Create a policy scoped to the Shay-Shay app
New-CsApplicationAccessPolicy `
  -Identity "Shay-Shay-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Shay-Shay meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "Shay-Shay-Meeting-Pipeline-Policy" `
...
Put the three values you collected into `~/.shay/.env`:
...
Shay-Shay ships a Graph auth smoke-test. From your Shay-Shay install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.shay/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `shay gateway restart`.
