"""
Microsoft Teams platform adapter for Shay-Shay.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Shay-Shay"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        shay_action = data.get("shay_action", "")
        session_key = data.get("session_key", "")

        if not shay_action or not session_key:
        ...
        choice = choice_map.get(shay_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="shay_approve",
                    data={**btn_data_base, "shay_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="shay_approve",
                    data={**btn_data_base, "shay_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="shay_approve",
                    data={**btn_data_base, "shay_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="shay_approve",
                    data={**btn_data_base, "shay_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Shay-Shay\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.shay/.env")
