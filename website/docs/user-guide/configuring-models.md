...
Shay-Shay uses two kinds of model slots:
...
- **Right** — the curated model list for the selected provider. These are the agentic models Shay-Shay recommends for that provider, not the raw `/models` dump (which on OpenRouter includes 400+ models including TTS, image generators, and rerankers).

Type in the filter box to narrow by provider name, slug, or model ID.

Pick a model, hit **Switch**, and Shay-Shay writes it to `~/.shay/config.yaml` under the `model` section. **This applies to new sessions only** — any chat tab you already have open keeps running whatever model it started with. To hot-swap the current chat, use the `/model` slash command inside it.
...
Every auxiliary task defaults to `auto` — meaning Shay-Shay uses your main model for that job too. Override a specific task when you want a cheaper or faster model for a side-job.
...
| **Skills Hub** | `shay skills search` uses this. Usually fine at `auto`. |
...
When you save via the dashboard, Shay-Shay writes to `~/.shay/config.yaml`:
...
`provider: auto` with `model: ''` tells Shay-Shay to use the main model for that task.

## When does it take effect?

- **CLI** (`shay chat`): next `shay chat` invocation.
- **Gateway** (Telegram, Discord, Slack, etc.): next *new* session. Existing sessions keep their model. Restart the gateway (`shay gateway restart`) if you want to force all sessions to pick up the change.
...
Shay-Shay lists a provider only if it has a working credential. Check **Keys** in the sidebar — you should see one of: an API key, a successful OAuth, or a custom endpoint URL. If the provider you want isn't there, run `shay setup` to wire it up, or go to **Keys** and add the env var.
...
### I picked a model but Shay-Shay switched providers on me
...
```bash
shay config set model.aliases.fav anthropic/claude-opus-4.6
shay config set model.aliases.grok x-ai/grok-4
```
...
### `shay model` subcommand

```bash
shay model            # Interactive provider + model picker (the canonical way to switch defaults)
```

`shay model` walks you through picking a provider, authenticating (OAuth flows open a browser; API-key providers prompt for the key), and then choosing a specific model from that provider's curated catalog. The choice is written to `model.provider` and `model.model` in `~/.shay/config.yaml`.

To list providers/models without launching the picker, use the dashboard or the REST endpoints below. To inspect what the CLI will actually use right now: `shay config get model` and `shay status`.

### Direct config edit

Edit `~/.shay/config.yaml` and restart whatever reads it. See the [Configuration reference](./configuration.md) for the full schema.
...
curl -H "X-Shay-Shay-Session-Token: $TOKEN" http://localhost:PORT/api/model/options

# Read current main + auxiliary assignments
curl -H "X-Shay-Shay-Session-Token: $TOKEN" http://localhost:PORT/api/model/auxiliary

# Set the main model
curl -X POST -H "Content-Type: application/json" -H "X-Shay-Shay-Session-Token: $TOKEN" \
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__SHAY_SESSION_TOKEN__`) if you're scripting against a running dashboard.
