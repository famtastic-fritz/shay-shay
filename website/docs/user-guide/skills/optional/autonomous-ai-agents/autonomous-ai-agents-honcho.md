...
description: "Configure and use Honcho memory with Shay-Shay -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Shay-Shay -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shay skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Shay-Shay

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Shay-Shay profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/shay#running-honcho-locally-with-shay

### Verify

```bash
shay honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Shay-Shay profile's AI peer
...
Honcho models conversations as interactions between **peers**. Shay-Shay creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Shay-Shay instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Shay-Shay run |
| `global` | Single session across all directories |

Manual override: `shay honcho map my-project-name`
...
Each Shay-Shay profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block shay.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `shay.coder` host block in `honcho.json`
...
```bash
shay honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Shay-Shay when Honcho memory is active.
...
Config file: `$SHAY_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `shay honcho setup`. Ensure `memory.provider: honcho` is in `~/.shay/config.yaml`.

### Memory not persisting across sessions
Check `shay honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `shay profile create <name> --clone`. For existing profiles: `shay honcho sync`.
...
| Command | Description |
|---------|-------------|
| `shay honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `shay honcho status` | Show resolved config, connection test, peer info for active profile |
| `shay honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `shay honcho disable` | Disable Honcho for the active profile |
| `shay honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `shay honcho peers` | Show peer identities across all profiles |
| `shay honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `shay honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `shay honcho sessions` | List known directory-to-session-name mappings |
| `shay honcho map <name>` | Map current working directory to a Honcho session name |
| `shay honcho identity` | Seed AI peer identity or show both peer representations |
| `shay honcho sync` | Create host blocks for all Shay-Shay profiles that don't have one yet |
| `shay honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Shay-Shay + Honcho |
| `shay memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `shay memory status` | Show active memory provider and config |
| `shay memory off` | Disable external memory provider |
