...
- A strong shared secret to use as the `clientState` value. Generate with `openssl rand -hex 32` and put it in `~/.shay/.env` as `MSGRAPH_WEBHOOK_CLIENT_STATE`.

## Quick Start

Minimum `~/.shay/config.yaml`:
...
Or via env vars in `~/.shay/.env` (auto-merged on startup):
...
Start the gateway: `shay gateway run`. The listener exposes:
...
| Every notification 403s | `clientState` mismatch (forged, or subscription registered with a different value). Re-create the subscription with `shay teams-pipeline subscribe --client-state "$MSGRAPH_WEBHOOK_CLIENT_STATE" ...` (ships with the pipeline runtime PR). |
...
- [Microsoft Teams bot setup](/docs/user-guide/messaging/teams) — the different platform that lets users chat with Shay-Shay in Teams
