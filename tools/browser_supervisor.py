...
One ``CDPSupervisor`` runs per Shay-Shay ``task_id`` that has a reachable CDP
...
DIALOG_BRIDGE_HOST = "shay-dialog-bridge.invalid"
...
(() => {
  if (window.__shayDialogBridgeInstalled) return;
  window.__shayDialogBridgeInstalled = true;
  const ENDPOINT = "http://shay-dialog-bridge.invalid/";
