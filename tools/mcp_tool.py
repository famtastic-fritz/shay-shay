...
transport, discovers their tools, and registers them into the shay-shay
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.shay/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.shay/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            shay_home = os.path.expanduser(
                os.getenv(
                    "SHAY_HOME", os.path.join(os.path.expanduser("~"), ".shay")
                )
            )
            candidates = [
                os.path.join(shay_home, "node", "bin", resolved_command),
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Shay-Shay MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Shay-Shay gateways know how to render.
    ...
    async def run(self, config: dict):
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Shay-Shay is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `shay mcp login {server_name}` (or delete the tokens "
            f"file under ~/.shay/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Shay-Shay config file.
    ...
    ``os.environ`` (which includes ``~/.shay/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Shay-Shay' MEDIA: tag convention and out to messaging
            ...
            # Shay-Shay' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Shay-Shay's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Shay-Shay registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``shay tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Shay-Shay registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple shay
