...
Config file: `$SHAY_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `shay` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `shay-{identity}` → `shay-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Shay-Shay profile:

```json
{
  "container_tag": "shay-{identity}"
}
```

For a profile named `coder`, this resolves to `shay-coder`. The default profile resolves to `shay-default`. Without `{identity}`, all profiles share the same container.
