...
def _restore_tool_modules():
    original_shay_home = os.environ.get("SHAY_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_shay_home is None:
            os.environ.pop("SHAY_HOME", None)
        else:
            os.environ["SHAY_HOME"] = original_shay_home
        _reset_modules(("tools", "shay_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    shay_cli = types.ModuleType("shay_cli")
    shay_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["shay_cli"] = shay_cli
    shay_home = tmp_path / "shay-home"
    os.environ["SHAY_HOME"] = str(shay_home)
    sys.modules["shay_cli.config"] = types.SimpleNamespace(
        get_shay_home=lambda: shay_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="shay-shay", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": shay_home / "modal_snapshots.json",
