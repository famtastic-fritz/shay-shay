...
```bash
shay teams-pipeline token-health
shay teams-pipeline token-health --force-refresh
```
...
```bash
shay teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
shay teams-pipeline maintain-subscriptions
shay teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Shay-Shay cron (recommended if you already run the Shay-Shay gateway)

Shay-Shay ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.shay/scripts/`. First create the script:

```bash
mkdir -p ~/.shay/scripts
cat > ~/.shay/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec shay teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.shay/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
shay cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/shay-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Shay-Shay Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=shay
EnvironmentFile=/etc/shay/env
ExecStart=/usr/local/bin/shay teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/shay-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Shay-Shay Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now shay-teams-pipeline-maintain.timer
systemctl list-timers shay-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/shay teams-pipeline maintain-subscriptions >> /var/log/shay/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.shay/.env` at the top of a wrapper script that crontab calls.
...
```bash
shay teams-pipeline subscriptions   # should show expirationDateTime advanced
shay teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
shay teams-pipeline list
shay teams-pipeline list --status failed
shay teams-pipeline show <job-id>
```

### Replay a stored job

```bash
shay teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
shay teams-pipeline fetch --meeting-id <meeting-id>
shay teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `shay teams-pipeline maintain-subscriptions --dry-run`
- inspect `shay teams-pipeline list --status failed`
...
- run `shay teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `shay teams-pipeline run`
...
- [ ] `shay teams-pipeline validate` returns an OK snapshot
- [ ] `shay teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Shay-Shay cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
