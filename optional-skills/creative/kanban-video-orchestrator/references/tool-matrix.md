...
Maps each role archetype to the Shay-Shay skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public shay-shay
...
## Shay-Shay skills relevant to video production

### Visual / rendering skills (`shay-shay/skills/creative/`)
...
### Design / pre-production skills (`shay-shay/skills/creative/`)
...
### Audio / media skills (`shay-shay/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`shay-shay/skills/devops/`)
...
These are **not** Shay-Shay skills but external CLIs / APIs that profiles invoke.
...
## Built-in Shay-Shay tools for media review

These are native Shay-Shay tools — not invoked via terminal but through their own
...
key is present in `~/.shay/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Shay-Shay profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.shay/.env` → environment variable.
...
When a new Shay-Shay-public video skill ships:
