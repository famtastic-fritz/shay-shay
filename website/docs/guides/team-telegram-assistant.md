...
This tutorial walks you through setting up a Telegram bot powered by Shay-Shay that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Shay-Shay installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.shay/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Shay-Shay itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Shay-Shay Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_shay_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Shay-Shay. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Shay-Shay Gateway...
...
# View live logs
journalctl --user -u shay-gateway -f
...
shay gateway start
shay gateway stop
tail -f ~/.shay/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `shay gateway install` to update the plist.
:::
...
```bash
shay gateway stop && shay gateway start
```
...
shay pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.shay/.env`:
...
Control how much detail the bot shows when using tools. In `~/.shay/config.yaml`:
...
Customize how the bot communicates by editing `~/.shay/SOUL.md`:

For a full guide, see [Use SOUL.md with Shay-Shay](/docs/guides/use-soul-with-shay).
...
```bash
# From the CLI
shay cron list          # View all scheduled jobs
shay cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u shay-gateway` (Linux) or `~/.shay/logs/gateway.log` (macOS) |
