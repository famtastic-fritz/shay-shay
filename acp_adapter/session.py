"""ACP session manager — maps ACP sessions to Shay-Shay AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.shay/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Shay-Shay itself is running in WSL.

    Windows ACP clients can launch ``shay acp`` inside WSL while still sending
    ...
    """
    from shay_constants import is_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Shay-Shay from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["shay-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Shay-Shay."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Shay-Shay AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Shay-Shay runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.shay/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``SHAY_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_shay_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=shay_home / "state.db")
            return self._db_instance
        except Exception:
