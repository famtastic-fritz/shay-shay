...
## 5. Shay-Shay Configuration

### Config location

`$SHAY_HOME/config.yaml` (defaults to `~/.shay/config.yaml` when `SHAY_HOME` is unset)
...
Restart the Shay-Shay session for changes to take effect. The MCP connection is
