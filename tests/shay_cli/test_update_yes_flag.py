"""Tests for `shay update --yes / -y` — assume yes for interactive prompts.
...
class TestUpdateYesConfigMigration:
    ...
    @patch("shay_cli.config.migrate_config")
    @patch("shay_cli.config.check_config_version", return_value=(1, 2))
    @patch("shay_cli.config.get_missing_config_fields", return_value=[])
    @patch("shay_cli.config.get_missing_env_vars", return_value=["NEW_KEY"])
    ...
    def test_no_yes_flag_still_prompts_in_tty(
        ...
        # real ``sys`` module instead of replacing ``shay_cli.main.sys`` with
        # a MagicMock. The MagicMock approach was flaky under ``pytest-xdist``
        # — a sibling test that imported ``shay_cli.main`` first could leave
