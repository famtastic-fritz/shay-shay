...
``shay_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``shay --help``, ``shay version``,
``shay logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``shay --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from shay_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["shay", "-w", "--tui"], None),
        (["shay", "version"], "version"),
        (["shay", "--tui", "chat"], "chat"),
        (["shay", "-w", "logs"], "logs"),
        (["shay", "chat", "hello world"], "chat"),
        (["shay", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["shay", "-m", "gpt5", "chat"], "chat"),
        (["shay", "--model", "gpt5", "chat", "hi"], "chat"),
        (["shay", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["shay", "-z", "hello world"], None),
        (["shay", "-z", "hello", "chat"], "chat"),
        (["shay", "--model=gpt5", "chat"], "chat"),     # inline form
        (["shay", "--", "chat"], "chat"),               # -- terminator
        (["shay", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["shay", "some-plugin-cmd"], "some-plugin-cmd"),
        (["shay", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["shay"],                          # bare → chat
        ["shay", "--help"],                # top-level help
        ["shay", "-h"],
        ["shay", "version"],               # known built-in
        ["shay", "logs"],
        ["shay", "gateway", "run"],
        ["shay", "--tui"],
        ["shay", "-w", "--tui"],
        ["shay", "chat", "hi"],
        ["shay", "help"],                  # accepted built-in-ish
        ["shay", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["shay", "meet", "join"],          # potential google_meet plugin
        ["shay", "honcho", "status"],      # potential memory plugin
        ["shay", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``shay help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"shay_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "
