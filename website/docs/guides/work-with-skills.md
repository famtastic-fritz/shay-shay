...
description: "Find, install, use, and create skills — on-demand knowledge that teaches Shay-Shay new workflows"
---

# Working with Skills

Skills are on-demand knowledge documents that teach Shay-Shay how to handle specific tasks — from generating ASCII art to managing GitHub PRs. This guide walks you through using them day to day.
...
Every Shay-Shay installation ships with bundled skills. See what's available:
...
You can also trigger skills through natural conversation — ask Shay-Shay to use a specific skill, and it will load it via the `skill_view` tool.
...
Official optional skills ship with Shay-Shay but aren't active by default. Install them explicitly:

```bash
# Install an official optional skill
shay skills install official/research/arxiv
...
What happens:
1. The skill directory is copied to `~/.shay/skills/`
...
For how to ship skills in your own plugin, see [Build a Shay-Shay Plugin → Bundle skills](/docs/guides/build-a-shay-plugin#bundle-skills).
...
When a skill with config is first loaded, Shay-Shay prompts you for the values. They're stored in `config.yaml` under `skills.config.*`.

Manage skill config from the CLI:

```bash
# Interactive config for a specific skill
shay skills config gif-search

# View all skill config
shay config get skills.config
```
...
```bash
mkdir -p ~/.shay/skills/my-category/my-skill
```

### 2. Write SKILL.md

```markdown title="~/.shay/skills/my-category/my-skill/SKILL.md"
---
name: my-skill
...
```bash
shay chat -q "/my-skill help me with the thing"
```

The skill appears automatically — no registration needed. Drop it in `~/.shay/skills/` and it's live.

:::info
The agent can also create and update skills itself using `skill_manage`. After solving a complex problem, Shay-Shay may offer to save the approach as a skill for next time.
:::
...
**Let the agent create skills.** After a complex multi-step task, Shay-Shay will often offer to save the approach as a skill. Say yes — these agent-authored skills capture the exact workflow including pitfalls that were discovered along the way.

**Use categories.** Organize skills into subdirectories (`~/.shay/skills/devops/`, `~/.shay/skills/research/`, etc.). This keeps the list manageable and helps the agent find relevant skills faster.

**Update skills when they go stale.** If you use a skill and hit issues not covered by it, tell Shay-Shay to update the skill with what you learned. Skills that aren't maintained become liabilities.
