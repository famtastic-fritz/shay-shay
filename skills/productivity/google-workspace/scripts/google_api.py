#!/usr/bin/env python3
"""Google Workspace API CLI for Shay-Shay.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Shay-Shay-facing JSON contract and falls back to the Python client
...
from _shay_home import get_shay_home
...
def _gws_binary() -> str | None:
    override = os.getenv("SHAY_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Shay-Shay")
