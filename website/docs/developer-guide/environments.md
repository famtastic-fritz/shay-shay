...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Shay-Shay-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Shay-Shay includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NousResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Shay-Shay/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- Shay-ShayAgentBaseEnv
    Shay-ShayAgentBaseEnv <|-- TerminalTestEnv
    Shay-ShayAgentBaseEnv <|-- Shay-ShaySweEnv
    Shay-ShayAgentBaseEnv <|-- TerminalBench2EvalEnv
...
### Shay-ShayAgentBaseEnv

The shay-shay layer (`environments/shay_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls shay-shay's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `Shay-ShayAgentLoop` and scores the result
...
Your environment inherits from `Shay-ShayAgentBaseEnv` and implements five methods:
...
`Shay-ShayAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as shay-shay's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any shay-shay tool |
...
parser = get_parser("shay")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `shay`, `mistral`, `llama3_json`, `llama4_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1` (alias `deepseek_v31`), `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "shay-shay[yc-bench]"
...
```bash
python environments/shay_swe_env/shay_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"shay"`, `"qwen"`, `"mistral"`)
...
```python
from environments.shay_base_env import Shay-ShayAgentBaseEnv, Shay-ShayAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(Shay-ShayAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(Shay-ShayAgentBaseEnv):
    name = "my-env"
...
### Shay-ShayAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which shay toolsets to enable |
...
| `tool_call_parser` | `str` | `"shay"` | Parser for Phase 2 raw output |
...
env:
  ...
  tokenizer_name: "NousResearch/Shay-Shay-3-Llama-3.1-8B"
  use_wandb: true
...
- [Modal](https://modal.com) account and CLI: `pip install "shay-shay[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "shay-shay[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── shay_base_env.py          # Abstract base class (Shay-ShayAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (Shay-ShayAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── shay_parser.py        # Shay-Shay/ChatML <tool_call> format
...
├── shay_swe_env/             # SWE-bench training environment
│
