...
Connect Shay-Shay to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Shay-Shay supports two WeCom integration modes:
...
3. Shay-Shay decrypts the message, queues it for the agent
...
(Use `shay gateway start` only after `shay gateway install` has registered the systemd/launchd service.)
