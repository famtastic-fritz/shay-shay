...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - shay_home (str): The active SHAY_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.shay``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "shay").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'shay memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], shay_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'shay memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``shay_home`` is the active SHAY_HOME directory path.
