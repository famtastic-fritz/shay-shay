"""Tests for _setup_feishu() in shay_cli/gateway.py.
...
def _run_setup_feishu(
    *,
    ...
    with patch("shay_cli.gateway.save_env_value", side_effect=mock_save), \
         patch("shay_cli.gateway.get_env_value", side_effect=mock_get), \
         patch("shay_cli.gateway.prompt_yes_no", side_effect=prompt_yes_no_responses), \
         patch("shay_cli.gateway.prompt_choice", side_effect=prompt_choice_responses), \
         patch("shay_cli.gateway.prompt", side_effect=prompt_responses), \
         patch("shay_cli.gateway.print_info"), \
         patch("shay_cli.gateway.print_success"), \
         patch("shay_cli.gateway.print_warning"), \
         patch("shay_cli.gateway.print_error"), \
         patch("shay_cli.gateway.color", side_effect=lambda t, c: t), \
         patch("gateway.platforms.feishu.qr_register", return_value=qr_result):

        from shay_cli.gateway import _setup_feishu
        _setup_feishu()
