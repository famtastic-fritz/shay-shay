"""disk_cleanup — ephemeral file cleanup for Shay-Shay.
...
Rules:
  ...
  - empty dirs    → always delete (under SHAY_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly SHAY_HOME and /tmp/shay-*
Never touches: ~/.shay/logs/ or any system directory.
"""
...
try:
    ...
    def get_shay_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("SHAY_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".shay").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$SHAY_HOME/logs/``."""
    return get_shay_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$SHAY_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under SHAY_HOME or ``/tmp/shay-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    shay_home = get_shay_home()
    try:
        path.resolve().relative_to(shay_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/shay-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("shay-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside SHAY_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under SHAY_HOME (but leave SHAY_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "shay-shay", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(shay_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == shay_home:
                continue
            try:
                rel_parts = dirpath.relative_to(shay_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    shay_home = get_shay_home()
    try:
        rel = path.resolve().relative_to(shay_home)
        ...
        # Path isn't under SHAY_HOME (e.g. /tmp/shay-*) — fall through.
        pass
