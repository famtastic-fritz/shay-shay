"""Tests for the Kanban DB layer (shay_cli.kanban_db)."""
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Shay-Shay profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="shay-task", assignee="daily")
...
def test_scratch_workspace_created_under_shay_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
...
# spawned with `shay -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's SHAY_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's SHAY_HOME."""

    def _set_home(self, monkeypatch, tmp_path, shay_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(shay_home))
        monkeypatch.delenv("SHAY_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_shay(
        self, tmp_path, monkeypatch
    ):
        # Standard install: SHAY_HOME == ~/.shay, no profile active.
        default_home = tmp_path / ".shay"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.shay/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.shay/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.shay/kanban.db.
        default_home = tmp_path / ".shay"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # SHAY_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `shay -p coder`).
    ...
    def test_docker_custom_shay_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: SHAY_HOME points outside ~/.shay.
        # `get_default_shay_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".shay"`.
        custom_root = tmp_path / "opt" / "shay"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: SHAY_HOME=/opt/shay/profiles/coder;
        # `get_default_shay_root()` walks up to /opt/shay because
    ...
    def test_explicit_override_via_shay_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: SHAY_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".shay"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(profile_home))
        monkeypatch.setenv("SHAY_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(default_home))
        monkeypatch.setenv("SHAY_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # SHAY_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile SHAY_HOME and reads.
        monkeypatch.setenv("SHAY_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_shay_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # SHAY_KANBAN_DB pins the file path directly and beats both
        # SHAY_KANBAN_HOME and the `get_default_shay_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(default_home))
        monkeypatch.setenv("SHAY_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SHAY_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows SHAY_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_shay_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # SHAY_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".shay"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("SHAY_HOME", str(default_home))
        monkeypatch.setenv("SHAY_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("SHAY_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows SHAY_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(default_home))
        monkeypatch.setenv("SHAY_KANBAN_DB", "   ")
        monkeypatch.setenv("SHAY_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject SHAY_KANBAN_DB
        # and SHAY_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites SHAY_HOME.
        default_home = tmp_path / ".shay"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["SHAY_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["SHAY_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["SHAY_KANBAN_TASK"] == "t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see shay_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_connect_falls_back_to_delete_on_locking_protocol(kanban_home, caplog):
    ...
    with _patch("shay_cli.kanban_db.sqlite3.connect", side_effect=wal_blocking_connect):
        with caplog.at_level("WARNING", logger="shay_state"):
            conn = kb.connect()
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'shay kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_shay_argv()
#
# Workers spawned by the dispatcher must use a `shay` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["shay", ...]`
...
def test_resolve_shay_argv_prefers_path_shim(monkeypatch):
    """When `shay` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import shay_cli.kanban_db as kb

    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/shay")
    argv = kb._resolve_shay_argv()
    assert argv == ["/usr/local/bin/shay"]


def test_resolve_shay_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m shay_cli.main`.

    Pins the correct module name (NOT `shay` — there is no top-level
    `shay` package). Regression for #23198: the original PR shipped
    `python -m shay` which fails with `No module named shay` on every
    invocation.
    """
    import shutil
    import sys
    ...
    argv = kb._resolve_shay_argv()
    assert argv == [sys.executable, "-m", "shay_cli.main"]


def test_resolve_shay_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `shay_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m shay_cli.main --version`
    ...
    assert "Shay-Shay" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
