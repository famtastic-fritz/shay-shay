...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Shay-Shay terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Shay-Shay's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
Shay-Shay also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `shay --tui` — see the [TUI](tui.md) guide.
:::
...
shay chat --model "anthropic/claude-sonnet-4"

# With a specific provider
shay chat --provider nous        # Use Nous Portal
shay chat --provider openrouter  # Force OpenRouter
...
shay chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
shay --continue             # Resume the most recent CLI session (-c)
shay --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
shay chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
shay -w                         # Interactive mode in worktree
shay -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Shay-Shay CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Shay-Shay CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`shay -c` or `shay --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Shay-Shay to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Shay-Shay supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Shay-Shay replies |
...
```bash
shay -s shay-shay-dev,github-auth
shay chat -s github-pr-workflow -s github-auth
```

Shay-Shay loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.shay/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.shay/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Shay-Shay..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Shay-Shay recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Shay-Shay — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Shay-Shay is working, Shay-Shay prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Shay-Shay to the background — just like any terminal process. The shell prints a confirmation:

```
Shay-Shay has been suspended. Run `fg` to bring Shay-Shay back.
```
...
Resume this session with:
  shay --resume 20260225_143052_a1b2c3
...
```bash
shay --continue                          # Resume the most recent CLI session
shay -c                                  # Short form
shay -c "my project"                     # Resume a named session (latest in lineage)
shay --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
shay --resume "refactoring auth"         # Resume by title
shay -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `shay sessions rename <id> <title>` from the command line. Use `shay sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Shay-Shay's SQLite state database under `~/.shay/state.db`. The database keeps:
...
Shay-Shay immediately confirms the task and gives you back the prompt:
