"""Tests for IRC gateway configuration via `shay setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Shay-Shay install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Shay-Shay install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#shay")
            monkeypatch.setenv("IRC_NICKNAME", "shay-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `shay setup gateway` experience with IRC present."""
