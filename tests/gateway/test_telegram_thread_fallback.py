...
Supergroup forum topics route with ``message_thread_id``. Shay-Shay-created
...
async def test_send_typing_skips_api_call_for_dm_topic_reply_fallback():
    """Shay-Shay-created DM topic lanes have no working Bot API typing route.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_shay_dm_topics():
    """Shay-Shay-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in Shay-Shay-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []
