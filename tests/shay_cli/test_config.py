"""Tests for shay_cli configuration management."""
...
class TestGetShayHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("SHAY_HOME", None)
            home = get_shay_home()
            assert home == Path.home() / ".shay"

    def test_env_override(self):
        with patch.dict(os.environ, {"SHAY_HOME": "/custom/path"}):
            home = get_shay_home()
            assert home == Path("/custom/path")


class TestEnsureShayHome:
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``shay logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from shay_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"SHAY_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="shay_cli.config"):
                config = load_config()
            ...
            # stands out at shay startup before logging is configured)
            captured = capsys.readouterr()
            assert "shay config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"SHAY_HOME": str(tmp_path)}):
            ...
            assert "shay config:" in after_edit, "edited file should re-warn"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"SHAY_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SHAY_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SHAY_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"SHAY_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"SHAY_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from shay_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
