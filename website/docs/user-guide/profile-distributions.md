...
A **profile distribution** packages a complete Shay-Shay — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Shay-Shay meant sending someone:
...
```bash
shay profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `shay profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Shay-Shay in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`shay profile export` / `import`](../reference/profile-commands.md#shay-profile-export) — that's what those are for.
...
```bash
shay profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.shay/profiles/research-bot/SOUL.md
...
Create `~/.shay/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
shay_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Recipients who run `shay profile update research-bot` will pull the latest.
...
```bash
shay profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.shay/profiles/research-bot/` (or wherever the manifest's `name` resolves).
...
```bash
# GitHub shorthand
shay profile install github.com/you/research-bot

# Full HTTPS
shay profile install https://github.com/you/research-bot.git

# SSH
shay profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
shay profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
shay profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
shay profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
shay profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
shay profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Shay-Shay distribution.
...
```bash
cp ~/.shay/profiles/research-bot/.env.EXAMPLE ~/.shay/profiles/research-bot/.env
...
```bash
shay profile info research-bot
```
...
`shay profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
shay profile update research-bot
```
...
```bash
shay profile delete research-bot
```
...
Path:    ~/.shay/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `shay profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
shay profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `shay profile update pr-reviewer` and everyone's on the new version within minutes.
...
```bash
# You
cd ~/.shay/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/shay-polymarket-trader.git
git push -u origin main --tags

# Anyone
shay profile install github.com/you/shay-polymarket-trader --alias
```
...
You built Shay-Shay-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
shay profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
shay profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `shay profile info <name>` and hold off on updates until you're ready.
:::
...
shay profile info research-bot | grep Version
...
```yaml
# ~/.shay/profiles/research-bot/local/my-overrides.yaml
...
shay profile delete research-bot --yes
shay profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
shay profile update research-bot --force-config --yes
```
...
shay profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.shay/profiles/forked-research-bot/
...
shay profile install ~/.shay/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
shay profile delete research-bot-test --yes
shay profile install ~/.shay/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `shay -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `shay profile` — not a parallel command tree.
...
- Reserved profile names (`shay`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`shay profile export` / `import`](../reference/profile-commands.md#shay-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Shay-Shay](../guides/use-soul-with-shay.md) — authoring personalities
