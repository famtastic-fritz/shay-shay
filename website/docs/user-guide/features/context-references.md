...
Type `@` followed by a reference to inject content directly into your message. Shay-Shay expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Shay-Shay env: `$SHAY_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$SHAY_HOME/skills/.hub/`
