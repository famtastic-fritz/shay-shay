...
SHAY_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot") while
_PROVIDER_MODELS and config.yaml use Shay-Shay IDs ("copilot").  The slug
...
def test_copilot_uses_shay_slug():
...
def test_kimi_for_coding_overlay_uses_shay_slug():
...
def test_kilo_overlay_uses_shay_slug():
...
def test_mapped_provider_credential_pool_visibility(monkeypatch):
    ...
    monkeypatch.setattr(
        "shay_cli.auth._load_auth_store",
