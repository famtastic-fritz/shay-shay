...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Shay-Shay to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Shay-Shay to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.shay/.env`:
...
All standard Shay-Shay commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.shay/logs/gateway.log`
...
4. Consider enabling verbose logging: `SHAY_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Shay-Shay automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
SHAY_LOG_LEVEL=debug shay gateway
```
...
```bash
shay chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
