...
class TestSendMessageTool:
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "SHAY_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "SHAY_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with patch("tools.send_message_tool._send_slack", send):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    SimpleNamespace(enabled=True, token="***", extra={}),
                    "C123",
                    "**hello** from [Shay-Shay](<https://example.com>)",
                )
            )
        ...
        send.assert_awaited_once_with(
            "***",
            "C123",
            "*hello* from <https://example.com|Shay-Shay>",
        )
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert result["success"] is True
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from shay")
...
class TestParseTargetRefMatrix:
    ...
    def test_matrix_user_mxid_is_explicit(self):
        ...
        chat_id, thread_id, is_explicit = _parse_target_ref("matrix", "@shay:matrix.org")
        assert chat_id == "@shay:matrix.org"
        assert thread_id is None
        assert is_explicit is True
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``shay cron``
    runs separately from ``shay gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``SHAY_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``SHAY_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``SHAY_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring SHAY_KANBAN_TASK must not depend on importing or calling
        gateway.status — the worker may run with a SHAY_HOME that has no
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``SHAY_SESSION_PLATFORM=local`` means CLI-style — must defer to
