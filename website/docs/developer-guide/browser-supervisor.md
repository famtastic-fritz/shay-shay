...
[PR #12550](https://github.com/NousResearch/shay-shay/pull/12550) proposed a
...
(`shay-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Shay-Shay `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `shay-acp`, `shay-api-server`, core toolsets (gated on CDP reachability)
...
- `shay_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
