"""Tests for the ``shay hooks`` CLI subcommand."""
...
from shay_cli import hooks as hooks_cli


@pytest.fixture(autouse=True)
def _isolated_home(tmp_path, monkeypatch):
    monkeypatch.setenv("SHAY_HOME", str(tmp_path / "home"))
    monkeypatch.delenv("SHAY_ACCEPT_HOOKS", raising=False)
...
class TestHooksList:
    def test_empty_config(self, tmp_path):
        with patch("shay_cli.config.load_config", return_value={}):
    ...
    def test_shows_configured_and_consent_status(self, tmp_path):
        ...
        with patch("shay_cli.config.load_config", return_value=cfg):
            out = _run(SimpleNamespace(hooks_action="list"))
...
class TestHooksDoctor:
    ...
    def test_unallowlisted_script_is_not_executed(self, tmp_path):
        """Regression for M4: `shay hooks doctor` used to run every
