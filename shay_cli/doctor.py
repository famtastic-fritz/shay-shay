"""
Doctor command for shay CLI.

Diagnoses issues with Shay-Shay setup.
"""
...
from shay_cli.config import get_project_root, get_shay_home, get_env_path
from shay_cli.env_loader import load_shay_dotenv
from shay_constants import display_shay_home

PROJECT_ROOT = get_project_root()
SHAY_HOME = get_shay_home()
_DHH = display_shay_home()  # user-facing display path (e.g. ~/.shay or ~/.shay/profiles/coder)

# Load environment variables from ~/.shay/.env so API key checks work
_env_path = get_env_path()
load_shay_dotenv(shay_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")

from shay_cli.colors import Colors, color
from shay_cli.models import _SHAY_USER_AGENT
from shay_cli.vercel_auth import describe_vercel_auth
from shay_constants import OPENROUTER_MODELS_URL
...
from shay_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.shay/.env contains provider auth/base URL settings."""
...
def _is_kanban_worker_env_gate(item: dict) -> bool:
    ...
    if item.get("name") != "kanban":
        return False
    if os.environ.get("SHAY_KANBAN_TASK"):
        return False
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("SHAY_KANBAN_TASK"):
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from shay_cli.providers import normalize_provider as _normalize_provider
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `shay`.
    os.environ.setdefault("SHAY_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Shay-Shay Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.shay/.env (primary location for user config)
    env_path = SHAY_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'shay setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'shay setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'shay setup' to create one")
                issues.append("Run 'shay setup' to create .env")
    
    # Check ~/.shay/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = SHAY_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from shay_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'shay config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in {"auto", "custom", "openrouter"}:
                try:
                    from shay_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.shay/.env or run 'shay setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'shay setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'shay config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from shay_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'shay setup' to migrate config")
                else:
                    issues.append("Run 'shay doctor --fix' or 'shay setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'shay doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses Shay-Shay' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `shay auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    if shay_home.exists():
        ...
        shay_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = shay_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Shay-Shay a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Shay-Shay Persona\n\n"
                "<!-- Edit this file to customize how Shay-Shay communicates. -->\n\n"
                "You are Shay-Shay, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = shay_home / "memories"
    ...
    state_db_path = shay_home / "state.db"
    ...
    wal_path = shay_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'shay doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (shay bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "shay"
        ...
        _cmd_link = _cmd_link_dir / "shay"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(shay not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/shay → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/shay points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/shay → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/shay — run 'shay doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/shay exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/shay not found",
                    "(shay command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/shay → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/shay symlink — run 'shay doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'shay-shay[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'shay-shay[vercel]'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `shay doctor` invocation.
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'shay config set model.provider <provider>' "
    ...
    # never the legitimate source for `shay doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'shay setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: shay skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = SHAY_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: shay memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: shay memory setup")
                issues.append("No Honcho API key — run 'shay memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run shay memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: shay memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: shay memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from shay_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "shay -p" in content:
                            _m = _re.search(r"shay -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'shay doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
