...
Let the shay-shay join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where shay runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `shay meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `shay meet` CLI |
...
| `cli.py` | `shay meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$SHAY_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `shay meet node {run,list,approve,remove,status,ping}` |
...
shay meet install                                      # pip + Chromium
shay meet setup                                        # preflight
shay meet auth                                         # optional
shay meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
shay meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.shay/.env
shay meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
shay meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
shay meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.shay/.env
shay meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, shay will **not** switch your system audio input automatically — the
...
shay plugins enable google_meet
shay meet node run --display-name my-mac --host 0.0.0.0 --port 18789
