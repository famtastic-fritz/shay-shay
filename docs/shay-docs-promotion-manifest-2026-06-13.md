# Shay docs promotion manifest — 2026-06-13

Status: captain promotion manifest, later reviewed by Fritz
Purpose: classify the current local docs packet into promote-now, active-completion-scope, and scratch/duplicate so the first merge into `main` stays clean while preserving the now-approved completion surface.

## Promote now

These belong in the first honest docs/control merge because they improve truth without pretending the runtime is finished.

- `docs/shay-full-autonomy-completion-mission-2026-06-13.md`
- `docs/shay-full-autonomy-completion-mission-amendment-2026-06-13.md`
- `docs/shay-life-os-interview-2026-06-13.md`
- `docs/shay-life-os-completion-implementation-plan-2026-06-13.md`
- `docs/shay-master-open-items-and-completion-tracker-2026-06-13.md`
- `docs/shay-master-open-items-and-completion-tracker-2026-06-13.yaml`
- `docs/shay-hyperswarm-final-mission-report-2026-06-13.md`
- `docs/shay-command-surface-map-2026-06-13.md`
- `docs/shay-command-surface-map-2026-06-13.yaml`
- `docs/shay-current-intelligence-schedule-audit-2026-06-13.md`
- `docs/shay-process-intelligence-architecture-2026-06-13.md`
- `docs/shay-run-ledger-schema-2026-06-13.yaml`
- `docs/shay-decision-ledger-schema-2026-06-13.yaml`
- `docs/shay-tool-agent-ledger-schema-2026-06-13.yaml`
- `docs/shay-artifact-ledger-schema-2026-06-13.yaml`
- `docs/shay-telemetry-redaction-policy-2026-06-13.md`
- `docs/shay-after-action-review-policy-2026-06-13.md`
- `docs/shay-pattern-scanner-design-2026-06-13.md`
- `docs/shay-process-intelligence-brutal-qa-report-2026-06-13.md`
- `docs/shay-prune-recommendations-2026-06-13.md`

## Active completion scope

These were originally marked defer-later, but Fritz has now reviewed the plans. Treat them as approved completion work, not waiting-for-review material.

- `docs/shay-adoption-backlog-2026-06-13.md`
- `docs/shay-awareness-completion-assessment-2026-06-13.md`
- `docs/shay-gap-lifecycle-status-2026-06-13.md`
- `docs/shay-pattern-scanner-autonomy-policy-2026-06-13.md`
- `docs/shay-runtime-truth-audit-2026-06-13.md`
- `docs/shay-truth-surface-rubric-2026-06-13.md`
- `docs/shay-post-merge-follow-on-plan-2026-06-13.md`
- `docs/shay-process-intelligence-watcher-design-2026-06-13.md`
- `docs/shay-process-learning-loop-2026-06-13.md`
- `docs/shay-process-query-examples-2026-06-13.md`

Restored deferred stash residue:
- `docs/shay-hermes-lane-packet-2026-06-13.md`
  - restored from the stash-backed original in `stash@{0}^3`
  - not previously committed to `main`
  - keep references pointed here as branch-local evidence; do not treat it as live-runtime proof

## Scratch / duplicate / lane residue

These can stay local as provenance/evidence or be reconsidered later, but they should not shape the first merge surface.

- `docs/shay-awareness-lane-packet-2026-06-13.md`
- `docs/shay-command-surface-lane-packet-2026-06-13.md`
- `docs/shay-process-intelligence-lane-packet-2026-06-13.md`
- `docs/shay-process-intelligence-pilot-run-2026-06-13.md`
- `docs/shay-process-intelligence-review-options-2026-06-13.md`
- `docs/shay-pruner-consolidator-lane-packet-2026-06-13.md`
- `docs/shay-scheduler-watcher-lane-packet-2026-06-13.md`

## Captain call

The first merge should privilege:
- current-state truth
- operator reality
- architecture/schema contracts
- safety boundaries
- anti-overclaim review
- implementation path clarity

The first merge should avoid:
- duplicate lane packets
- example-only proof stand-ins
- stale lane snapshots presented as current truth
- paper expansion that outruns runtime reality
