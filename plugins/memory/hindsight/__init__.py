...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID                — memory bank identifier (default: shay)
...
Or via $SHAY_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Shay-Shay can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $SHAY_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_shay_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "shay": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "shay"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Shay-Shay config."""
    profile = config.get("profile", "shay")
    return str(profile or "shay")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Shay-Shay profile name (from agent_identity)
      {workspace} — Shay-Shay workspace name (from agent_workspace)
      {platform}  — "cli", "telegram", "discord", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``shay-{user}`` with no user becomes ``shay``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Shay-Shay and the User"
    ...
    def save_config(self, values, shay_home):
        """Write config to $SHAY_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(shay_home) / "hindsight"
    ...
    def post_setup(self, shay_home: str, config: dict) -> None:
        ...
        from shay_cli.memory_setup import _curses_select
        ...
        provider_config.setdefault("bank_id", "shay")
        ...
        self.save_config(provider_config, shay_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(shay_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(shay_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "shay"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: shay-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Shay-Shay and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "shay"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "shay"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "shay", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "shay")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Shay-Shay and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Shay-Shay created/used that client on the shared async loop,
