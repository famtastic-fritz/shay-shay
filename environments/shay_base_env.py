"""
ShayAgentBaseEnv -- Abstract Base Environment for Shay-Shay-Agent + Atropos

Provides the Atropos integration plumbing that all shay-shay environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via ShayAgentLoop
...
# Ensure the shay-shay repo root is on sys.path so that imports like
...
# Load API keys from shay-shay/.env so all environments can access them
...
from environments.agent_loop import AgentResult, ShayAgentLoop
...
# Import shay-shay toolset infrastructure
...
class ShayAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for shay-shay Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of shay toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: shay, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/shay-results/ "
...
class ShayAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for shay-shay Atropos integration.
    ...
    name: Optional[str] = "shay-shay"
    env_config_cls = ShayAgentEnvConfig

    def __init__(
        self,
        config: ShayAgentEnvConfig,
        ...
        # Set terminal environment variables so shay tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY shay-shay tool (terminal, file, web,
