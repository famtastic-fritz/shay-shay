import type { RunExternalProcess } from '@shay/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`shay ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching shay: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`shay ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
