...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NousResearch/shay-shay/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    shay_meta = (fm.get("metadata") or {}).get("shay") or {}
    tags = shay_meta.get("tags") or []
    related = shay_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `shay skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Shay-Shay"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Shay-Shay ships with a large built-in skill library copied into `~/.shay/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Shay-Shay also syncs bundled skills on `shay update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.shay/skills/` tree, it is still shipped with Shay-Shay; restore it with `shay skills reset <name> --restore`.",
        "",
