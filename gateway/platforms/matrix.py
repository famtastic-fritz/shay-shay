...
# Uses get_shay_home() so each profile gets its own Matrix store.
from shay_constants import get_shay_dir as _get_shay_dir

_STORE_DIR = _get_shay_dir("platforms/matrix/store", "matrix/store")
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(
            os.getenv("SHAY_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6")
        )
        self._text_batch_split_delay_seconds = float(
            os.getenv("SHAY_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0")
        )
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption:
            ...
            try:
                ...
                _acct_id = self._user_id or "shay"
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        otherwise normal phrases like "Shay-Shay" become "Agent".
        """
        if not body:
            return ""
