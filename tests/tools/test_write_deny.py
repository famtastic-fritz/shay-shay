...
class TestWriteDenyExactPaths:
    ...
    def test_shay_env(self):
        # ``.env`` under the active SHAY_HOME (profile-aware, not just
        # ``~/.shay``) must be write-denied. The hermetic test conftest
        # points SHAY_HOME at a tempdir — resolve via get_shay_home()
        # to match the denylist.
        from shay_constants import get_shay_home
        path = str(get_shay_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_shay_config_not_env(self):
        path = os.path.join(str(Path.home()), ".shay", "config.yaml")
