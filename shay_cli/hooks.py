"""shay hooks — inspect and manage shell-script hooks.

Usage::

    shay hooks list
    shay hooks test <event> [--for-tool X] [--payload-file F]
    shay hooks revoke <command>
    shay hooks doctor

Consent records live under ``~/.shay/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.shay/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``shay hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: shay hooks {list|test|revoke|doctor}")
        print("Run 'shay hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.shay/config.yaml.")
        print("See `shay hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `shay hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `shay hooks test` and `shay hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Shay-Shay wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `shay hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `shay hooks doctor` would execute
