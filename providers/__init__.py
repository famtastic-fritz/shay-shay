...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with shay-shay)
2. User plugins: ``$SHAY_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$SHAY_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$SHAY_HOME/plugins/model-providers/`` if it exists."""
    try:
        from shay_constants import get_shay_home

        d = get_shay_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple SHAY_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_shay_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$SHAY_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with shay-shay.
    ...
    # 2. User plugins — under $SHAY_HOME/plugins/model-providers/<name>/.
