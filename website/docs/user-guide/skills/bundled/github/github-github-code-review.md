...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.shay/.env ] && grep -q "^GITHUB_TOKEN=" ~/.shay/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.shay/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Shay-Shay\",
    \"comments\": [
...
```bash
source "${SHAY_HOME:-$HOME/.shay}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Shay-Shay. Code looks clean — good test coverage, no security concerns."
