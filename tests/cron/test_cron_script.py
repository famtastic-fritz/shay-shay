...
- Path resolution (absolute, relative to SHAY_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp SHAY_HOME."""
    shay_home = tmp_path / ".shay"
    shay_home.mkdir()
    (shay_home / "cron").mkdir()
    (shay_home / "cron" / "output").mkdir()
    (shay_home / "scripts").mkdir()
    monkeypatch.setenv("SHAY_HOME", str(shay_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "SHAY_DIR", shay_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", shay_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", shay_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", shay_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.shay/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("SHAY_SESSION_PLATFORM") is None
        assert os.environ.get("SHAY_SESSION_CHAT_ID") is None
        assert os.environ.get("SHAY_SESSION_CHAT_NAME") is None
