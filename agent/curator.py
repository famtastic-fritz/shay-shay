...
def _state_file() -> Path:
    return get_shay_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.shay/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``shay update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``shay curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `shay curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Shay-Shay' background skill CURATOR. This is an "
    ...
    "into ~/.shay/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: shay-config-*, shay-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.shay/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.shay/logs/curator/``)
    ...
    authored skill data in ``~/.shay/skills/``.

    ``ensure_shay_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_shay_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every shay install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: shay curator status
        keep an umbrella stable: shay curator pin document-tools
    ...
    lines.append("full report: shay curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: shay curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.shay/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.shay/skills/.archive/` for "
            "safety and can be restored via `shay curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.shay/skills/.archive/`. "
            "Restore any via `shay curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `shay curator restore <name>`")
    lines.append("- All archives live under `~/.shay/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `shay curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `shay curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``shay model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `shay model` → auxiliary
