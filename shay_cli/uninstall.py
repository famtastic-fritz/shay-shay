"""
Shay-Shay Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.shay/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Shay-Shay PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing shay-shay or shay PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Shay-Shay" comment and following line
                if '# Shay-Shay' in line or '# shay-shay' in line:
                    skip_next = True
                    continue
                if skip_next and ('shay' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing shay
                if 'shay' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the shay wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "shay",
        Path("/usr/local/bin/shay"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains shay_cli reference)
                content = wrapper.read_text()
                if 'shay_cli' in content or 'shay-shay' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``shay gateway run`` processes
    ...
    try:
        from shay_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from shay_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``SHAY_HOME`` and ``SHAY_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\shay\git\cmd``, ``%LOCALAPPDATA%\shay\git\bin``,
#      ``%LOCALAPPDATA%\shay\git\usr\bin``, ``%LOCALAPPDATA%\shay\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\shay\git\`` and Node to
#      ``%LOCALAPPDATA%\shay\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``shay dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\shay\gateway-service\`` and sometimes
...
def _shay_path_markers(shay_home: Path) -> list[str]:
    """Path-entry substrings that identify Shay-Shay-owned User-PATH entries."""
    root = str(shay_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare shay-shay install dir.
    markers = [root + "\\shay-shay", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if SHAY_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "shay".  We don't want to catch
    # unrelated entries like "cshay-foo" or "ephermeral", so we look for
    # backslash-shay as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(shay_home: Path) -> list[str]:
    """Strip Shay-Shay-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _shay_path_markers(shay_home)
            kept: list[str] = []
...
def remove_shay_env_vars_windows() -> list[str]:
    """Delete SHAY_HOME and SHAY_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("SHAY_HOME", "SHAY_GIT_BASH_PATH"):
                try:
...
def _is_default_shay_home(shay_home: Path) -> bool:
    """Return True when ``shay_home`` points at the default (non-profile) root."""
    try:
        from shay_constants import get_default_shay_root
        return shay_home.resolve() == get_default_shay_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its SHAY_HOME directory.

    We shell out to ``shay -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current SHAY_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m shay_cli.main` so we don't depend on a `shay`
    #    wrapper that may be half-removed mid-uninstall.
    shay_invocation = [_sys.executable, "-m", "shay_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                shay_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's SHAY_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.shay/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.shay/ for future reinstall
    """
    project_root = get_project_root()
    shay_home = get_shay_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie SHAY_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_shay_home(shay_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Shay-Shay Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {shay_home / 'config.yaml'}")
    print(f"  Secrets: {shay_home / '.env'}")
    print(f"  Data:    {shay_home / 'cron/'}, {shay_home / 'sessions/'}, {shay_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their SHAY_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Shay-Shay data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Shay-Shay code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in shay_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\shay\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(shay_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Shay-Shay-owned PATH entries in User environment")

        log_info("Removing SHAY_HOME / SHAY_GIT_BASH_PATH User env vars...")
        removed_env = remove_shay_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Shay-Shay-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing shay command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.shay/, just remove the shay-shay subdir
            if shay_home in project_root.parents or project_root.parent == shay_home:
    ...
    #     under SHAY_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(shay_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(shay_home)
    ...
    # 5. Optionally remove ~/.shay/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile SHAY_HOME dirs live under
        ...
        try:
            if shay_home.exists():
                shutil.rmtree(shay_home)
                log_success(f"Removed {shay_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {shay_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {shay_home}")
    ...
    if not full_uninstall:
        ...
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/NousResearch/shay-shay/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/NousResearch/shay-shay/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Shay-Shay! ⚕")
    print()
