...
class TestParserRegistry:
    ...
    def test_shay_parser_registered(self):
        parsers = list_parsers()
        assert "shay" in parsers
...
# ─── Shay-Shay parser tests ────────────────────────────────────────────────

class TestShayParser:
    @pytest.fixture
    def parser(self):
        return get_parser("shay")
...
class TestParseResultContract:
    ...
    @pytest.fixture(params=["shay"])  # Add more as needed
    ...
    def test_tool_calls_are_proper_objects(self, parser):
        ...
        # Use shay format since that's universal
