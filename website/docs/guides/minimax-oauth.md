...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Shay-Shay — no API key required"
---

# MiniMax OAuth

Shay-Shay supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Shay-Shay automatically refreshes your session.
...
# → Shay-Shay opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.shay/auth.json` and are refreshed automatically before each session.
...
```bash
shay auth add minimax-cn --type oauth   # if OAuth is supported on your CN account
# or simpler:
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.shay/.env
```
...
```bash
shay auth add minimax-oauth --no-browser
```

Shay-Shay will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Shay-Shay implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. Shay-Shay generates a PKCE verifier / challenge pair and a random state value.
...
4. Shay-Shay polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.shay/auth.json` under the `minimax-oauth` key.
...
```bash
shay config set model MiniMax-M2.7
shay config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.shay/config.yaml` will contain entries similar to:
...
```bash
shay --provider minimax-oauth    # canonical
shay --provider minimax-portal   # alias
shay --provider minimax-global   # alias
shay --provider minimax_oauth    # alias (underscore form)
```
...
```bash
SHAY_INFERENCE_PROVIDER=minimax-oauth shay
```
...
Shay-Shay refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Shay-Shay marks the session as requiring re-login.

**Fix:** run `shay auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Shay-Shay raises a timeout error.

**Fix:** re-run `shay auth add minimax-oauth` (or `shay model`). The flow starts fresh.

### State mismatch (possible CSRF)

Shay-Shay detected that the `state` value returned by the authorization server does not match what it sent.
...
If `shay` cannot open a browser window, use `--no-browser`:

```bash
shay auth add minimax-oauth --no-browser
```

Shay-Shay prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `shay model` and select MiniMax (OAuth), or run `shay auth add minimax-oauth`.
...
```bash
shay auth remove minimax-oauth
```
...
- [shay doctor](../reference/cli-commands.md)
