...
shay meet install                 # pip deps + Chromium (transcribe only)
shay meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
shay meet auth                    # optional; skips guest-lobby wait
shay meet setup                   # preflight checks
```

`shay meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or SHAY_MEET_REALTIME_KEY in ~/.shay/.env
```
...
pip install playwright websockets && python -m playwright install chromium
shay plugins enable google_meet
shay meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
shay meet node approve my-mac ws://<mac-ip>:18789 <token>
shay meet node ping my-mac                   # confirm reachable
```

Run `shay meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Shay-Shay bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `shay meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `SHAY_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `shay_meet_src`). |
...
Local:
```
$SHAY_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$SHAY_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `shay meet node approve`.
