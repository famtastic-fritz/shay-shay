...
def _is_shay_provider_credential(name: str) -> bool:
    """True if ``name`` is a Shay-Shay-managed provider credential (API key,
    token, or similar) per ``_SHAY_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Shay-Shay API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _SHAY_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _SHAY_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Shay-Shay-managed provider credentials (from
    ``_SHAY_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Shay-Shay-managed
    ...
    Non-Shay-Shay third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_shay_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Shay-Shay provider "
                "credential %r (blocked by _SHAY_PROVIDER_ENV_BLOCKLIST). "
