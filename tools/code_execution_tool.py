...
Lets the LLM write a Python script that calls Shay-Shay tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `shay_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `shay_tools.py` with file-based RPC stubs
...
# available on every platform Shay-Shay itself runs on.
...
def generate_shay_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the shay_tools.py stub module.
...
"""Auto-generated Shay-Shay tools RPC stubs."""
...
def _connect():
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["SHAY_RPC_SOCKET"]
...
"""Auto-generated Shay-Shay tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("SHAY_RPC_DIR") or os.path.join(tempfile.gettempdir(), "shay_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated shay_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/shay_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_shay_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/shay_tools.py", tools_src)
        ...
        env_prefix = (
            f"SHAY_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("SHAY_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
def execute_code(
    ...
    to a subset of Shay-Shay tools.
    ...
    # --- Set up temp directory with shay_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="shay_sandbox_")
    ...
    # selector from SHAY_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"shay_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated shay_tools module.
        ...
        tools_src = generate_shay_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "shay_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  SHAY_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["SHAY_RPC_SOCKET"] = rpc_endpoint
        ...
        # Ensure the shay-shay root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from shay_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _shay_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _shay_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # SHAY_TIMEZONE is an internal Shay-Shay setting and must not leak
        # into child processes.
        _tz_name = os.getenv("SHAY_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("SHAY_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {SHAY_HOME}/home/ when that directory exists.
        from shay_constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.shay/.env')).
...
def _load_config() -> dict:
    ...
    ``shay --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (shay-shay's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``shay tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and shay-shay's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.shay/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Shay-Shay tools programmatically. "
        ...
        f"Available via `from shay_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into shay_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from shay_tools import {import_str}` "
