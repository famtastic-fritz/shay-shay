...
// Deferred @shay/ink import: loading `@shay/ink` at module top-level
...
async function _ensureEvictInkCaches(): Promise<(level: 'all' | 'half') => unknown> {
  ...
  _evictInkCachesPromise ??= import('@shay/ink')
    .then(mod => {
...
export function startMemoryMonitor({
  ...
  const tick = async () => {
    ...
    // Deferred import keeps `@shay/ink` off the cold-start critical path;
