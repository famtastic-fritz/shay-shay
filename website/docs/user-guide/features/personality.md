...
description: "Customize Shay-Shay's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Shay-Shay's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `SHAY_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Shay-Shay is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Shay-Shay now seeds a default `SOUL.md` automatically in:

```text
~/.shay/SOUL.md
```

More precisely, it uses the current instance's `SHAY_HOME`, so if you run Shay-Shay with a custom home directory, it will use:
...
- Shay-Shay creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Shay-Shay loads `SOUL.md` only from `SHAY_HOME`
- Shay-Shay does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Shay-Shay falls back to a built-in default identity
...
If Shay-Shay loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `SHAY_HOME`, the personality belongs to the Shay-Shay instance itself.

That also makes it easier to teach users:
- "Edit `~/.shay/SOUL.md` to change Shay-Shay' default personality."
...
- how Shay-Shay should handle uncertainty, disagreement, or ambiguity
...
## What Shay-Shay injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Shay-Shay falls back to a built-in default identity ("You are Shay-Shay, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Shay-Shay ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Shay-Shay, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Shay-Shay its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.shay/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.shay/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-shay)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Shay-Shay speaks
- `display.skin` and `/skin` affect how Shay-Shay looks in the terminal
