...
description: "Run Shay-Shay directly on an Android phone with Termux"
---

# Shay-Shay on Android with Termux

This is the tested path for running Shay-Shay directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Shay-Shay from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Shay-Shay now ships a Termux-aware installer path:
...
- links `shay` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `shay` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/shay" "$PREFIX/bin/shay"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `shay` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.shay/.env`.
...
### `shay doctor` says ripgrep or Node is missing
