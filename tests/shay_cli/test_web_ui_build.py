...
Critical invariant: the Vite build outputs to shay_cli/web_dist/
(vite.config.ts: outDir: "../shay_cli/web_dist"), NOT web/dist/.
...
from shay_cli.main import _web_ui_build_needed, _build_web_ui, _run_npm_install_deterministic
...
def _make_web_dir(tmp_path: Path) -> tuple[Path, Path]:
    ...
    dist_dir = tmp_path / "shay_cli" / "web_dist"
...
class TestWebUIBuildNeeded:
    ...
    def test_web_dist_dir_not_web_dist_subdir(self, tmp_path):
        """Regression: sentinel must be in shay_cli/web_dist/, NOT web/dist/."""
        web_dir, dist_dir = _make_web_dir(tmp_path)
        _touch(web_dir / "src" / "App.tsx", offset=-10)
...
class TestBuildWebUISkipsWhenFresh:
    ...
    def test_skips_npm_when_dist_is_fresh(self, tmp_path):
        ...
        with patch("shay_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("shay_cli.main.subprocess.run") as mock_run:
    ...
    def test_runs_npm_when_dist_missing(self, tmp_path):
        ...
        with patch("shay_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("shay_cli.main.subprocess.run", return_value=mock_cp) as mock_run:
            result = _build_web_ui(web_dir)
    ...
    def test_npm_install_uses_utf8_replace_output_decoding(self, tmp_path):
        ...
        with patch("shay_cli.main.subprocess.run", return_value=mock_cp) as mock_run:
    ...
    def test_web_build_uses_utf8_replace_output_decoding(self, tmp_path):
        ...
        mock_cp = __import__("subprocess").CompletedProcess([], 0, stdout="", stderr="")
        with patch("shay_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("shay_cli.main.subprocess.run", side_effect=[mock_cp, mock_cp]) as mock_run:
            result = _build_web_ui(web_dir)
...
class TestBuildWebUIRetryAndStaleFallback:
    ...
    def test_retries_build_once_on_failure(self, tmp_path):
        ...
        with patch("shay_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("shay_cli.main._time.sleep") as mock_sleep, \
             patch("shay_cli.main.subprocess.run",
    ...
    def test_falls_back_to_stale_dist_when_retry_also_fails(self, tmp_path, capsys):
        ...
        with patch("shay_cli.main.shutil.which", return_value="/usr/bin/npm"), \
             patch("shay_cli.main._time.sleep"), \
             patch("shay_cli.main.subprocess.run",
