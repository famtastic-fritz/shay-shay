...
| | |
|---|---|
| Source | Optional — install with `shay skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Shay-Shay or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Shay-Shay. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.shay/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.shay/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Shay-Shay integration, either:

- configure the server in `~/.shay/config.yaml` using the `native-mcp` skill, or
...
### Shay-Shay cannot see the deployed server

The server-building part may be correct while the Shay-Shay config is not. Load the `native-mcp` skill and configure the server in `~/.shay/config.yaml`, then restart Shay-Shay.
