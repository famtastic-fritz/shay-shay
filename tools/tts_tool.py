...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.shay/config.yaml``. Shay-Shay
...
Configuration is loaded from ~/.shay/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``shay_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from shay_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import shay_xai_user_agent
...
def _get_default_output_dir() -> str:
    from shay_constants import get_shay_dir
    return str(get_shay_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.shay/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.shay/config.yaml.
    ...
    try:
        ...
        logger.debug("shay_cli.config not available, using default TTS config")
        return {}
...
# custom voice-cloning scripts, etc.) into Shay-Shay without any Python code
...
#           output_format: wav
#
# Shay-Shay writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__SHAY_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when Shay-Shay actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": shay_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Shay-Shay caches Piper voice models.

    Resolves to ``~/.shay/cache/piper-voices/`` under the active
    SHAY_HOME so voice downloads follow profile boundaries.
    """
    from shay_constants import get_shay_dir
    root = Path(get_shay_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.shay/config.yaml (tts: section).
    ...
    platform = get_session_env("SHAY_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'shay-shay[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run shay setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'shay setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'shay tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_shay_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
