...
def test_shay_provider_subclass_exists():
    """ShayMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _SHAY_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _SHAY_PROVIDER_CLS is not None
    assert issubclass(_SHAY_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_shay_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ShayMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _SHAY_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("SHAY_HOME", str(tmp_path))
    ...
    assert _SHAY_PROVIDER_CLS is not None
    assert isinstance(provider, _SHAY_PROVIDER_CLS)
    assert provider._shay_server_name == "srv"
