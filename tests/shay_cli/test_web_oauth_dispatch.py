"""Regression tests for the OAuth dispatcher in shay_cli.web_server.
...
from shay_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Shay-Shay-Session-Token": _SESSION_TOKEN}
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "shay_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "shay_cli.auth._minimax_pkce_pair",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "shay_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    """
    from shay_cli import web_server as ws
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "shay auth add hypothetical-pkce-provider",
