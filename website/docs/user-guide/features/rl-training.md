...
Shay-Shay includes an integrated RL (Reinforcement Learning) training pipeline built on **Tinker-Atropos**. This enables training language models on environment-specific tasks using GRPO (Group Relative Policy Optimization) with LoRA adapters, orchestrated entirely through the agent's tool interface.
...
- The `tinker-atropos` submodule (at `tinker-atropos/` relative to the Shay-Shay root)

```bash
# Set up API keys
shay config set TINKER_API_KEY your-tinker-key
shay config set WANDB_API_KEY your-wandb-key
```
...
Each training run generates log files in `~/.shay/logs/rl_training/`:
