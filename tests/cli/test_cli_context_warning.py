...
def _isolate(tmp_path, monkeypatch):
    """Isolate SHAY_HOME so tests don't touch real config."""
    home = tmp_path / ".shay"
    home.mkdir()
...
def cli_obj(_isolate):
    """Create a minimal ShayCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ShayCLI.__new__(ShayCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["shay-core"]
        obj.compact = False
