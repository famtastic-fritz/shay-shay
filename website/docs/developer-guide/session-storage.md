...
Shay-Shay uses a SQLite database (`~/.shay/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `shay_state.py`


## Architecture Overview

```
~/.shay/state.db (SQLite, WAL mode)
...
Multiple shay processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.shay/state.db
...
Default path: `~/.shay/state.db`

This is derived from `shay_constants.get_shay_home()` which resolves to
`~/.shay/` by default, or the value of `SHAY_HOME` environment variable.
