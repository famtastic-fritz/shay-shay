...
def _get_shay_version() -> str:
    """Return the shay-shay package version, or 'dev' if unavailable."""
    try:
        from importlib.metadata import version
        return version("shay-shay")
    except Exception:
        return "dev"
...
def build_user_agent() -> str:
    ...
    Format::
        ...
        QQBotAdapter/<qqbot_version> (Python/<py_version>; <os>; Shay-Shay/<shay_version>)

    Example::

        QQBotAdapter/1.0.0 (Python/3.11.15; darwin; Shay-Shay/0.9.0)
    """
    py_version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"
    os_name = platform.system().lower()
    shay_version = _get_shay_version()
    return f"QQBotAdapter/{QQBOT_VERSION} (Python/{py_version}; {os_name}; Shay-Shay/{shay_version})"
