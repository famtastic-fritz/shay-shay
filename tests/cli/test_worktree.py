...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("shay/shay-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned shay/* and pr-* branches."""

    def test_prunes_orphaned_shay_branch(self, git_repo):
        """shay/shay-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "shay/shay-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "shay/shay-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "shay/shay-deadbeef" in result.stdout
        ...
        assert "shay/shay-deadbeef" in orphaned
        ...
        assert "shay/shay-deadbeef" not in result.stdout
