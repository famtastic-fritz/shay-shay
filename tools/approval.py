...
def _fire_approval_hook(hook_name: str, **kwargs) -> None:
    ...
    """
    try:
        from shay_cli.plugins import invoke_hook
    except Exception:
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("SHAY_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("SHAY_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("SHAY_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set SHAY_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind SHAY_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds SHAY_SESSION_PLATFORM via
    ...
    """
    if os.getenv("SHAY_CRON_SESSION"):
        return False
    if os.getenv("SHAY_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $SHAY_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_SHAY_ENV_PATH = (
    r'(?:~\/\.shay/|'
    r'(?:\$home|\$\{home\})/\.shay/|'
    r'(?:\$shay_home|\$\{shay_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bkillall\b(?=.*(?:\s-(?:9|KILL|SIGKILL)\b|\s-s\s+(?:9|KILL)\b|\s-r\b))', "force/regex killall processes"),
    (r':\(\)\s*\{\s*:\s*\|\s*:\s*&\s*\}\s*;\s*:', "fork bomb"),
    ...
    (r'\bshay\s+gateway\s+(stop|restart)\b', "stop/restart shay gateway (kills running agents)"),
    (r'\bshay\s+update\b', "shay update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart shay-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart shay-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(shay|gateway|cli\.py)\b', "kill shay/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill shay` but not
    # `kill -9 $(pgrep -f shay)` because the substitution is opaque
    # to regex at detection time. Catch the structural pattern instead.
    (r'\bkill\b.*\$\(\s*pgrep\b', "kill process via pgrep expansion (self-termination)"),
    (r'\bkill\b.*`\s*pgrep\b', "kill process via backtick pgrep expansion (self-termination)"),
    # File copy/move/edit into sensitive system paths
    (rf'\b(cp|mv|install)\b.*\s["\']?{_SYSTEM_CONFIG_PATH}', "copy/move file into system config"),
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["SHAY_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "SHAY_SPINNER_PAUSE" in os.environ:
            del os.environ["SHAY_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("SHAY_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("SHAY_INTERACTIVE")
    ...
    if is_gateway or os.getenv("SHAY_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("SHAY_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("SHAY_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = os.getenv("SHAY_EXEC_ASK")
