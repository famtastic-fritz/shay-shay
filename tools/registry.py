"""Central registry for all shay-shay tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``shay tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``shay tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``shay tools enable foo``)
        ...
        * All exceptions are caught and returned as ``{"error": "..."}``
        ...
        try:
            ...
            try:
                ...
                sanitized = raw  # defensive: never let sanitizer block errors
