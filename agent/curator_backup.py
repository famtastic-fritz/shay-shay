...
A pre-run snapshot of ``~/.shay/skills/`` (excluding ``.curator_backups/``
itself) is taken before any mutating curator pass. Snapshots are tar.gz
files under ``~/.shay/skills/.curator_backups/<utc-iso>/`` with a
...
``~/.shay/cron/jobs.json`` as ``cron-jobs.json`` when it exists. Cron
...
def _backups_dir() -> Path:
    return get_shay_home() / "skills" / ".curator_backups"
...
def _cron_jobs_file() -> Path:
    """Source path for the live cron jobs store (``~/.shay/cron/jobs.json``)."""
    return get_shay_home() / "cron" / "jobs.json"
...
def snapshot_skills(reason: str = "manual") -> Optional[Path]:
    """Create a tar.gz snapshot of ``~/.shay/skills/`` and prune old ones.
    ...
    if not skills.exists():
        logger.debug("No ~/.shay/skills/ directory — nothing to back up")
        return None
...
def rollback(backup_id: Optional[str] = None) -> Tuple[bool, str, Optional[Path]]:
    """Restore ``~/.shay/skills/`` from a snapshot.
    ...
    Strategy:
      ...
      4. Extract the chosen snapshot into ``~/.shay/skills/``.
    ...
    if target is None:
        return (
            ...
            + " (use `shay curator rollback --list` to see available snapshots)",
            None,
        )
