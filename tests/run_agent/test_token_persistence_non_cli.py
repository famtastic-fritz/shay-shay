...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    shay_state = ModuleType("shay_state")
    shay_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "shay_state", shay_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Shay-Shay"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Shay-Shay"
