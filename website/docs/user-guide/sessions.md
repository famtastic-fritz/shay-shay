...
Shay-Shay automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`shay` or `shay chat`) |
...
shay -c "my project"   # → resumes "my project #3"
```
...
shay --resume 20250305_091523_a1b2c3d4
shay -r 20250305_091523_a1b2c3d4

# Resume by title
shay --resume "refactoring auth"

# Or with the chat subcommand
shay chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `shay sessions list`.

### Conversation Recap on Resume

When you resume a session, Shay-Shay displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Shay-Shay session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.shay/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `shay -r "<title>"` from the shell) and pick up where the platform left off.
...
Shay-Shay automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `shay sessions list` or `shay sessions browse`.
...
```bash
shay sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Shay-Shay creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`shay -c "my project"`), it automatically picks the most recent session in the lineage.
...
Shay-Shay provides a full set of session management commands via `shay sessions`:
...
# Filter by platform
shay sessions list --source telegram

# Show more sessions
shay sessions list --limit 50
```
...
shay sessions export backup.jsonl

# Export sessions from a specific platform
shay sessions export telegram-history.jsonl --source telegram

# Export a single session
shay sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
shay sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
shay sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
shay sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
shay sessions prune --older-than 30

# Only prune sessions from a specific platform
shay sessions prune --source telegram --older-than 60

# Skip confirmation
shay sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`shay insights`](/docs/reference/cli-commands#shay-insights).
...
When Shay-Shay cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Shay-Shay uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Shay-Shay in the same Discord channel without sharing transcript history
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Shay-Shay process in the same `SHAY_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.shay/config.yaml`:
...
shay sessions delete <session_id>
