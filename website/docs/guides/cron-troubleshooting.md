...
description: "Diagnose and fix common Shay-Shay cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`shay gateway` for foreground, or `shay gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `shay cron tick`.
...
```bash
date
shay cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.shay/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.shay/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.shay/.env` |
...
| `local` | Write access to `~/.shay/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `shay cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `shay skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `shay skills list`.
...
2. `~/.shay/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `shay cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Shay-Shay config directory). Verify:
```bash
ls ~/.shay/scripts/your-script.py   # Must exist
shay cron edit <job_id> --script ~/.shay/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `shay skills install <skill-name>`.
...
Jobs are stored in `~/.shay/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.shay/cron/jobs.json
chmod 600 ~/.shay/cron/jobs.json   # Your user should own it
```
...
```bash
shay cron list                    # Show all jobs, states, next_run times
shay cron run <job_id>            # Schedule for next tick (for testing)
shay cron edit <job_id>           # Fix configuration issues
shay logs                         # View recent Shay-Shay logs
shay skills list                  # Verify installed skills
```
...
1. Run the job with `shay cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.shay/logs/agent.log` for scheduler messages and `~/.shay/logs/errors.log` for warnings
3. Open an issue at [github.com/NousResearch/shay-shay](https://github.com/NousResearch/shay-shay) with:
