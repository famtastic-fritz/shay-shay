"""ACP agent server — exposes Shay-Shay via the Agent Client Protocol."""
...
try:
    from shay_cli import __version__ as SHAY_VERSION
except Exception:
    SHAY_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Shay-Shay running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Shay-Shay cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Shay-Shay/OpenAI-compatible user content payload."""
...
class ShayACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Shay-Shay AIAgent."""
    ...
    _SLASH_COMMANDS = {
        ...
        "version": "Show Shay-Shay version",
    }
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Shay-Shay version",
        },
    )
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from shay_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from shay_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Shay-Shay estimates ``used`` from the same
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["shay-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Shay-Shay using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="shay-shay", version=SHAY_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Shay-Shay' threat model (ACP is stdio-only, local-trust), but poor
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications; merely restoring server-side state makes Shay-Shay remember
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Shay-Shay on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Shay-Shay' local kawaii waiting/status
        ...
        # not the executor's. Also set SHAY_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue SHAY_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind SHAY_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("SHAY_INTERACTIVE")
            ...
            try:
                ...
                # Restore SHAY_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("SHAY_INTERACTIVE", None)
                else:
                    os.environ["SHAY_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (SHAY_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Shay-Shay v{SHAY_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Shay-Shay has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
