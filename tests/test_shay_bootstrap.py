"""Tests for shay_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Shay-Shay entry point
(shay, shay-shay, shay-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Shay-Shay entry point imports shay_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported shay_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("shay_bootstrap", None)
    import shay_bootstrap  # noqa: WPS433
    return shay_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Shay-Shay CLI
...
class TestEntryPointsImportBootstrap:
    """Every Shay-Shay entry point must import shay_bootstrap as its
    ...
    # Entry points that invoke Shay-Shay as a process.  Each one must
    # import shay_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "shay_cli/main.py",   # shay CLI (console_script)
        "run_agent.py",          # shay-shay (console_script)
        "acp_adapter/entry.py",  # shay-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import shay_bootstrap' and that
        ...
        half-finished ``shay update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``shay_bootstrap`` as a top-level module) leaves shay
        ...
        """
        # Resolve relative to the shay-shay repo root.  Tests live
        # at tests/test_shay_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # shay start even when shay_bootstrap hasn't been
        ...
        assert first_import_name == "shay_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'shay_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import shay_bootstrap' line to be the first import."
        )
