...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Shay-Shay uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Shay-Shay has `image_generate`
...
- License: MIT (inherits from shay-shay repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Shay-Shay contributors
