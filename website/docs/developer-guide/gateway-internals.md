...
The messaging gateway is the long-running process that connects Shay-Shay to 20+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `shay_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.shay/.env` | API keys, bot tokens, platform credentials |
| `~/.shay/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (an extension point — currently empty in the shipped distribution; `_register_builtin_hooks()` is a no-op stub) and `~/.shay/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `shay gateway start` / `shay gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.shay/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `shay gateway stop` stops only the current profile's gateway. `shay gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
