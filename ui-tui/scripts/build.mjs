...
await build({
  ...
  // Skip the prebuilt @shay/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@shay/ink': resolve(root, 'packages/shay-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The shay_cli launcher
