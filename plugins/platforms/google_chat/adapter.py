...
# ``model_tools`` import time, so ``shay status``, ``shay chat``, etc.
...
# ``shay_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Shay-Shay is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a shay MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${SHAY_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _shay_home = _get_shay_home()
        except (ModuleNotFoundError, ImportError):
            _shay_home = _Path.home() / ".shay"
        self._thread_count_store = _ThreadCountStore(
            _shay_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Shay-Shay is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("SHAY_HOME", str(_Path.home() / ".shay"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${SHAY_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a shay MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Shay-Shay is thinking…' marker message.
        ...
        "Shay-Shay is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Shay-Shay is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Shay-Shay is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Shay-Shay is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Shay-Shay is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``shay setup``.

    The setup wizard at ``shay_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.shay/.env`` so the next
    gateway restart picks them up.
    """
    from shay_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. shay-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.shay/.env")
    print_info("Restart the gateway: shay gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``shay cron`` running as a
    separate process from ``shay gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Shay-Shay plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'shay-shay[google_chat]'",
        ...
        # the "Shay-Shay is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Shay-Shay is thinking…' marker message "
