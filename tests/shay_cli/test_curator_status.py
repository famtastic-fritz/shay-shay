"""Tests for `shay curator status` output.
...
def test_status_uses_last_activity_not_only_last_used(monkeypatch, capsys):
    ...
    import shay_cli.curator as curator_cli
...
def curator_status_env(tmp_path, monkeypatch):
    """Isolated SHAY_HOME with real agent-created skills on disk."""
    home = tmp_path / ".shay"
    skills = home / "skills"
    ...
    from shay_cli import curator as curator_cli
