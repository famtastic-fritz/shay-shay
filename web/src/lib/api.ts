...
// (e.g. https://mission-control.tilos.com/shay/). The Python backend
// injects ``window.__SHAY_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__SHAY_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const SHAY_BASE_PATH = readBasePath();
const BASE = SHAY_BASE_PATH;
...
declare global {
  interface Window {
    ...
    __SHAY_BASE_PATH__?: string;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Shay-Shay-Session-Token";
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__SHAY_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Shay-Shay dashboard server");
}
