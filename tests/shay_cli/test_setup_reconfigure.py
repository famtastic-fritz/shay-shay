...
- Bare `shay setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `shay setup --quick` runs the narrower "fill in missing items" flow.
- `shay setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("shay_cli.setup.ensure_shay_home", {}),
        ("shay_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("shay_cli.config.is_managed", {"return_value": False}),
        ("shay_cli.setup.load_config", {"return_value": {}}),
        ("shay_cli.setup.save_config", {}),
        ("shay_cli.setup.get_env_value", {"return_value": None}),
        ("shay_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("shay_cli.setup._print_setup_summary", {}),
        ("shay_cli.setup._offer_launch_chat", {}),
        ("shay_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("shay_cli.setup.load_config", {"return_value": {}}),
        ("shay_cli.setup.save_config", {}),
        ("shay_cli.auth.get_active_provider", {"return_value": None}),
        ("shay_cli.setup.get_env_value", {"return_value": None}),
        ("shay_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
...
class TestExistingInstallDefault:
    """Bare `shay setup` on an existing install = full reconfigure wizard."""
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="shay_cli.setup.prompt_choice",
                quick="shay_cli.setup._run_quick_setup",
                model="shay_cli.setup.setup_model_provider",
                terminal="shay_cli.setup.setup_terminal_backend",
                agent="shay_cli.setup.setup_agent_settings",
                gateway="shay_cli.setup.setup_gateway",
                tools="shay_cli.setup.setup_tools",
            )
    ...
    def test_reconfigure_flag_is_backwards_compat_noop(self, existing_install):
        """`shay setup --reconfigure` behaves the same as bare `shay setup`."""
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("shay_cli.setup.prompt_choice", {"return_value": 0}),
                first="shay_cli.setup._run_first_time_quick_setup",
            )
            from shay_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "shay_cli.setup.run_setup_wizard",
            lambda args: captured.setdefault("args", args),
        )
        monkeypatch.setattr(sys, "argv", ["shay", "setup", "--reconfigure"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_quick_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["shay", "setup", "--quick"])
        try:
            main()
        except SystemExit:
            pass
    ...
    def test_bare_setup_has_both_flags_false(self, monkeypatch):
        ...
        monkeypatch.setattr(sys, "argv", ["shay", "setup"])
        try:
            main()
        except SystemExit:
            pass
