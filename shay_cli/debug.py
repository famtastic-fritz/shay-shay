"""``shay debug`` debug tools for Shay-Shay.

Currently supports:
    shay debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.shay/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[shay debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.shay/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``shay debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``shay debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, Shay-Shay version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `shay debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "shay-shay/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``shay debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.shay/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``shay debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"shay debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "shay-shay/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----ShayDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from shay_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_shay_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``shay dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``shay dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "shay debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  shay debug delete <url>")

    print(f"\nShare these links with the Shay-Shay team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: shay debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'shay debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``shay debug`` call.
    ...
    # best-effort — any failure is swallowed so ``shay debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: shay debug <command>")
        print()
        print("Commands:")
