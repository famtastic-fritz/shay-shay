...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("shay_cli.config.get_config_path", return_value=config_path), \
             patch("shay_cli.config.ensure_shay_home"):
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("shay_cli.config.get_env_path", return_value=env_path), \
             patch("shay_cli.config.ensure_shay_home"):
    ...
    def test_ensure_shay_home_sets_0700(self):
        home = Path(self.tmpdir) / ".shay"
        with patch("shay_cli.config.get_shay_home", return_value=home):
