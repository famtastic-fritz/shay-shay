...
| | |
|---|---|
| Source | Optional — install with `shay skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Shay-Shay |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Shay-Shay terminal calls (desktop app flow only)
...
### Service Account (recommended for Shay-Shay)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.shay/.env` (the skill will prompt for this on first load).
...
## Shay-Shay Execution Pattern (desktop app flow)

Shay-Shay terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/shay-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/shay-op.sock"
