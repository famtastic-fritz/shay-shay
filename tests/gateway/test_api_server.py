...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_config_reasoning_effort(self, monkeypatch):
        ...
        monkeypatch.setattr("shay_cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestHealthEndpoint:
    ...
    async def test_health_returns_ok(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["platform"] == "shay-shay"
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_shay_agent(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == "shay-shay"
            assert data["data"][0]["owned_by"] == "shay"
    ...
    def test_resolve_model_name_default_profile(self):
        """Default profile falls back to 'shay-shay'."""
        with patch("shay_cli.profiles.get_active_profile_name", return_value="default"):
            assert APIServerAdapter._resolve_model_name("") == "shay-shay"

    def test_resolve_model_name_named_profile(self):
        """Named profile uses the profile name as model name."""
        with patch("shay_cli.profiles.get_active_profile_name", return_value="lucas"):
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "shay.api_server.capabilities"
            assert data["platform"] == "shay-shay"
            assert data["model"] == "shay-shay"
            ...
            assert data["features"]["session_continuity_header"] == "X-Shay-Shay-Session-Id"
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: shay.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``shay.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: shay.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: shay.tool.progress":
                    continue
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "shay-shay", "input": "What is 1+1?"},
                )
    ...
    async def test_previous_response_id_preserves_session(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "shay-shay", "input": "Hello"},
                )
            assert resp1.status == 200
    ...
    async def test_stream_true_returns_responses_sse(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "shay-shay", "input": "hi", "stream": True},
                )
                assert resp.status == 200
                assert "text/event-stream" in resp.headers.get("Content-Type", "")
                body = await resp.text()
    ...
    async def test_stream_emits_function_call_and_output_items(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "shay-shay", "input": "read the file", "stream": True},
                )
                assert resp.status == 200
                body = await resp.text()
    ...
    async def test_streamed_response_is_stored_for_get(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "shay-shay", "input": "store this", "stream": True},
                )
                body = await resp.text()
                response_id = None
                for line in body.splitlines():
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "shay-shay", "input": "What is 6*7?"},
                )
    ...
    async def test_truncation_with_partial_text_uses_length_finish_reason(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "tell me everything"}]},
                )
            assert resp.status == 200
            data = await resp.json()
            assert data["choices"][0]["finish_reason"] == "length"
            assert data["choices"][0]["message"]["content"] == "Here is part one of the answer"
            assert data["shay"]["partial"] is True
            assert data["shay"]["completed"] is False
            assert data["shay"]["error_code"] == "output_truncated"
            assert resp.headers.get("X-Shay-Shay-Completed") == "false"
            assert resp.headers.get("X-Shay-Shay-Partial") == "true"
    ...
    async def test_failure_with_no_text_returns_502_error_envelope(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "x"}]},
                )
            ...
            assert data["error"]["shay"]["partial"] is True
            assert data["error"]["shay"]["failed"] is True
            assert resp.headers.get("X-Shay-Shay-Completed") == "false"

    @pytest.mark.asyncio
    async def test_normal_completion_unchanged(self, adapter):
        """Sanity: a completed-True result still returns finish_reason='stop'
        and no shay extras (preserves the existing happy-path contract)."""
        mock_result = {
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            data = await resp.json()
            assert data["choices"][0]["finish_reason"] == "stop"
            assert data["choices"][0]["message"]["content"] == "All good."
            assert "shay" not in data
            assert "X-Shay-Shay-Completed" not in resp.headers
...
# ---------------------------------------------------------------------------
# X-Shay-Shay-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_new_session_response_includes_session_id_header(self, adapter):
        """Without X-Shay-Shay-Session-Id, a new session is created and returned in the header."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
            ...
            assert resp.status == 200
            assert resp.headers.get("X-Shay-Shay-Session-Id") is not None

    @pytest.mark.asyncio
    async def test_provided_session_id_is_used_and_echoed(self, auth_adapter):
        """When X-Shay-Shay-Session-Id is provided, it's passed to the agent and echoed in the response."""
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Shay-Shay-Session-Id": "my-session-123", "Authorization": "Bearer sk-secret"},
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "Continue"}]},
                )

            assert resp.status == 200
            assert resp.headers.get("X-Shay-Shay-Session-Id") == "my-session-123"
            call_kwargs = mock_run.call_args.kwargs
            assert call_kwargs["session_id"] == "my-session-123"

    @pytest.mark.asyncio
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-Shay-Shay-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Shay-Shay-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
    ...
    async def test_db_failure_falls_back_to_empty_history(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run, \
                 patch("shay_state.SessionDB", side_effect=Exception("DB unavailable")):
                mock_run.return_value = (mock_result, {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0})

                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Shay-Shay-Session-Id": "some-session", "Authorization": "Bearer sk-secret"},
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "Hi"}]},
                )
...
# ---------------------------------------------------------------------------
# X-Shay-Shay-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
    ...
    async def test_session_key_passed_to_agent_and_echoed(self, auth_adapter):
        """X-Shay-Shay-Session-Key reaches _run_agent as gateway_session_key and is echoed back."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Shay-Shay-Session-Key": "webui:user-42",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Shay-Shay-Session-Key") == "webui:user-42"
            call_kwargs = mock_run.call_args.kwargs
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Shay-Shay-Session-Key": "channel-abc",
                        "X-Shay-Shay-Session-Id": "transcript-xyz",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Shay-Shay-Session-Key") == "channel-abc"
            assert resp.headers.get("X-Shay-Shay-Session-Id") == "transcript-xyz"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_session_key_absent_yields_none(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            assert "X-Shay-Shay-Session-Key" not in resp.headers
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_session_key_rejected_without_api_key(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Shay-Shay-Session-Key": "whatever"},
                json={"model": "shay-shay", "messages": [{"role": "user", "content": "hi"}]},
            )
            assert resp.status == 403
    ...
    async def test_session_key_rejects_control_chars(self, auth_adapter):
        ...
        """
        mock_request = MagicMock()
        mock_request.headers = {"X-Shay-Shay-Session-Key": "bad\rvalue"}
    ...
    async def test_session_key_rejects_oversized(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Shay-Shay-Session-Key": "x" * 1000, "Authorization": "Bearer sk-secret"},
                json={"model": "shay-shay", "messages": [{"role": "user", "content": "hi"}]},
            )
            assert resp.status == 400
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Shay-Shay-Session-Key": "agent:main:webui:dm:user-7",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "shay-shay", "messages": [{"role": "user", "content": "hi"}]},
                )
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-Shay-Shay-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-Shay-Shay-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "shay-shay", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Shay-Shay-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-Shay-Shay-Session-Key"
