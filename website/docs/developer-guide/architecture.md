...
sidebar_position: 1
title: "Architecture"
description: "Shay-Shay internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Shay-Shay internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ shay_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ShayCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── shay_state.py           # SQLite session/state database with FTS5
├── shay_constants.py       # SHAY_HOME, profile-aware paths
...
│
├── shay_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `shay` subcommands (large file)
...
│   ├── skills_config.py      # shay skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # shay tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # shay gateway start/stop
│
...
```text
User input → ShayCLI.process_input()
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .shay.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.shay/plugins/` (user), `.shay/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `shay plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-shay-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Shay-Shay as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`shay -p <name>`) gets its own SHAY_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
