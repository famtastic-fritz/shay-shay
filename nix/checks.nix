...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      shay-shay = self'.packages.default;
      shayVenv = shay-shay.shayVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "shay-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${shayVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "shay-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "shay-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${shay-shay}/bin/shay || (echo "FAIL: shay binary missing"; exit 1)
          test -x ${shay-shay}/bin/shay-shay || (echo "FAIL: shay-shay binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${shay-shay}/bin/shay version 2>&1 | grep -qi "shay" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "shay-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in shay shay-shay shay-acp; do
            test -x ${shay-shay}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "shay-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking shay --help ==="
          ${shay-shay}/bin/shay --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${shay-shay}/bin/shay --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "shay-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${shay-shay}/share/shay-shay/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${shay-shay}/share/shay-shay/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "SHAY_BUNDLED_SKILLS" ${shay-shay}/bin/shay || \
            (echo "FAIL: SHAY_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: SHAY_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "shay-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${shay-shay}/share/shay-shay/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${shay-shay}/share/shay-shay/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "SHAY_BUNDLED_PLUGINS" ${shay-shay}/bin/shay || \
            (echo "FAIL: SHAY_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: SHAY_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "shay-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${shay-shay}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${shay-shay}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "SHAY_TUI_DIR" ${shay-shay}/bin/shay || \
            (echo "FAIL: SHAY_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: SHAY_TUI_DIR set in wrapper"
        ...
        # Verify SHAY_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        shay-node = pkgs.runCommand "shay-node-version" { } ''
          set -e
          echo "=== Checking SHAY_NODE in wrapper ==="
          grep -q "SHAY_NODE" ${shay-shay}/bin/shay || \
            (echo "FAIL: SHAY_NODE not set in wrapper"; exit 1)
          echo "PASS: SHAY_NODE present in wrapper"

          SHAY_NODE=$(sed -n "s/^export SHAY_NODE='\(.*\)'/\1/p" ${shay-shay}/bin/shay)
          test -x "$SHAY_NODE" || (echo "FAIL: SHAY_NODE=$SHAY_NODE not executable"; exit 1)
          echo "PASS: SHAY_NODE executable at $SHAY_NODE"

          NODE_MAJOR=$("$SHAY_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All SHAY_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify SHAY_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "shay-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(SHAY_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking SHAY_MANAGED guards ==="
          check_blocked "config set" ${shay-shay}/bin/shay config set model foo
          check_blocked "config edit" ${shay-shay}/bin/shay config edit
        ...
        extra-python-packages = let
          ...
          shayWithExtra = shay-shay.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "shay-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${shayWithExtra}/bin/shay || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${shayWithExtra}/bin/shay || \
          ...
          if grep -q "PYTHONPATH" ${shay-shay}/bin/shay; then
        ...
        extra-dependency-groups = let
          shayWithGroups = shay-shay.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "shay-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${shayWithGroups}"
          echo "venv: ${shayWithGroups.shayVenv}"
        ...
        in pkgs.runCommand "shay-config-roundtrip" {
          ...
          merge_and_load() {
            local shay_home="$1"
            export SHAY_HOME="$shay_home"
            ${configMergeScript} ${nixSettings} "$shay_home/config.yaml"
            ${shayVenv}/bin/python3 -c '
import json, sys
