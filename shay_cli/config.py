"""
Configuration management for Shay-Shay.

Config files are stored in ~/.shay/ for easy access:
- ~/.shay/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.shay/.env         - API keys and secrets

This module provides:
- shay config          - Show current configuration
- shay config edit     - Open config in editor
- shay config set      - Set a specific value
- shay config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.shay/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``shay logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  shay config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `shay plugins enable
    # observability/langfuse` or `shay tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "SHAY_LANGFUSE_ENV",
    "SHAY_LANGFUSE_RELEASE",
    "SHAY_LANGFUSE_SAMPLE_RATE",
    "SHAY_LANGFUSE_MAX_CHARS",
    "SHAY_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from shay_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("SHAY_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_shay_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Shay-Shay is running in package-manager-managed mode.

    Two signals: the SHAY_MANAGED env var (set by the systemd service),
    or a .managed marker file in SHAY_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "shay update"


def format_managed_message(action: str = "modify this Shay-Shay installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("SHAY_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Shay-Shay installation is managed by NixOS "
            f"(SHAY_MANAGED={env_hint}).\n"
            "Edit services.shay-shay.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Shay-Shay installation is managed by Homebrew "
            f"(SHAY_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade shay-shay"
        )

    return (
        f"Cannot {action}: this Shay-Shay installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Shay-Shay."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from SHAY_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, shay_bin
    or None if container mode is not active, we're already inside the
    container, or SHAY_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("SHAY_DEV") == "1":
        return None
    ...
    container_mode_file = get_shay_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "shay-shay")
    exec_user = info.get("exec_user", "shay")
    shay_bin = info.get("shay_bin", "/data/current-package/bin/shay")
...
# Re-export from shay_constants — canonical definition lives there.
from shay_constants import get_shay_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_shay_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_shay_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the shay group can
    share state with the gateway service.

    The mode can be overridden via the SHAY_HOME_MODE environment variable
    (e.g. SHAY_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse SHAY_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("SHAY_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Shay-Shay runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("SHAY_CONTAINER") or os.environ.get("SHAY_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set SHAY_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into SHAY_HOME if the user doesn't have one yet."""
...
def ensure_shay_home():
    """Ensure ~/.shay directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_shay_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_shay_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"SHAY_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Shay-Shay-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Shay-Shay auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Shay-Shay sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Shay-Shay
        ...
        # bundled Shay-Shay image, which drops to the `shay` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Shay-Shay sends a stable profile-scoped userId to Camofox
    ...
    # Opt in via ``shay chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.shay/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: shay sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Shay-Shay truncates the
    ...
    "auxiliary": {
        ...
        # Invoked by ``shay kanban specify`` (single id or --all). Set a
        ...
        # model; override via `shay model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `shay --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `shay -c` muscle memory.  Default off so existing
        # users aren't surprised.  SHAY_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/shay`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.shay/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.shay/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, Shay-Shay feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Shay-Shay auto-pauses the goal and
    ...
    # always goes to ~/.shay/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${SHAY_SKILL_DIR} and ${SHAY_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `shay curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.shay/skills/ into
        # ~/.shay/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `shay curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for shay-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Shay-Shay*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (SHAY_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.shay/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or SHAY_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via SHAY_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `shay -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     shay-shay's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.shay/logs/.
    ...
    # update model picker lists without shipping a shay-shay release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://shay-shay.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `shay model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.shay/state.db.
    ...
    "sessions": {
        ...
        # default of ``shay sessions prune``.
        "retention_days": 90,
    ...
    # ``shay update`` behaviour.
    "updates": {
        # Run a full ``shay backup``-style zip of SHAY_HOME before every
        # ``shay update``.  Backups land in ``<SHAY_HOME>/backups/`` and
        # can be restored with ``shay import <path>``.  Off by default —
        # on large SHAY_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'shay model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Shay-Shay auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @shay:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. SHAY_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #shay)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: shay-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'shay-shay' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Shay-Shay API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Shay-Shay API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Shay-Shay API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # SHAY_TOOL_PROGRESS and SHAY_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.shay.config`` entries, then checks
    ...
    try:
        ...
        # should never break `shay update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Shay-Shay versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``shay_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'shay setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Shay-Shay won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'shay doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("SHAY_HOME", "~/.shay")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("SHAY_TOOL_PROGRESS")
            old_mode = get_env_value("SHAY_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from SHAY_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from SHAY_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("SHAY_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from SHAY_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$SHAY_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_shay_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `shay plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `shay curator status` has no
    ...
    #   3. Creates `~/.shay/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_shay_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_shay_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: shay config set <key> <value>")
    ...
    # metadata.shay.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _shay_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.shay/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.shay/config.yaml.
    ...
    (which change ``SHAY_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_shay_home()
...
#   openai-codex (OAuth — shay auth) — OpenAI Codex
#   nous         (OAuth — shay auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.shay/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.shay/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Shay-Shay env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.shay/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.shay/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.shay/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.shay/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Shay-Shay — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Shay-Shay vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.shay/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Shay-Shay Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  shay config edit     # Edit config file", Colors.DIM))
    print(color("  shay config set <key> <value>", Colors.DIM))
    print(color("  shay setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: shay config set <key> <value>")
            print()
            print("Examples:")
            print("  shay config set model anthropic/claude-sonnet-4")
            print("  shay config set terminal.backend docker")
            print("  shay config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'shay config migrate' to add them")
        ...
        print("  shay config           Show current configuration")
        print("  shay config edit      Open config in editor")
        print("  shay config set <key> <value>   Set a config value")
        print("  shay config check     Check for missing/outdated config")
        print("  shay config migrate   Update config with new options")
        print("  shay config path      Show config file path")
        print("  shay config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``shay config`` UI so users
