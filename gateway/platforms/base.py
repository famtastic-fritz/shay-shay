...
# Audio file extensions Shay-Shay recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Shay-Shay' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Shay-Shay-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.shay/.env manually."
)
...
# Default location: {SHAY_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_shay_dir("cache/images", "image_cache")
...
AUDIO_CACHE_DIR = get_shay_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_shay_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_shay_dir("cache/documents", "document_cache")
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?shay\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+shay[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from shay_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          SHAY_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          SHAY_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          SHAY_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("SHAY_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("SHAY_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("SHAY_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _shay_run_generation is set on
