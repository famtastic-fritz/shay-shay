"""
Tests for mcp_serve — Shay-Shay MCP server.
...
def _isolate_shay_home(tmp_path, monkeypatch):
    """Redirect SHAY_HOME to a temp directory."""
    monkeypatch.setenv("SHAY_HOME", str(tmp_path))
    try:
        import shay_constants
        monkeypatch.setattr(shay_constants, "get_shay_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking shay_state schema."""
