...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"shay-shay[termux]"' in text
    assert '"shay-shay[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"shay-shay[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
