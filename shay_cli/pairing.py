...
Usage:
    shay pairing list              # Show all pending + approved users
    shay pairing approve <platform> <code>  # Approve a pairing code
    shay pairing revoke <platform> <user_id> # Revoke user access
    shay pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle shay pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: shay pairing {list|approve|revoke|clear-pending}")
        print("Run 'shay pairing --help' for details.")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.shay/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'shay pairing list' to see pending codes.\n")
