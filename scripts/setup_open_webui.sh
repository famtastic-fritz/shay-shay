...
# Bootstrap Open WebUI against Shay-Shay's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.shay/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-shay.sh
...
#   OPEN_WEBUI_NAME='Johnny Shay-Shay'
...
#   SHAY_API_PORT=8642
#   SHAY_API_HOST=127.0.0.1
#   SHAY_API_MODEL_NAME='Shay-Shay'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Shay-Shay WebUI}"
...
SHAY_ENV_FILE="${SHAY_ENV_FILE:-$HOME/.shay/.env}"
SHAY_API_PORT="${SHAY_API_PORT:-8642}"
SHAY_API_HOST="${SHAY_API_HOST:-127.0.0.1}"
SHAY_API_CONNECT_HOST="${SHAY_API_CONNECT_HOST:-127.0.0.1}"
SHAY_API_MODEL_NAME="${SHAY_API_MODEL_NAME:-Shay-Shay}"
SHAY_API_BASE_URL="http://${SHAY_API_CONNECT_HOST}:${SHAY_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-shay.sh"
LOG_DIR="$HOME/.shay/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$SHAY_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$SHAY_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.shay.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.shay</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.shay"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.shay"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-shay.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Shay-Shay
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-shay.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.shay/logs/openwebui.log
StandardError=append:%h/.shay/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-shay.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$SHAY_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Shay-Shay API server is configured...'
  upsert_env API_SERVER_ENABLED true "$SHAY_ENV_FILE"
  upsert_env API_SERVER_HOST "$SHAY_API_HOST" "$SHAY_ENV_FILE"
  upsert_env API_SERVER_PORT "$SHAY_API_PORT" "$SHAY_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$SHAY_API_MODEL_NAME" "$SHAY_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$SHAY_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Shay-Shay gateway so API server settings take effect...'
  shay gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${SHAY_API_CONNECT_HOST}:${SHAY_API_PORT}/health" >/dev/null; then
    log 'Shay-Shay API server did not answer on the first check. Trying to start gateway in the background...'
    nohup shay gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${SHAY_API_CONNECT_HOST}:${SHAY_API_PORT}/health" >/dev/null
  ...
  log "Shay-Shay API endpoint: ${SHAY_API_BASE_URL}"
