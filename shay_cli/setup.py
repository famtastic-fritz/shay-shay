"""
Interactive setup wizard for Shay-Shay.
...
from shay_cli.nous_subscription import get_nous_subscription_features
...
_DOCS_BASE = "https://shay-shay.nousresearch.com/docs"
...
# display_shay_home imported lazily at call sites (stale-module safety during shay update)
...
from shay_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Shay-Shay Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Shay-Shay using environment variables or config commands:")
    print_info("  shay config set model.provider custom")
    print_info("  shay config set model.base_url http://localhost:8080/v1")
    print_info("  shay config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'shay setup' in an interactive terminal to use the full wizard.")
    print()
...
def _curses_prompt_choice(question: str, choices: list, default: int = 0, description: str | None = None) -> int:
    ...
    from shay_cli.curses_ui import curses_radiolist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'shay setup')")


def _print_setup_summary(config: dict, shay_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'shay setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from shay_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'shay setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'shay setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'shay setup terminal'"))
    ...
    # Spotify (OAuth via shay auth spotify — check auth.json, not env vars)
    try:
        from shay_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'shay setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {shay_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('shay setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('shay setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('shay setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('shay setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('shay setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('shay config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('shay config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('shay config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('shay', Colors.GREEN)}              Start chatting")
    print(f"   {color('shay gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('shay doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `shay tools` and `shay setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``shay model``)
    ...
    provider added to ``shay model`` is automatically available here.
    ...
    # Delegate to the shared shay model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from shay_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: shay model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from shay_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Shay-Shay can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Shay-Shay can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'shay setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.shay/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'shay setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Shay-Shay runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("Requires the optional SDK: pip install 'shay-shay[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'shay-shay[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into SHAY_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("SHAY_MAX_ITERATIONS")
    ...
    print_info("  Run `shay setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # SHAY_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Shay-Shay delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `shay update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://shay-shay.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under SHAY_HOME, and print
    ...
    """
    try:
        from shay_cli.slack_cli import _build_full_manifest
        from shay_constants import get_shay_home

        manifest = _build_full_manifest(
            bot_name="Shay-Shay",
            bot_description="Your Shay-Shay on Slack",
        )
        target = Path(get_shay_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `shay slack manifest --write` anytime to refresh after "
            "Shay-Shay adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Shay-Shay delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Shay-Shay delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  shay config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Shay-Shay to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from shay_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://shay-shay.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://shay-shay.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  shay config edit")
    print_info("   Open config in your editor:  shay config edit")
...
def setup_gateway(config: dict):
    ...
    from shay_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Shay-Shay from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'shay setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     shay config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_shay_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  shay gateway")
        else:
            from shay_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   shay gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   shay gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `shay setup tools` and `shay tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``shay_cli.auth`` — lists every supported
    ...
    """
    try:
        from shay_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from shay_cli.config
    ...
    if section_key == "model":
        ...
        from shay_cli.gateway import _all_platforms, _platform_status
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_shay migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_shay", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Shay-Shay.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Shay-Shay to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Shay-Shay at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Shay-Shay at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Shay-Shay at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Shay-Shay at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Shay-Shay equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Shay-Shay config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Shay-Shay.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Shay-Shay's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(shay_home: Path) -> bool:
    ...
    print_info("Shay-Shay can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: shay claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: shay claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Shay-Shay configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Shay-Shay config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Shay-Shay (use shay claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      shay setup           — full or quick (auto-detected)
      shay setup model     — just model/provider
      shay setup tts       — just text-to-speech
      shay setup terminal  — just terminal backend
      shay setup gateway   — just messaging platforms
      shay setup tools     — just tool configuration
      shay setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Shay-Shay Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Shay-Shay Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Shay-Shay installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, shay_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Shay-Shay configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'shay setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(shay_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Shay-Shay?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, shay_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {shay_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'shay config edit'")
    ...
    _print_setup_summary(config, shay_home)
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch shay chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, shay_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'shay setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    shay setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  shay setup gateway")
    print()
...
def _run_quick_setup(config: dict, shay_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'shay setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Shay-Shay to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'shay setup gateway'.")
