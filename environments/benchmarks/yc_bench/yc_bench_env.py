...
The evaluate flow:
    ...
    2. evaluate()  -- Iterates over all runs sequentially through:
        a. rollout_and_score_eval()  -- Per-run agent loop
            ...
            - Runs ShayAgentLoop with terminal tool only
...
Requires: pip install shay-shay[yc-bench]
"""
...
from environments.agent_loop import ShayAgentLoop
...
class YCBenchEvalConfig(ShayAgentEnvConfig):
    """
    Configuration for the YC-Bench evaluation environment.

    Extends ShayAgentEnvConfig with YC-Bench-specific settings for
...
class YCBenchEvalEnv(ShayAgentBaseEnv):
    """
    ...
    a competing built-in agent loop). The ShayAgentLoop then drives the
    ...
    async def rollout_and_score_eval(self, eval_item: Dict[str, Any]) -> Dict:
        ...
        3. Runs ShayAgentLoop with terminal tool
        ...
        try:
            ...
            # LiteLLM), which would compete with our ShayAgentLoop.
