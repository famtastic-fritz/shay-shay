"""
Single source of truth for provider identity in Shay-Shay.
...
2. **Shay-Shay overlays** — transport type, auth patterns, aggregator flags,
...
# -- Shay-Shay overlay ----------------------------------------------------------
# Shay-Shay-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ShayOverlay:
    """Shay-Shay-specific provider metadata layered on top of models.dev."""
...
SHAY_OVERLAYS: Dict[str, ShayOverlay] = {
    "openrouter": ShayOverlay(
    ...
    "openai-codex": ShayOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": ShayOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="SHAY_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ShayOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": ShayOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ShayOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ShayOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ShayOverlay(
    ...
    "minimax-oauth": ShayOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ShayOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ShayOverlay(
        transport="openai_chat",
    ...
    "alibaba-coding-plan": ShayOverlay(
        transport="openai_chat",
    ...
    "opencode": ShayOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ShayOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "shay", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Shay-Shay overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Shay-Shay overlay
    ...
    overlay = SHAY_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + shay extra
    ...
    if overlay is not None:
        # Shay-Shay-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in SHAY_OVERLAYS
    if provider == "bedrock":
