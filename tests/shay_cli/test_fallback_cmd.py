"""Tests for `shay fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate SHAY_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".shay" / "config.yaml"
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Shay-Shay-4-Llama-3.1-405B"},
            ]
        }
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from shay_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestListCommand:
    ...
    def test_list_with_entries(self, isolated_home, capsys):
        _write_config(isolated_home, {
            ...
            "fallback_providers": [
                {"provider": "openrouter", "model": "anthropic/claude-sonnet-4.6"},
                {"provider": "nous", "model": "Shay-Shay-4"},
            ],
        })
        from shay_cli.fallback_cmd import cmd_fallback_list
        cmd_fallback_list(types.SimpleNamespace())
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("shay_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("shay_cli.main._require_tty"):
            from shay_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from shay_cli.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Shay-Shay-4")
        with patch("shay_cli.setup._curses_prompt_choice", return_value=1):
            from shay_cli.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from shay_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `shay fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "shay_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
