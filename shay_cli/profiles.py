"""
Profile management for multiple isolated Shay-Shay instances.

Each profile is a fully independent SHAY_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.shay/profiles/<name>/`` by default.

The "default" profile is ``~/.shay`` itself — backward compatible,
zero migration needed.

Usage::

    shay profile create coder          # fresh profile + bundled skills
    shay profile create coder --clone  # also copy config, .env, SOUL.md, skills
    shay profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    shay -p coder chat                 # or via flag
    shay profile use coder             # set as sticky default
    shay profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See shay_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# Marker file written by `shay profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `shay
...
# `shay skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Shay-Shay infrastructure directories that only the default profile
         (``~/.shay``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_shay_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.shay) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "shay-shay",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "shay", "default", "test", "tmp", "root", "sudo",
})

# Shay-Shay subcommands that cannot be used as profile names/aliases
_SHAY_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the shay root, NOT to the current SHAY_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where SHAY_HOME points outside
    ``~/.shay``, profiles live under ``SHAY_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_shay_home() / "profiles"


def _get_default_shay_home() -> Path:
    """Return the default (pre-profile) SHAY_HOME path.

    In standard deployments this is ``~/.shay``.
    In Docker/custom deployments where SHAY_HOME is outside ``~/.shay``
    (e.g. ``/opt/data``), returns SHAY_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_shay_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Shay-Shay installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its SHAY_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_shay_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, shay subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _SHAY_SUBCOMMANDS:
        return f"'{canon}' conflicts with a shay subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``shay profile list`` for an unrelated profile.
    """
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_shay_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``shay update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.shay)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.shay/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `shay update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`shay profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `shay update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches SHAY_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``shay profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "SHAY_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.shay).\n"
            "To remove everything, use: shay uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set SHAY_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("SHAY_HOME")
    try:
        os.environ["SHAY_HOME"] = str(profile_dir)
        from shay_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["SHAY_HOME"] = old_home
        elif "SHAY_HOME" in os.environ:
            del os.environ["SHAY_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.shay/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: shay profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from SHAY_HOME.

    Returns ``"default"`` if SHAY_HOME is not set or points to ``~/.shay``.
    Returns the profile name if SHAY_HOME points into ``~/.shay/profiles/<name>``.
    Returns ``"custom"`` if SHAY_HOME is set to an unrecognized path.
    """
    from shay_constants import get_shay_home
    shay_home = get_shay_home()
    resolved = shay_home.resolve()

    default_resolved = _get_default_shay_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.shay itself — its parent is ~/ and its
        # directory name is ".shay", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: shay profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.shay itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.shay). "
            "Specify a different name: shay profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="shay_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"shay.{old_name}"
    new_host = f"shay.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_shay_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for shay profile names."""
    return '''# Shay-Shay profile completion
# Add to ~/.bashrc: eval "$(shay completion bash)"

_shay_profiles() {
    local profiles_dir="$HOME/.shay/profiles"
...
_shay_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_shay_profiles)" -- "$cur"))
        return
    fi
...
complete -F _shay_completion shay
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for shay profile names."""
    return '''#compdef shay
# Shay-Shay profile completion
# Add to ~/.zshrc: eval "$(shay completion zsh)"

_shay() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.shay/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.shay/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a SHAY_HOME path string.

    Called early in the CLI entry point, before any shay modules
    are imported, to set the SHAY_HOME environment variable.
    """
