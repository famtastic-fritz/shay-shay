---
title: "Telephony — Give Shay-Shay phone capabilities without core tool changes"
sidebar_label: "Telephony"
description: "Give Shay-Shay phone capabilities without core tool changes"
---
...
Give Shay-Shay phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `shay skills install official/productivity/telephony` |
...
This optional skill gives Shay-Shay practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.shay/.env`
...
It does **not** turn Shay-Shay into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Shay-Shay memory
...
### 1) "I want Shay-Shay to own a real phone number"
Use **Twilio**.
...
- pairs well with Shay-Shay `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.shay/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
shay skills search telephony
shay skills install official/productivity/telephony
```
...
Then save credentials into Shay-Shay:
...
3. Buy it and save it into `~/.shay/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Shay-Shay calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Shay-Shay's existing `text_to_speech` support.

Use this when:
- you want the call to use Shay-Shay's configured TTS voice rather than Twilio `<Say>`
...
Recommended Shay-Shay TTS -> Twilio Play workflow:

1. Generate the audio with Shay-Shay `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Shay-Shay to create the message audio with `text_to_speech`
...
Important note:
- Shay-Shay TTS is great for prerecorded outbound messages
...
- Shay-Shay STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Shay-Shay memory.
...
- Do not store arbitrary third-party phone numbers in Shay-Shay memory.
...
3. persist that number to `~/.shay/.env`
