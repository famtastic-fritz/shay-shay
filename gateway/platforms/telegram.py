...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("SHAY_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = self._env_float_clamped(
            "SHAY_TELEGRAM_TEXT_BATCH_DELAY_SECONDS",
            0.3,
            min_value=0.08,
            max_value=2.0,
        )
        self._text_batch_split_delay_seconds = self._env_float_clamped(
            "SHAY_TELEGRAM_TEXT_BATCH_SPLIT_DELAY_SECONDS",
            1.0,
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Shay-Shay-created private-chat topic lanes are marked with
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "(possibly OpenClaw or another Shay-Shay instance). "
            "Shay-Shay stopped Telegram polling after %d retries. "
            "Only one poller can run per token — stop the other process "
            "and restart with 'shay start'."
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            request_kwargs = {
                "connection_pool_size": _env_int("SHAY_TELEGRAM_HTTP_POOL_SIZE", 512),
                "pool_timeout": _env_float("SHAY_TELEGRAM_HTTP_POOL_TIMEOUT", 8.0),
                "connect_timeout": _env_float("SHAY_TELEGRAM_HTTP_CONNECT_TIMEOUT", 10.0),
                "read_timeout": _env_float("SHAY_TELEGRAM_HTTP_READ_TIMEOUT", 20.0),
                "write_timeout": _env_float("SHAY_TELEGRAM_HTTP_WRITE_TIMEOUT", 20.0),
            }

            disable_fallback = (os.getenv("SHAY_TELEGRAM_DISABLE_FALLBACK_IPS", "").strip().lower() in {"1", "true", "yes", "on"})
            ...
            try:
                ...
                from shay_cli.commands import telegram_menu_commands
    ...
    async def send_typing(self, chat_id: str, metadata: Optional[Dict[str, Any]] = None) -> None:
        ...
        if self._bot:
            try:
                ...
                # Skip the Bot API call entirely for Shay-Shay-created DM topic
    ...
    def _message_mentions_bot(self, message: Message) -> bool:
        ...
        # raw substring matches like "foo@shay_bot.example" are not mentions
