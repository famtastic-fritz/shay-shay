# nix/nixosModules.nix — NixOS module for shay-shay
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: shay runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $SHAY_HOME/.env
# and read by shay at startup — no container recreation needed for env changes.
#
# Tool resolution: the shay wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    ...
    shay-shay = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-shay-shay)
    deepConfigType = lib.types.mkOptionType {
      name = "shay-config-attrs";
      description = "Shay-Shay YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "shay-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "shay-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'SHAY_DOC_EOF'\n${value}\nSHAY_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "shay-shay";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/shay";
    ...
    # shay user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "shay-container-entrypoint" ''
      set -eu

      SHAY_UID="''${SHAY_UID:?SHAY_UID must be set}"
      SHAY_GID="''${SHAY_GID:?SHAY_GID must be set}"

      # ── Group: ensure a group with GID=$SHAY_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$SHAY_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="shay"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$SHAY_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$SHAY_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$SHAY_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$SHAY_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="shay"
        TARGET_HOME="/home/shay"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$SHAY_UID" -g "$SHAY_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$SHAY_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$SHAY_UID:$SHAY_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure SHAY_HOME is owned by the target user.
      ...
      if [ -n "''${SHAY_HOME:-}" ] && [ -d "$SHAY_HOME" ]; then
        find "$SHAY_HOME" \! -user "$SHAY_UID" -exec chown "$SHAY_UID:$SHAY_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/shay-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/shay-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/shay ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/shay
        chmod 0440 /etc/sudoers.d/shay
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$SHAY_UID" --regid="$SHAY_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $SHAY_HOME/.env.
    ...
    # Default: /var/lib/shay/workspace → /data/workspace.
  ...
  in {
    options.services.shay-shay = with lib; {
      enable = mkEnableOption "Shay-Shay gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = shay-shay;
        description = "The shay-shay package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/shay";
        description = "State directory. Contains .shay/ subdir (SHAY_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Shay-Shay config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $SHAY_HOME/.env at activation time.
          Shay-Shay reads this file on every startup via load_shay_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $SHAY_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $SHAY_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `shay gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the shay user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the shay plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Shay-Shay discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          shay_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as shay (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in shay-shay's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the shay CLI to environment.systemPackages and export
          SHAY_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.shay symlink to the service
            stateDir. These users are automatically added to the shay group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.shay-shay.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the shay CLI to system PATH and export SHAY_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.shay/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.SHAY_HOME = "${cfg.stateDir}/.shay";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.shay-shay.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the shay user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.shay-shay: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed shay
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure shay is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.shay        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shay/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shay/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shay/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shay/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.shay/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."shay-shay-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.shay
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.shay ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.shay ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.shay -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.shay/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.shay/$_subdir"
            chmod 2770 "${cfg.stateDir}/.shay/$_subdir"
            find "${cfg.stateDir}/.shay/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.shay/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.shay/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shay/config.yaml
            chmod 0640 ${cfg.stateDir}/.shay/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.shay/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shay/.managed
          chmod 0644 ${cfg.stateDir}/.shay/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.shay/.container-mode <<'SHAY_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    shay_bin=${containerDataDir}/current-package/bin/shay
    SHAY_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shay/.container-mode
            chmod 0644 ${cfg.stateDir}/.shay/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.shay/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.shay";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.shay" ]; then
                  rm -f "${symlinkPath}"
                  echo "shay-shay: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.shay -> stateDir/.shay for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.shay";
                target = "${cfg.stateDir}/.shay";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "shay-shay: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.shay/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.shay/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.shay/auth.json
              fi
            ''}
          ''}
          ...
          # Shay-Shay reads $SHAY_HOME/.env at startup via load_shay_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.shay/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'SHAY_NIX_ENV_EOF'
    ${envFileContent}
    SHAY_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.shay/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.shay/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.shay/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.shay-shay = {
          ...
          serviceConfig = {
            ...
            # $SHAY_HOME/.env by the activation script. load_shay_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/shay"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the shay group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.shay-shay = {
          description = "Shay-Shay Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              SHAY_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              SHAY_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env SHAY_UID="$SHAY_UID" \
                --env SHAY_GID="$SHAY_GID" \
                --env SHAY_HOME=${containerDataDir}/.shay \
                --env SHAY_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/shay gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
