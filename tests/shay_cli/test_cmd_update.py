...
from shay_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        from shay_cli import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "shay_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "shay_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("shay_cli.config.check_config_version", return_value=(1, 2)), patch(
            "shay_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("shay_cli.main.sys") as mock_sys:
            ...
            from shay_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.shay"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.shay/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.shay/profiles/work"))
        ...
        with (
            patch("shay_cli.profiles.list_profiles", return_value=all_profiles),
            patch("shay_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("shay_cli.profiles.list_profiles", return_value=[default_p]),
            patch("shay_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
