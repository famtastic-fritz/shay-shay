...
Plugins built on Shay-Shay often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Shay-Shay
...
:class:`~shay_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Shay-Shay' main config
...
handles every provider, fallback chain, and per-task override Shay-Shay
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`shay_cli.plugins.PluginContext`
