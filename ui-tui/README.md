...
React + Ink terminal UI for Shay-Shay. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `SHAY_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.shay/.shay_history` or under `SHAY_HOME`.
...
```text
ui-tui/
  packages/shay-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      shay-ink.d.ts    type declarations for @shay/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ShayCLI subprocess for slash commands
```
