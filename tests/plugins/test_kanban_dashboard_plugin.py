...
def _load_plugin_router():
    ...
    spec = importlib.util.spec_from_file_location(
        "shay_dashboard_plugin_kanban_test", plugin_file,
    )
...
def test_board_auto_initializes_missing_db(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("SHAY_HOME", str(home))
    monkeypatch.delenv("SHAY_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("SHAY_KANBAN_DB", raising=False)
...
def test_ws_events_rejects_when_token_required(tmp_path, monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "shay_cli.web_server", stub)
    monkeypatch.setattr(shay_cli, "web_server", stub, raising=False)
...
def test_create_task_includes_warning_when_no_dispatcher(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "shay_cli.kanban._check_dispatcher_presence",
        lambda: (False, "No gateway is running — start `shay gateway start`."),
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_create_task_probe_error_does_not_break_create(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "shay_cli.kanban._check_dispatcher_presence", _raise,
    )
    r = client.post(
        "/api/plugins/kanban/tasks",
...
def test_board_surfaces_warnings_field_for_hallucinated_completions(client):
    ...
    raw event kind — see shay_cli.kanban_diagnostics for the rule
    that produces it.
    """
    conn = kb.connect()
    try:
...
def test_dashboard_bulk_actions_include_reclaim_first():
    ...
    assert "shay-kanban-bulk-reclaim-first" in dist
    assert '"→ todo"' in dist
...
def test_dashboard_failed_card_highlight_class_exists():
    ...
    assert "shay-kanban-card--failed" in js
    assert "shay-kanban-card--failed" in css
