...
Shay-Shay can drive your Mac's desktop — clicking, typing, scrolling,
...
```
shay computer-use install
```

This fetches and runs the upstream cua-driver installer:
`curl -fsSL https://raw.githubusercontent.com/trycua/cua/main/libs/cua-driver/scripts/install.sh`.
Use `shay computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `shay tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
3. Grant macOS permissions when prompted:
   - **System Settings → Privacy & Security → Accessibility** → allow the
     terminal (or Shay-Shay app).
   - **System Settings → Privacy & Security → Screen Recording** → allow
     the same.
4. Start a session with the toolset enabled:
   ```
   shay -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.shay/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Shay-Shay refreshes the binary in two
places so you don't get stuck on a stale release:

- **`shay update`** — when you update Shay-Shay itself, if `cua-driver` is
...
- **`shay computer-use install --upgrade`** — manual force-refresh.
...
`shay computer-use status` shows the installed version next to the
binary path.
...
Shay-Shay applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.shay/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Shay-Shay applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `SHAY_CUA_DRIVER_VERSION`
...
```
SHAY_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
SHAY_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
SHAY_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`shay computer-use install` to fetch the cua-driver binary, or run
`shay tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/shay-shay/blob/main/skills/apple/macos-computer-use/SKILL.md)
