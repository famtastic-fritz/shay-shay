...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or shay -c.",
    ...
    "Ctrl+Z suspends Shay-Shay to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "shay -c resumes your most recent CLI session. shay -c \"project name\" resumes by title.",
    "shay -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "shay -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "shay chat -t web,terminal enables only specific toolsets for a focused session.",
    "shay chat -s github-pr-workflow preloads a skill at launch.",
    "shay chat -q \"query\" runs a single non-interactive query and exits.",
    "shay chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "shay chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "shay --yolo bypasses all dangerous command approval prompts for the entire session.",
    "shay chat --source telegram tags the session for filtering in shay sessions list.",
    "shay -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "shay doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "shay dump outputs a compact setup summary — great for bug reports.",
    "shay config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "shay config edit opens config.yaml in your default editor.",
    "shay config check scans for missing or stale configuration options.",
    "shay sessions browse opens an interactive session picker with search.",
    "shay sessions stats shows session counts by platform and database size.",
    "shay sessions prune --older-than 30 cleans up old sessions.",
    "shay skills search react --source skills-sh searches the skills.sh public directory.",
    "shay skills check scans installed hub skills for upstream updates.",
    "shay skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "shay skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "shay mcp add github --command npx adds MCP servers from the command line.",
    "shay mcp serve runs Shay-Shay itself as an MCP server for other agents.",
    "shay auth add lets you add multiple API keys for credential pool rotation.",
    "shay completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "shay logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "shay backup creates a zip backup of your entire Shay-Shay home directory.",
    "shay profile create coder creates an isolated profile that becomes its own command.",
    "shay profile create work --clone copies your current config and keys to a new profile.",
    "shay update syncs new bundled skills to ALL profiles automatically.",
    "shay gateway install sets up Shay-Shay as a system service (systemd/launchd).",
    "shay memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "shay webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: shay tools disables unused tools, shay skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "shay models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as shay -w).",
    ...
    "SHAY_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Shay-Shay tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'shay profile create coder' creates the 'coder' command.",
    "shay profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Shay-Shay prints a resume command with session ID and stats.",
    "shay sessions export backup.jsonl exports all sessions for backup or analysis.",
    "shay -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "shay skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .shay/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: shay cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Shay-Shay runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "shay gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Shay-Shay via a local macOS server.",
    ...
    "Context files (.shay.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.shay/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Shay-Shay loads project context from .shay.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — shay tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.shay/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "shay chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "shay plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "shay acp runs Shay-Shay as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "SHAY_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "shay login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "shay status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.shay/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Shay-Shay your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.shay/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Shay-Shay auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.shay/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at SHAY_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "SHAY_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in SHAY_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.shay/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'shay profile create ls' would be rejected.",
    "shay profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "shay claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "SHAY_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via shay tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.shay/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.shay/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Shay-Shay auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Shay-Shay config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Shay-Shay is working.',
    ...
    '/reload reloads ~/.shay/.env into the running session — pick up new API keys without restarting.',
    ...
    "SHAY_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.shay/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.shay/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'shay curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "shay curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'shay curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'shay auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'SHAY_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "SHAY_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'shay dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'shay dashboard --tui embeds the full Shay-Shay TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.shay/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.shay/dashboard-plugins/ — no npm build required.',
    ...
    'SHAY_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'SHAY_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'SHAY_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'SHAY_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    '/reload-skills re-scans ~/.shay/skills/ so drop-in skills appear without restarting the session.',
    ...
    'shay -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'shay chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'shay chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'shay chat --ignore-user-config skips ~/.shay/config.yaml — reproducible bug reports and CI runs.',
    "shay chat --source tool tags programmatic chats so they don't clutter shay sessions list.",
    'shay dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'shay sessions rename <ID> "new title" renames any past session; shay sessions delete <ID> removes one.',
    'shay import restores a session export or profile archive produced by sessions export or profile export.',
    'shay fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'shay pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'shay setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'shay status --deep runs the full health sweep across every component; plain shay status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'SHAY_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'SHAY_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.shay/plugins/ — trust-gated by design.',
    "SHAY_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'SHAY_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'SHAY_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'SHAY_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'SHAY_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'SHAY_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'SHAY_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'SHAY_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'SHAY_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'SHAY_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'SHAY_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Shay-Shay block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from shay sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.shay/dashboard-plugins/.',
]
