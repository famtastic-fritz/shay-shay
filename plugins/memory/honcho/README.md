...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/shay>
...
```bash
shay honcho setup    # full interactive wizard (cloud or local)
shay memory setup    # generic picker, also works
```

Or manually:
```bash
shay config set memory.provider honcho
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$SHAY_HOME/honcho.json` | Profile-local (isolated Shay-Shay instances) |
| 2 | `~/.shay/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Shay-Shay profile: `shay` (default) or `shay.<profile>`.
...
| 4 | `per-session` strategy | Shay-Shay session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`shay-shay`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`shay`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-shay-shay`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening shay in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Shay-Shay session ID (timestamp + hex). Every `shay` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Shay-Shay profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`shay`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Shay-Shay profile: `shay` (default) or `shay.<profile>` (e.g. `shay -p coder` → host key `shay.coder`).
...
| `SHAY_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `shay honcho setup` | Full interactive setup wizard |
| `shay honcho status` | Show resolved config for active profile |
| `shay honcho enable` / `disable` | Toggle Honcho for active profile |
| `shay honcho mode <mode>` | Change recall or observation mode |
| `shay honcho peer --user <name>` | Update user peer name |
| `shay honcho peer --ai <name>` | Update AI peer name |
| `shay honcho tokens --context <N>` | Set context token budget |
| `shay honcho tokens --dialectic <N>` | Set dialectic max chars |
| `shay honcho map <name>` | Map current directory to a session name |
| `shay honcho sync` | Create host blocks for all Shay-Shay profiles |
