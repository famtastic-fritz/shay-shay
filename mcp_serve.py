"""
Shay-Shay MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Shay-Shay-specific extra)

Usage:
    shay mcp serve
    shay mcp serve --verbose
...
logger = logging.getLogger("shay.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using SHAY_HOME."""
    try:
        from shay_constants import get_shay_home
        return get_shay_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("SHAY_HOME", Path.home() / ".shay")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from shay_constants import get_shay_home
        directory_file = get_shay_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("SHAY_HOME", Path.home() / ".shay")
...
class EventBridge:
    ...
    This is the Shay-Shay equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("SHAY_HOME", Path.home() / ".shay")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Shay-Shay MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "shay",
        instructions=(
            "Shay-Shay messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Shay-Shay MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
