"""Tests for Codex auth — tokens stored in Shay-Shay auth store (~/.shay/auth.json)."""
...
def _setup_shay_auth(shay_home: Path, *, access_token: str = "access", refresh_token: str = "refresh"):
    """Write Codex tokens into the Shay-Shay auth store."""
...
def test_read_codex_tokens_missing(tmp_path, monkeypatch):
    ...
    shay_home.mkdir(parents=True, exist_ok=True)
    # Empty auth store
    (shay_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
    monkeypatch.setenv("SHAY_HOME", str(shay_home))
...
def test_resolve_codex_runtime_credentials_missing_access_token(tmp_path, monkeypatch):
    shay_home = tmp_path / "shay"
    _setup_shay_auth(shay_home, access_token="")
    monkeypatch.setenv("SHAY_HOME", str(shay_home))
...
def test_resolve_codex_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    _setup_shay_auth(shay_home, access_token=expiring_token, refresh_token="refresh-old")
    monkeypatch.setenv("SHAY_HOME", str(shay_home))
    ...
    monkeypatch.setattr("shay_cli.auth._refresh_codex_auth_tokens", _fake_refresh)
...
def test_resolve_codex_runtime_credentials_force_refresh(tmp_path, monkeypatch):
    shay_home = tmp_path / "shay"
    _setup_shay_auth(shay_home, access_token="access-current", refresh_token="refresh-old")
    monkeypatch.setenv("SHAY_HOME", str(shay_home))
...
def test_codex_tokens_not_written_to_shared_file(tmp_path, monkeypatch):
    """Verify _save_codex_tokens writes only to Shay-Shay auth store, not ~/.codex/."""
    shay_home = tmp_path / "shay"
    codex_home = tmp_path / "codex-cli"
    shay_home.mkdir(parents=True, exist_ok=True)
    codex_home.mkdir(parents=True, exist_ok=True)
    ...
    _save_codex_tokens({"access_token": "shay-at", "refresh_token": "shay-rt"})

    # ~/.codex/auth.json should NOT exist — _save_codex_tokens only touches Shay-Shay store
    assert not (codex_home / "auth.json").exists()

    # Shay-Shay auth store should have the tokens
    data = _read_codex_tokens()
    assert data["tokens"]["access_token"] == "shay-at"


def test_resolve_returns_shay_auth_store_source(tmp_path, monkeypatch):
    shay_home = tmp_path / "shay"
    _setup_shay_auth(shay_home)
    monkeypatch.setenv("SHAY_HOME", str(shay_home))

    creds = resolve_codex_runtime_credentials()
    assert creds["source"] == "shay-auth-store"
...
def _patch_httpx(monkeypatch, response):
    ...
    monkeypatch.setattr("shay_cli.auth.httpx.Client", _factory)
...
def test_login_openai_codex_force_new_login_skips_existing_reuse_prompt(monkeypatch):
    ...
    monkeypatch.setattr(
        "shay_cli.auth.resolve_codex_runtime_credentials",
        lambda: {"base_url": DEFAULT_CODEX_BASE_URL},
    )
    monkeypatch.setattr(
        "shay_cli.auth._import_codex_cli_tokens",
    ...
    monkeypatch.setattr("shay_cli.auth._save_codex_tokens", _fake_save)
    monkeypatch.setattr("shay_cli.auth._update_config_for_provider", lambda *args, **kwargs: "/tmp/config.yaml")
    monkeypatch.setattr(
        "builtins.input",
