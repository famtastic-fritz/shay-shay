...
sidebar_position: 4
title: "Slack"
description: "Set up Shay-Shay as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Shay-Shay to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Shay-Shay instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Shay-Shay uses the modern
...
The fastest path is to paste a manifest Shay-Shay generates for you. It
...
### Option A: From a Shay-Shay-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   shay slack manifest --write
   ```
   This writes `~/.shay/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "Shay-Shay") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Shay-Shay can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `shay-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Shay-Shay configuration issue.
:::
...
Shay-Shay uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
shay gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
shay gateway              # Foreground
shay gateway install      # Install as a user service
sudo shay gateway install --system   # Linux only: boot-time system service
```
...
Every Shay-Shay command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Shay-Shay command with its description.

Under the hood: Shay-Shay ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NousResearch/shay-shay/blob/main/shay_cli/commands.py)
...
When Shay-Shay adds new commands (e.g. after `shay update`), regenerate
...
3. Paste the new contents of `~/.shay/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/shay <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/shay btw run the tests` — Shay-Shay routes it the same way as `/btw
run the tests`. Free-form questions also work: `/shay what's the
...
```bash
shay slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Shay-Shay behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Shay-Shay what time is it?`). In channels, Shay-Shay replies in a thread attached to that message. |
| **Threads** | If you @mention Shay-Shay inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.shay/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Shay-Shay in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Shay-Shay will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Shay-Shay will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Shay-Shay`).

---

## Multi-Workspace Support

Shay-Shay can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Shay-Shay also loads tokens from an **OAuth token file** at:

```
~/.shay/slack_tokens.json
```
...
- When a message arrives, Shay-Shay uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Shay-Shay` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Shay-Shay` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Shay-Shay now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Shay-Shay`)
...
- Tokens should be stored in `~/.shay/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Shay-Shay config directory
