...
When ``SHAY_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.shay/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.shay            (has its own auth.json fixture)
    * Profile     -> tmp_path/.shay/profiles/coder   (active, SHAY_HOME points here)
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (SHAY_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.shay as the "real home" — which is NOT equal
    # to the SHAY_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    shay_home = tmp_path / "classic"
    shay_home.mkdir()
    monkeypatch.setenv("SHAY_HOME", str(shay_home))

    _write(shay_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from shay_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: SHAY_HOME is set to a custom path that is NOT under
    # ~/.shay/profiles/ — get_default_shay_root() returns SHAY_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from shay_cli.auth import read_credential_pool, write_credential_pool
