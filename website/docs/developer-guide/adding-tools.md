...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Shay-Shay — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Shay-Shay tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Shay-Shay core, use the plugin route instead:

- [Plugins](/docs/user-guide/features/plugins)
- [Build a Shay-Shay Plugin](/docs/guides/build-a-shay-plugin)
...
2. **`toolsets.py`** — add tool name to `_SHAY_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `shay_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `shay_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `shay chat -q "Use the weather tool for London"`
