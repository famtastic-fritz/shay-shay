"""Tests for shay_constants module."""
...
from shay_constants import (
    ...
    get_default_shay_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultShayRoot:
    """Tests for get_default_shay_root() — Docker/custom deployment awareness."""

    def test_no_shay_home_returns_native(self, tmp_path, monkeypatch):
        """When SHAY_HOME is not set, returns ~/.shay."""
        monkeypatch.delenv("SHAY_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_shay_root() == tmp_path / ".shay"

    def test_shay_home_is_native(self, tmp_path, monkeypatch):
        """When SHAY_HOME = ~/.shay, returns ~/.shay."""
        native = tmp_path / ".shay"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(native))
        assert get_default_shay_root() == native

    def test_shay_home_is_profile(self, tmp_path, monkeypatch):
        """When SHAY_HOME is a profile under ~/.shay, returns ~/.shay."""
        native = tmp_path / ".shay"
    ...
    def test_shay_home_is_docker(self, tmp_path, monkeypatch):
        """When SHAY_HOME points outside ~/.shay (Docker), returns SHAY_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(docker_home))
        assert get_default_shay_root() == docker_home

    def test_shay_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any SHAY_HOME outside ~/.shay is treated as the root."""
        custom = tmp_path / "my-shay-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(custom))
        assert get_default_shay_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (SHAY_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("SHAY_HOME", str(profile))
        assert get_default_shay_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(shay_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(shay_constants, "_container_detected", True)
        assert is_container() is True
