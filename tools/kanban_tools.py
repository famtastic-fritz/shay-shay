...
running under the dispatcher (env var ``SHAY_KANBAN_TASK`` set). A
normal ``shay chat`` session sees **zero** kanban tools in its schema.

Why tools instead of just shelling out to ``shay kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``shay kanban complete …``
   inside the container, where ``shay`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.shay/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``shay kanban …``), the dashboard
(``shay dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``SHAY_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``shay chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("SHAY_KANBAN_TASK")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``SHAY_KANBAN_TASK`` set
    ...
    ``SHAY_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set SHAY_KANBAN_TASK in the env)"
        )
    try:
        kb, conn = _connect()
        try:
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # SHAY_KANBAN_CLAIM_LOCK in the worker env at spawn time
            # (see _default_spawn in kanban_db.py); falling back to the
            # default _claimer_id() covers locally-driven workers that
            # never went through the dispatcher path.
            claim_lock = os.environ.get("SHAY_KANBAN_CLAIM_LOCK")
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``shay-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("SHAY_PROFILE") or "worker"
    try:
        kb, conn = _connect()
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("SHAY_TENANT")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                skills=skills,
                created_by=os.environ.get("SHAY_PROFILE") or "worker",
            )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to SHAY_KANBAN_TASK from the env "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to SHAY_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
