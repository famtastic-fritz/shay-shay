"""Windows UTF-8 bootstrap for Shay-Shay entry points.
...
should be imported at the very top of every Shay-Shay entry point
(``shay``, ``shay-shay``, ``shay-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import shay_bootstrap``
# (or ``from shay_bootstrap import apply_windows_utf8_bootstrap``) at
