"""`shay checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.shay/checkpoints/``.  Actions:

    shay checkpoints               # same as `status`
    shay checkpoints status        # total size, project count, breakdown
    shay checkpoints list          # per-project checkpoint counts + workdir
    shay checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    shay checkpoints clear [-f]    # nuke the entire base (asks first)
    shay checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    shay checkpoints
    shay checkpoints prune --retention-days 3 --max-size-mb 200
    shay checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: shay checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``shay checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `shay checkpoints` → status
