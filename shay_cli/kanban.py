"""CLI for the Shay-Shay Kanban board — ``shay kanban …`` subcommand.

Exposes the full 15-verb surface documented in the design spec
(``docs/shay-kanban-v1-spec.pdf``).  All DB work is delegated to
...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    - ``running=True``: a gateway is alive for this SHAY_HOME and its
    ...
    Used by ``shay kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`shay kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Shay-Shay profiles. "
            "Tasks are claimed atomically, can depend on other tasks, and "
            "are executed by a named profile in an isolated workspace. "
            "See https://shay-shay.nousresearch.com/docs/user-guide/features/kanban "
            "or docs/shay-kanban-v1-spec.pdf for the full design."
        ),
    )
    # --- global --board flag ---
    # Applies to every subcommand below. When set, scopes all reads and
    # writes to that board's DB. When omitted, resolves via the
    # SHAY_KANBAN_BOARD env var, then the persisted current-board
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `shay kanban boards switch <slug>` or the "
            "SHAY_KANBAN_BOARD env var). Use `shay kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_list.add_argument("--mine", action="store_true",
                        help="Filter by $SHAY_PROFILE as assignee")
    ...
    p_comment.add_argument("--author", default=None,
                           help="Author name (default: $SHAY_PROFILE or 'user')")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `shay gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.shay/profiles/ and current assignees on the board)",
    )
    ...
    p_specify.add_argument(
        ...
        help="Author name recorded on the audit comment "
             "(default: $SHAY_PROFILE or 'specifier')",
    )
    p_specify.add_argument(
        "--json",
        action="store_true",
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``shay kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: shay kanban <action> [options]\n"
                "Run 'shay kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0

    # `--board <slug>` applies to every subcommand below by way of an
    # env-var pin for the duration of this call. Using SHAY_KANBAN_BOARD
    ...
    prev_board_env = os.environ.get("SHAY_KANBAN_BOARD")
    ...
    def _restore_board_env() -> None:
        ...
        if prev_board_env is None:
            os.environ.pop("SHAY_KANBAN_BOARD", None)
        else:
            os.environ["SHAY_KANBAN_BOARD"] = prev_board_env
    if board_override:
        try:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `shay kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    # SHAY_HOME. Previously only `init` and `daemon` triggered
...
def _profile_author() -> str:
    ...
    for env in ("SHAY_PROFILE_NAME", "SHAY_PROFILE"):
...
# ---------------------------------------------------------------------------
# Boards management (shay kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``shay kanban boards <action>``.
    ...
    DB, so a fresh SHAY_HOME that has never called ``kanban init``
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `shay kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `shay kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `shay kanban boards switch {meta['slug']}` to make it current.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.shay/profiles/.")
        print("Create one with `shay -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  shay gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `shay -p <name> setup`)")
        return 0
    # Header
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    # comments / runs.
    from shay_cli import kanban_diagnostics as kd
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``shay kanban show <id>``
...
def _worker_run_id_for(task_id: str) -> Optional[int]:
    if os.environ.get("SHAY_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("SHAY_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "shay kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    shay gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`shay kanban list --status ready` / "
                    f"`shay kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Shay-Shay profile (i.e. one the
...
def _cmd_specify(args: argparse.Namespace) -> int:
    ...
    from shay_cli import kanban_specify as spec
