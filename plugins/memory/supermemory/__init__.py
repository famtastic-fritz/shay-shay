...
def _load_supermemory_config(shay_home: str) -> dict:
    config = _default_config()
    config_path = Path(shay_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, shay_home: str) -> None:
    config_path = Path(shay_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `shay memory setup`.
        # All other options are documented for $SHAY_HOME/supermemory.json
    ...
    def save_config(self, values, shay_home):
        ...
        _save_supermemory_config(sanitized, shay_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from shay_constants import get_shay_home
        self._shay_home = kwargs.get("shay_home") or str(get_shay_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._shay_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "shay", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "shay_memory", "target": target, "type": "explicit_memory"},
