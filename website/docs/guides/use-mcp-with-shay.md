---
sidebar_position: 6
title: "Use MCP with Shay-Shay"
description: "A practical guide to connecting MCP servers to Shay-Shay, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Shay-Shay

This guide shows how to actually use MCP with Shay-Shay in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Shay-Shay tool
- you want Shay-Shay to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Shay-Shay to internal APIs, databases, or company systems without modifying Shay-Shay core

Do not use MCP when:
- a built-in Shay-Shay tool already solves the job well
...
- Shay-Shay remains the agent
- MCP servers contribute tools
- Shay-Shay discovers those tools at startup or reload time
...
If you installed Shay-Shay with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Shay-Shay banner/status should show MCP integration when configured
- ask Shay-Shay what tools it has available
...
## WSL2: bridge Shay-Shay in WSL to Windows Chrome

This is the practical setup when:

- Shay-Shay runs inside WSL2
...
In this setup, Shay-Shay does **not** connect to Chrome directly. Instead:

- Shay-Shay runs in WSL
- Shay-Shay starts a local stdio MCP server
...
```text
Shay-Shay (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Shay-Shay stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Shay-Shay core browser transport
...
```bash
shay mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
shay mcp test chrome-devtools-win
```

Then start a fresh Shay-Shay session or run:
...
Once loaded, Shay-Shay can use the MCP-prefixed browser tools directly. For example:
...
If Shay-Shay runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Shay-Shay from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Shay-Shay from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Shay-Shay:
...
2. Shay-Shay-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Shay-Shay will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Shay-Shay to reason over a bounded workspace.
...
Now Shay-Shay can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Shay-Shay core.
...
Because Shay-Shay now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
