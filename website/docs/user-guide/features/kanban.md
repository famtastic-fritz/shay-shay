...
description: "Durable SQLite-backed task board for coordinating multiple Shay-Shay profiles"
---
...
Shay-Shay Kanban is a durable task board, shared across all your Shay-Shay profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.shay/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.shay/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `shay kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `shay kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/shay-kanban-v1-spec.pdf` in the repository.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.shay/kanban/workspaces/<id>/` (or `~/.shay/kanban/boards/<slug>/workspaces/<id>/` on non-default boards).
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `SHAY_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
shay kanban boards list

# Create a new board.
shay kanban boards create atm10-server \
    --name "ATM10 Server" \
...
shay kanban --board atm10-server list
shay kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
shay kanban boards switch atm10-server
shay kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
shay kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `SHAY_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.shay/kanban/current` — the slug persisted by `shay kanban
   boards switch`.
4. `default`.
...
`shay dashboard` → Kanban tab shows a board switcher at the top as soon
...
shay kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `shay kanban show t_abcd`.
...
Override the config flag at runtime via `SHAY_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `shay gateway
...
until one comes up — `shay kanban create` warns about this at creation
time.

Running `shay kanban daemon` as a separate process is **deprecated**;
...
shay kanban create "nightly ops review" \
    --assignee ops \
...
```bash
shay kanban complete t_abc t_def t_hij --result "batch wrap"
shay kanban archive  t_abc t_def t_hij
shay kanban unblock  t_abc t_def
shay kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `shay kanban`.** When the dispatcher spawns a worker it sets `SHAY_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `shay kanban` CLI.
...
kanban_show()                                     # no args — uses SHAY_KANBAN_TASK
...
### Why tools instead of shelling to `shay kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `shay kanban complete` *inside* the container, where `shay` isn't installed and `~/.shay/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.shay/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `shay chat` session has zero `kanban_*` tools in its schema. The `check_fn` on each tool only returns True when `SHAY_KANBAN_TASK` is set, which only happens when the dispatcher spawned this process. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/shay_cli/test_kanban_db.py -q"],
...
2. `cd $SHAY_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
shay -p <your-worker-profile> skills list | grep kanban-worker
```

If the bundled copy is missing, restore it for that profile:

```bash
shay -p <your-worker-profile> skills reset kanban-worker --restore
```
...
```bash
shay kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

shay kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
shay -p orchestrator skills list | grep kanban-orchestrator
```

If the bundled copy is missing, restore it for that profile:

```bash
shay -p orchestrator skills reset kanban-orchestrator --restore
```
...
- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas a specifier is expected to flesh out. Tasks created with `shay kanban create --triage` (or via the Triage column's inline create) land here and the dispatcher leaves them alone until a human or specifier promotes them to `todo` / `ready`. Run `shay kanban specify <id>` to have the auxiliary LLM expand a triage task into a concrete spec (title + body with goal, approach, acceptance criteria) and promote it to `todo` in one shot; `--all` sweeps every triage task at once. Configure which model runs the specifier under `auxiliary.triage_specifier` in `config.yaml`.
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.shay/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `shay dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.shay/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `shay -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard Shay-Shay dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
shay kanban init                                     # create kanban.db + print daemon hint
shay kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
shay kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
shay kanban show <id> [--json]
shay kanban assign <id> <profile>                    # or 'none' to unassign
shay kanban link <parent_id> <child_id>
shay kanban unlink <parent_id> <child_id>
shay kanban claim <id> [--ttl SECONDS]
shay kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
shay kanban complete <id>... [--result "..."]
shay kanban block <id> "<reason>" [--ids <id>...]
shay kanban unblock <id>...
shay kanban archive <id>...

shay kanban tail <id>                                # follow a single task's event stream
shay kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
shay kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
shay kanban runs <id> [--json]                       # attempt history (one row per run)
shay kanban assignees [--json]                       # profiles on disk + per-assignee task counts
shay kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
shay kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `shay gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
shay kanban stats [--json]                           # per-status + per-assignee counts
shay kanban log <id> [--tail BYTES]                  # worker log from ~/.shay/kanban/logs/
shay kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
shay kanban notify-list [<id>] [--json]
shay kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
shay kanban context <id>                             # what a worker sees
shay kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
shay kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
Every `shay kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `shay chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `shay_cli.kanban.run_slash()` entry point that reuses the `shay kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `shay kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.shay/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`shay kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `shay kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/shay-kanban-v1-spec.pdf`.
...
```bash
shay kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$SHAY_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
shay kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
shay kanban notify-list
shay kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
shay kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
shay kanban runs t_abcd
...
**Bulk close caveat.** `shay kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`shay kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `shay kanban heartbeat $TASK` to signal liveness during long operations. |
...
`shay kanban tail <id>` shows these for a single task. `shay kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.shay/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/shay-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
