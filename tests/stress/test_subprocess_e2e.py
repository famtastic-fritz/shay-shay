...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves shay_cli.kanban_db on its own
  ...
  - worker logs are captured to SHAY_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "SHAY_KANBAN_TASK": task.id,
            "SHAY_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="shay_e2e_")
    os.environ["SHAY_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from shay_cli import kanban_db as kb

    # Point the `shay` CLI child processes will run at the worktree
    # shay_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "shay")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m shay_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/shay_e2e_*/worker_*.log
    import glob
