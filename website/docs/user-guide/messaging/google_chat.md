...
sidebar_position: 12
title: "Google Chat"
description: "Set up Shay-Shay as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Shay-Shay to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Shay-Shay
...
download the file. Save it somewhere only Shay-Shay can read (e.g.,
`~/.shay/google-chat-sa.json`, `chmod 600`).
...
- Topic ID: `shay-chat-events`
- Leave the defaults for everything else.

After creation, the topic's detail page has a **Subscriptions** tab. Create one:

- Subscription ID: `shay-chat-events-sub`
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a shay restart)
...
- Principal: `shay-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Shay-Shay calls
...
- **App name**: whatever you want users to see ("Shay-Shay" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/shay-chat-events`.
...
`ADDED_TO_SPACE` event that Shay-Shay uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Shay-Shay

Add the Google Chat section to `~/.shay/.env`:

```bash
# Required
GOOGLE_CHAT_PROJECT_ID=my-chat-bot-123
GOOGLE_CHAT_SUBSCRIPTION_NAME=projects/my-chat-bot-123/subscriptions/shay-chat-events-sub
GOOGLE_CHAT_SERVICE_ACCOUNT_JSON=/home/you/.shay/google-chat-sa.json
...
Install the dependencies the Google Chat adapter needs (no Shay-Shay extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Shay-Shay is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Shay-Shay detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Shay-Shay session.
...
3. Download the JSON. Move it onto the host that runs Shay-Shay.
4. On the host, register the client with Shay-Shay:
...
That writes `~/.shay/google_chat_user_client_secret.json`. This is shared
...
The token lands at `~/.shay/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.shay/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Shay-Shay isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `shay gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Shay-Shay will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.shay/google_chat_user_tokens/<sanitized_email>.json` (filesystem
