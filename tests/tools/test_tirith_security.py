...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._shay_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_shay_bin(self):
        """After _INSTALL_FAILED, manual install in SHAY_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        shay_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(shay_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(shay_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._shay_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == shay_bin
            assert _tirith_mod._resolved_path == shay_bin
            mock_clear.assert_called_once()
...
class TestShayHomeIsolation:
    def test_shay_bin_dir_respects_shay_home(self):
        """_shay_bin_dir must use SHAY_HOME, not hardcoded ~/.shay."""
        from tools.tirith_security import _shay_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"SHAY_HOME": tmpdir}):
            result = _shay_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_shay_home(self):
        """_failure_marker_path must use SHAY_HOME, not hardcoded ~/.shay."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"SHAY_HOME": "/custom/shay"}):
            result = _failure_marker_path()
        assert result == "/custom/shay/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets SHAY_HOME; verify it's active."""
        shay_home = os.getenv("SHAY_HOME")
        assert shay_home is not None, "SHAY_HOME should be set by conftest"
        assert "shay_test" in shay_home, "Should point to test temp dir"

    def test_get_shay_home_fallback(self):
        """Without SHAY_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_shay_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove SHAY_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".shay")
            result = _get_shay_home()
