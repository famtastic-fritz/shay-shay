/**
 * ChatPage — embeds `shay --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (shay_cli/web_server.py)                          .
 *          ▼                                                              .
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__SHAY_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `shay dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // let the inner Shay-Shay TUI own transcript history/scroll behavior.
    ...
    // Shay-Shay TUI own transcript scrolling.
    //
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[shay-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Shay-Shay. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="shay-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
