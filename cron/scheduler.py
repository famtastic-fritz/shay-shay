...
Uses a file-based lock (~/.shay/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `shay update` reloads
# the module) fail with ModuleNotFoundError for shay_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from shay_constants import get_shay_home
from shay_cli.config import load_config, _expand_env_vars
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``shay tools`` config for the ``cron`` platform.
    ...
    if per_job:
        return per_job
    try:
        from shay_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
_shay_home: Path | None = None


def _get_shay_home() -> Path:
    """Resolve Shay-Shay home dynamically while preserving test monkeypatch hooks."""
    return _shay_home or get_shay_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    shay_home = _get_shay_home()
    lock_dir = shay_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("SHAY_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid SHAY_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within SHAY_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    Args:
        script_path: Path to the script.  Relative paths are resolved
            against SHAY_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    scripts_dir = _get_shay_home() / "scripts"
    ...
    # escape — scripts MUST reside within SHAY_HOME/scripts/.
    try:
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    if job.get("no_agent"):
        ...
        now_iso = _shay_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_shay_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_shay_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["SHAY_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed SHAY_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads SHAY_SESSION_PLATFORM
    #     and SHAY_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via SHAY_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     SHAY_SESSION_PLATFORM == "telegram".
    ...
    #     SHAY_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read SHAY_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the SHAY_CRON_AUTO_DELIVER_* vars set
    # below, so clearing SHAY_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "SHAY_CRON_AUTO_DELIVER_PLATFORM",
        "SHAY_CRON_AUTO_DELIVER_CHAT_ID",
        "SHAY_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        try:
            load_dotenv(str(_get_shay_home() / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_get_shay_home() / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["SHAY_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["SHAY_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["SHAY_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        model = job.get("model") or os.getenv("SHAY_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_shay_home() / "config.yaml")
        ...
        prefill_file = os.getenv("SHAY_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _get_shay_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject SHAY_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # SHAY_HOME. When a workdir is configured, also inject project
        ...
        # override via SHAY_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("SHAY_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid SHAY_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_shay_now().strftime('%Y-%m-%d %H:%M:%S')}
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _shay_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _shay_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set SHAY_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("SHAY_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid SHAY_CRON_MAX_PARALLEL value; defaulting to unbounded")
