...
Shay-Shay includes a full browser automation toolset with multiple backend options:
...
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, you can use browser automation through the **[Tool Gateway](tool-gateway.md)** without any separate API keys. Run `shay model` or `shay tools` to enable it.
:::
...
When a cloud provider is configured, Shay-Shay auto-spawns a **local Chromium sidecar**
...
mode, so you need it installed (`shay setup tools → Browser Automation`
...
Then set in `~/.shay/.env`:

```bash
CAMOFOX_URL=http://localhost:9377
```

Or configure via `shay tools` → Browser Automation → Camofox.
...
By default, each Camofox session gets a random identity — cookies and logins don't survive across agent restarts. To enable persistent browser sessions, add the following to `~/.shay/config.yaml`:
...
Then fully restart Shay-Shay so the new config is picked up.

:::warning Nested path matters
Shay-Shay reads `browser.camofox.managed_persistence`, **not** a top-level `managed_persistence`. A common mistake is writing:

```yaml
# ❌ Wrong — Shay-Shay ignores this
managed_persistence: true
```

If the flag is placed at the wrong path, Shay-Shay silently falls back to a random ephemeral `userId` and your login state will be lost on every session.
:::
...
- Scopes the `userId` to the active Shay-Shay profile, so different Shay-Shay profiles get different browser profiles (profile isolation).

##### What Shay-Shay does not do
- It does not force persistence on the Camofox server. Shay-Shay only sends a stable `userId`; the server must honor it by mapping that `userId` to a persistent Firefox profile directory.
- If your Camofox server build treats every request as ephemeral (e.g. always calls `browser.newContext()` without loading a stored profile), Shay-Shay cannot make those sessions persist. Make sure you are running a Camofox build that implements userId-based profile persistence.

##### Verify it's working

1. Start Shay-Shay and your Camofox server.
...
If step 5 logs you out, the Camofox server isn't honoring the stable `userId`. Double-check your config path, confirm you fully restarted Shay-Shay after editing `config.yaml`, and verify your Camofox server version supports persistent per-user profiles.

##### Where state lives

Shay-Shay derives the stable `userId` from the profile-scoped directory `~/.shay/browser_auth/camofox/` (or the equivalent under `$SHAY_HOME` for non-default profiles). The actual browser profile data lives on the Camofox server side, keyed by that `userId`. To fully reset a persistent profile, clear it on the Camofox server and remove the corresponding Shay-Shay profile's state directory.

#### VNC live view

When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Shay-Shay automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chrome via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Shay-Shay browser tools to your own running Chrome instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.

:::note
`/browser connect` is an **interactive-CLI slash command** — it is not dispatched by the gateway. If you try to run it inside a WebUI, Telegram, Discord, or other gateway chat, the message will be sent to the agent as plain text and the command will not execute. Start Shay-Shay from the terminal (`shay` or `shay chat`) and issue `/browser connect` there.
:::
...
If Chrome isn't already running with remote debugging, Shay-Shay will attempt to auto-launch it with `--remote-debugging-port=9222`.
...
google-chrome \
  ...
  --user-data-dir=$HOME/.shay/chrome-debug \
  --no-first-run \
...
Then launch the Shay-Shay CLI and run `/browser connect`.
...
If Shay-Shay runs inside WSL2 but the Chrome window you want to control runs on the Windows host, `/browser connect` is often not the best path.

Why:

- `/browser connect` expects Shay-Shay itself to reach a usable CDP endpoint
...
- even when Windows Chrome is debuggable, the cleanest integration is often to let a Windows-side browser MCP server attach to Chrome and let Shay-Shay talk to that MCP server

For that setup, prefer `chrome-devtools-mcp` through Shay-Shay MCP support.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Shay-Shay can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `shay config set toolsets '["shay-cli", "browser"]'`.
:::
...
```
Type "shay-shay" into the search field @e3
```
...
Screenshots are stored in `~/.shay/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.shay/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Shay-Shay automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
