...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["shay", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "shay_cli.plugins",
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["shay", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "shay_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from shay_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "shay -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """shay -z bypasses ShayCLI, but recall still needs SessionDB."""
    from shay_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "shay_state", mod("shay_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "shay_cli.config",
        mod("shay_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "shay_cli.models",
        mod("shay_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "shay_cli.runtime_provider",
        mod(
            "shay_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "shay_cli.tools_config",
        mod("shay_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["SHAY_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/shay-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["SHAY_MODEL"] == "nous/shay-test"
    assert env["SHAY_INFERENCE_MODEL"] == "nous/shay-test"
    assert env["SHAY_TUI_PROVIDER"] == "nous"
    assert env["SHAY_INFERENCE_PROVIDER"] == "nous"
    assert env["SHAY_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["SHAY_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("shay-tui-active-session-")
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import shay_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "shay_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "shay --tui --resume 20260409_000001_abc123" in out
    assert 'shay --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "shay --tui --resume actual_session" in out
