...
from shay_cli.config import get_env_value, load_env
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``shay model`` / ``shay auth`` which
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("shay_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("shay_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Shay-Shay profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `shay model` / `shay auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `shay auth remove <provider> <N>` is stable across all source types.
    try:
        from shay_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Shay-Shay PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_shay_oauth_credentials

        for source_name, creds in (
            ("shay_pkce", read_shay_oauth_credentials()),
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # `shay auth add nous --label <name>`).  Fall back to the
        ...
        # Shay-Shay auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from shay_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.shay/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `shay auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `shay auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Shay-Shay auth store.  Without this gate the removal is instantly
        ...
        # Shay-Shay owns its own Codex auth state — we do NOT auto-import from
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.shay/.env over os.environ — the user's config file is the
    # authoritative source for Shay-Shay credentials. Stale env vars from parent
    ...
    # Honour user suppression — `shay auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.shay/.env.
    ...
    try:
        from shay_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.shay/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "shay_pkce"}
        )
    ]
