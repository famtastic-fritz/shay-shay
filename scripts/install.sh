...
# Python-driven tool session (e.g. Shay-Shay terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:NousResearch/shay-shay.git"
REPO_URL_HTTPS="https://github.com/NousResearch/shay-shay.git"
SHAY_HOME="${SHAY_HOME:-$HOME/.shay}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${SHAY_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$SHAY_INSTALL_DIR"
...
#   code at /usr/local/lib/shay-shay, command at /usr/local/bin/shay,
#   data still at /root/.shay (SHAY_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (non-root):  ~/.shay/shay-shay"
            echo "                   default (root, Linux): /usr/local/lib/shay-shay"
            echo "  --shay-home PATH  Data directory (default: ~/.shay, or \$SHAY_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Shay-Shay installs the code under"
            echo "  /usr/local/lib/shay-shay and links the command into"
            echo "  /usr/local/bin/shay (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$SHAY_HOME"
            echo "  (default /root/.shay).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$SHAY_HOME/shay-shay are preserved in-place."
            exit 0
            ;;
        *)
...
# Decide where the repo checkout + venv live, and where the `shay` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $SHAY_HOME/shay-shay
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $SHAY_HOME/shay-shay
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/shay-shay
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $SHAY_HOME/shay-shay — then preserve it)
#
# Always no-op when the user set --dir or $SHAY_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in SHAY_HOME.
    if is_termux; then
        INSTALL_DIR="$SHAY_HOME/shay-shay"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$SHAY_HOME/shay-shay/.git" ]; then
            INSTALL_DIR="$SHAY_HOME/shay-shay"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/shay-shay)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/shay-shay"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/shay"
        log_info "  Data:    $SHAY_HOME (unchanged)"
        return 0
    fi
...
get_shay_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/shay" ]; then
        echo "$link_dir/shay"
    else
        echo "shay"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/NousResearch/shay-shay/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$SHAY_HOME/node/bin/node" ]; then
        export PATH="$SHAY_HOME/node/bin:$PATH"
        local found_ver=$("$SHAY_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Shay-Shay-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.shay/node/..."
    ...
    # Place into ~/.shay/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$SHAY_HOME/node"
    mkdir -p "$SHAY_HOME"
    mv "$extracted_dir" "$SHAY_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$SHAY_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$SHAY_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$SHAY_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$SHAY_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$SHAY_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.shay/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Shay-Shay install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Shay-Shay itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="shay-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Shay-Shay behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up shay command..."

    if [ "$USE_VENV" = true ]; then
        SHAY_BIN="$INSTALL_DIR/venv/bin/shay"
    else
        SHAY_BIN="$(which shay 2>/dev/null || echo "")"
        if [ -z "$SHAY_BIN" ]; then
            log_warn "shay not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$SHAY_BIN" ]; then
        log_warn "shay entry point not found at $SHAY_BIN"
    ...
    # Create a user-facing shim for the shay command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    cat > "$command_link_dir/shay" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$SHAY_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/shay"
    log_success "Installed shay launcher → $command_link_display_dir/shay"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "shay command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v shay' \
                >/dev/null 2>&1; then
        ...
        log_info "shay not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Shay-Shay — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Shay-Shay — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Shay-Shay — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so shay works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.shay directory structure (config at top level, code in subdir)
    mkdir -p "$SHAY_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.shay/.env (top level, easy to find)
    if [ ! -f "$SHAY_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$SHAY_HOME/.env"
            log_success "Created ~/.shay/.env from template"
        else
            touch "$SHAY_HOME/.env"
            log_success "Created ~/.shay/.env"
        fi
    else
        log_info "~/.shay/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.shay/config.yaml (top level, easy to find)
    if [ ! -f "$SHAY_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$SHAY_HOME/config.yaml"
            log_success "Created ~/.shay/config.yaml from template"
        fi
    else
        log_info "~/.shay/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$SHAY_HOME/SOUL.md" ]; then
        cat > "$SHAY_HOME/SOUL.md" << 'SOUL_EOF'
# Shay-Shay Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Shay-Shay communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.shay/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.shay/"

    # Seed bundled skills into ~/.shay/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.shay/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.shay/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$SHAY_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$SHAY_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.shay/skills/"
        fi
    fi
}
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ubuntu|debian|raspbian|pop|linuxmint|elementary|zorin|kali|parrot)
                ...
                log_info "This is standard Playwright setup — Shay-Shay itself does not require root access."
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (shay --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'shay setup' after install."
        return 0
    fi
    ...
    # Run shay setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m shay_cli.main setup < /dev/tty
    else
        python -m shay_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$SHAY_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Shay-Shay to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$SHAY_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'shay whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                SHAY_CMD="$(get_shay_command_path)"
                $SHAY_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'shay whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'shay gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $SHAY_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $SHAY_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: shay gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: shay gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $SHAY_CMD gateway > "$SHAY_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.shay/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: shay gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: shay gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $SHAY_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $SHAY_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $SHAY_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}shay${NC}              Start chatting"
    echo -e "   ${GREEN}shay setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}shay config${NC}       View/edit configuration"
    echo -e "   ${GREEN}shay config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}shay gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}shay update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'shay' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'shay' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'shay' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
