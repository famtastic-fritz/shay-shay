# Shay-Shay Has Had "Routines" Since March
...
Shay-Shay equivalent — works today:
```bash
...
Shay-Shay equivalent — works today:
```bash
shay webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Shay-Shay implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Shay-Shay |
|---|---|---|
...
## Things Shay-Shay Does That Routines Can't
...
Shay-Shay has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Shay-Shay is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.

```bash
pip install shay-shay
shay setup
```
...
```bash
shay gateway setup    # enable webhooks
shay webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [shay-shay.nousresearch.com/docs/guides/automation-templates](https://shay-shay.nousresearch.com/docs/guides/automation-templates)

Documentation: [shay-shay.nousresearch.com](https://shay-shay.nousresearch.com)

GitHub: [github.com/NousResearch/shay-shay](https://github.com/NousResearch/shay-shay)

---

*Shay-Shay is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
