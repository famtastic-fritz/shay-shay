"""Shared fixtures for the shay-shay test suite.
...
2. **Isolated SHAY_HOME.** SHAY_HOME points to a per-test tempdir so
   code reading ``~/.shay/*`` via ``get_shay_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".shay"`` instead of the canonical
   ``get_shay_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No SHAY_SESSION_* inheritance** — the agent's current gateway
...
# SHAY_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_SHAY_BEHAVIORAL_VARS = frozenset({
    "SHAY_YOLO_MODE",
    "SHAY_INTERACTIVE",
    "SHAY_QUIET",
    "SHAY_TOOL_PROGRESS",
    ...
    "SHAY_SESSION_CHAT_ID",
    "SHAY_SESSION_CHAT_NAME",
    "SHAY_SESSION_THREAD_ID",
    "SHAY_SESSION_SOURCE",
    "SHAY_SESSION_KEY",
    "SHAY_GATEWAY_SESSION",
    "SHAY_PLATFORM",
    "SHAY_MODEL",
    "SHAY_INFERENCE_MODEL",
    "SHAY_INFERENCE_PROVIDER",
    "SHAY_TUI_PROVIDER",
    "SHAY_MANAGED",
    "SHAY_DEV",
    "SHAY_CONTAINER",
    ...
    "SHAY_TIMEZONE",
    "SHAY_REDACT_SECRETS",
    "SHAY_BACKGROUND_NOTIFICATIONS",
    "SHAY_EXEC_ASK",
    "SHAY_HOME_MODE",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.shay/kanban.db instead of the per-test SHAY_HOME.
    "SHAY_KANBAN_DB",
    "SHAY_KANBAN_BOARD",
    "SHAY_KANBAN_WORKSPACES_ROOT",
    "SHAY_KANBAN_LOGS_ROOT",
    "SHAY_KANBAN_TASK",
    "SHAY_KANBAN_WORKSPACE",
    "SHAY_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and SHAY_HOME to per-test tempdirs so code that
    reads ``~/.shay/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral SHAY_* vars that could change test semantics.
    for name in _SHAY_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect SHAY_HOME to a per-test tempdir. Code that reads
    #    ``~/.shay/*`` via ``get_shay_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.shay/*`` via
    #    ``Path.home() / ".shay"`` instead of ``get_shay_home()``
    #    is a bug to fix at the callsite.
    fake_shay_home = tmp_path / "shay_test"
    fake_shay_home.mkdir()
    (fake_shay_home / "sessions").mkdir()
    (fake_shay_home / "cron").mkdir()
    (fake_shay_home / "memories").mkdir()
    (fake_shay_home / "skills").mkdir()
    monkeypatch.setenv("SHAY_HOME", str(fake_shay_home))
    ...
    #    ~/.shay/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import shay_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_shay_home(_hermetic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "shay_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal shay config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``shay-gateway`` process
...
# correlated with a ``tests/shay_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> shay-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart shay-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting shay/python patterns are also blocked.
    """
    ...
    def _matches_shay_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _SHAY_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_shay_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch shay-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m shay_cli.main".
                if (
                    "shay" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live shay-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting shay/python could hit the live gateway. "
