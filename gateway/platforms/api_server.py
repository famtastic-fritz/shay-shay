...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Shay-Shay-Session-Id header; opt-in long-term memory scoping via X-Shay-Shay-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Shay-Shay-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists shay-shay as an available model
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to shay-shay
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_shay_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        try:
        ...
        # gracefully on NFS/SMB/FUSE-mounted SHAY_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # shay_state._WAL_INCOMPAT_MARKERS).
...
def _derive_chat_session_id(
    ...
    the same Shay-Shay session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through shay-shay's AIAgent.
    """
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Shay-Shay-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Shay-Shay-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Shay-Shay-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Shay-Shay-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Shay-Shay-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``shay sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the shay-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Shay-Shay-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "shay-shay"})
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — return shay-shay as an available model."""
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Shay-Shay version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Shay-Shay AIAgent; "
            ...
            "features": {
                ...
                "session_continuity_header": "X-Shay-Shay-Session-Id",
                "session_key_header": "X-Shay-Shay-Session-Key",
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # stable per-channel identifier via X-Shay-Shay-Session-Key.  This
        # is independent of X-Shay-Shay-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Shay-Shay-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Shay-Shay-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Shay-Shay-Session-Id rejected: "
            ...
            # conversation map to the same Shay-Shay session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``shay.tool.progress`` with ``status: running``.
        ...
        response_headers = {
            "X-Shay-Shay-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Shay-Shay-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            response_headers["X-Shay-Shay-Completed"] = "false"
            response_headers["X-Shay-Shay-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Shay-Shay-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if is_partial or is_failed or not completed:
            ...
            response_headers["X-Shay-Shay-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Shay-Shay-Error"] = err_msg[:200]
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Shay-Shay-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Shay-Shay-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: shay.tool.progress`` SSE event so
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Shay-Shay-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Shay-Shay-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        def _run():
            ...
            # (e.g. X-Shay-Shay-Session-Id header) can track compression-
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = (
            {"X-Shay-Shay-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "shay.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if is_network_accessible(self._host) and self._api_key:
                try:
                    from shay_cli.auth import has_usable_secret
