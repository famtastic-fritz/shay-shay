"""Tests for shay_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate SHAY_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".shay"
    home.mkdir()
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from shay_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in shay_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from shay_cli import model_catalog
        from shay_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from shay_cli import model_catalog
        from shay_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_manifest(self, tmp_path, monkeypatch):
        ...
        the slash-command picker until the next Shay-Shay release.
        """
        ...
        # ``SHAY_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` SHAY_HOME directly instead.
        import importlib
        from shay_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            from shay_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["SHAY_HOME"])
