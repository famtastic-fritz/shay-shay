"""Unit tests for shay_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf shay-ok"])
        try:
            output = _read_until(bridge, b"shay-ok")
            assert b"shay-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$SHAY_PTY_TEST\""],
            env={**os.environ, "SHAY_PTY_TEST": "pty-env-works"},
        )
        try:
