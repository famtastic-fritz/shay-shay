...
title: "Plugins"
description: "Extend Shay-Shay with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Shay-Shay has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/docs/developer-guide/adding-tools) page is for built-in Shay-Shay
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Shay-Shay Plugin](/docs/guides/build-a-shay-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.shay/plugins/` with a `plugin.yaml` and Python code:

```
~/.shay/plugins/my-plugin/
...
Start Shay-Shay — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.shay/plugins/hello-world/plugin.yaml`**
...
**`~/.shay/plugins/hello-world/__init__.py`**

```python
"""Minimal Shay-Shay plugin — registers a tool and a hook."""
...
Drop both files into `~/.shay/plugins/hello-world/`, restart Shay-Shay, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.shay/plugins/` are disabled by default. Enable them only for trusted repositories by setting `SHAY_ENABLE_PROJECT_PLUGINS=true` before starting Shay-Shay.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `shay <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `shay plugins install` |
| Distribute via pip | `[project.entry-points."shay_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Shay-Shay — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.shay/plugins/` | Personal plugins |
| Project | `.shay/plugins/` | Project-specific plugins (requires `SHAY_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `shay_agent.plugins` entry_points | Distributed packages |
| Nix | `services.shay-shay.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Shay-Shay also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.shay/plugins/model-providers/<name>/` and `~/.shay/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `shay plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.shay/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
shay plugins                    # interactive toggle (space to check/uncheck)
shay plugins enable <name>      # add to allow-list
shay plugins disable <name>     # remove from allow-list + add to disabled
```

After `shay plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Shay-Shay' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.shay/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.shay/plugins/`.

### Migration for existing users

When you upgrade to a version of Shay-Shay that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.shay/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Shay-Shay has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.shay/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Shay-Shay also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Shay-Shay Plugin](/docs/guides/build-a-shay-plugin) · [Adding Tools](/docs/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/docs/user-guide/features/hooks) · [Build a Shay-Shay Plugin](/docs/guides/build-a-shay-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Shay-Shay Plugin](/docs/guides/build-a-shay-plugin) · [Extending the CLI](/docs/developer-guide/extending-the-cli) |
| A **subcommand** for `shay <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/docs/developer-guide/extending-the-cli) |
...
| An **STT backend** (custom whisper binary, local ASR CLI) | Config-driven — set `SHAY_LOCAL_STT_COMMAND` env var to a shell template | [Voice Message Transcription (STT)](/docs/user-guide/features/tts#voice-message-transcription-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Shay-Shay auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/docs/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `shay skills tap add <repo>` | [Skills Hub](/docs/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/docs/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.shay/hooks/<name>/` | [Event Hooks](/docs/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `shay plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
shay plugins                               # unified interactive UI
shay plugins list                          # table: enabled / disabled / not enabled
shay plugins install user/repo             # install from Git, then prompt Enable? [y/N]
shay plugins install user/repo --enable    # install AND enable (no prompt)
shay plugins install user/repo --no-enable # install but leave disabled (no prompt)
shay plugins update my-plugin              # pull latest
shay plugins remove my-plugin              # uninstall
shay plugins enable my-plugin              # add to allow-list
shay plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `shay plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `shay plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-shay-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
