...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.shay/.env``.
"""
...
def _managed_persistence_enabled() -> bool:
    """Return whether Shay-Shay-managed persistence is enabled for Camofox.
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Shay-Shay profile so the Camofox server can map it
    ...
    with _sessions_lock:
        ...
        if _managed_persistence_enabled():
            ...
            session = {
                "user_id": f"shay_{uuid.uuid4().hex[:10]}",
                "tab_id": None,
...
def camofox_vision(question: str, annotate: bool = False,
    ...
    try:
        ...
        screenshots_dir = get_shay_home() / "browser_screenshots"
