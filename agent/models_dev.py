...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.shay/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Shay-Shay ↔ models.dev
# ---------------------------------------------------------------------------

# Shay-Shay provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Shay-Shay (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_shay_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``shay config refresh``, the
...
# Gemma models whose TPM quotas are too small for normal Shay-Shay traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Shay-Shay provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Shay-Shay ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Shay-Shay or models.dev provider ID.  Tries exact match then
