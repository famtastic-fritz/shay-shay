...
def provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_shay_home", lambda: tmp_path
    )

    p = HindsightMemoryProvider()
    p.initialize(session_id="test-session", shay_home=str(tmp_path), platform="cli")
...
def test_normalize_retain_tags_accepts_csv_and_dedupes():
    assert _normalize_retain_tags("agent:fakeassistantname, source_system:shay-shay, agent:fakeassistantname") == [
        "agent:fakeassistantname",
        "source_system:shay-shay",
    ]


def test_normalize_retain_tags_accepts_json_array_string():
    value = json.dumps(["agent:fakeassistantname", "source_system:shay-shay"])
    assert _normalize_retain_tags(value) == ["agent:fakeassistantname", "source_system:shay-shay"]
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Shay-Shay and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_shay_home",
        ...
        assert cfg["banks"]["shay"]["bankId"] == "env-bank"
        assert cfg["banks"]["shay"]["budget"] == "high"
...
class TestPostSetup:
    def test_local_embedded_setup_materializes_profile_env(self, tmp_path, monkeypatch):
        shay_home = tmp_path / "shay-home"
        user_home = tmp_path / "user-home"
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))

        selections = iter([1, 0])  # local_embedded, openai
        monkeypatch.setattr("shay_cli.memory_setup._curses_select", lambda *args, **kwargs: next(selections))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "sk-local-test")
        saved_configs = []
        monkeypatch.setattr("shay_cli.config.save_config", lambda cfg: saved_configs.append(cfg.copy()))

        provider = HindsightMemoryProvider()
        provider.post_setup(str(shay_home), {"memory": {}})

        assert saved_configs[-1]["memory"]["provider"] == "hindsight"
        env_text = (shay_home / ".env").read_text()
        assert "HINDSIGHT_LLM_API_KEY=sk-local-test\n" in env_text
        assert "HINDSIGHT_TIMEOUT=120\n" in env_text
        assert "HINDSIGHT_IDLE_TIMEOUT=300\n" in env_text

        profile_env = user_home / ".hindsight" / "profiles" / "shay.env"
    ...
    def test_local_embedded_setup_respects_existing_profile_name(self, tmp_path, monkeypatch):
        ...
        provider = HindsightMemoryProvider()
        provider.save_config({"profile": "coder"}, str(shay_home))
        provider.post_setup(str(shay_home), {"memory": {}})

        coder_env = user_home / ".hindsight" / "profiles" / "coder.env"
        shay_env = user_home / ".hindsight" / "profiles" / "shay.env"
        assert coder_env.exists()
        assert not shay_env.exists()
    ...
    def test_local_embedded_setup_blank_inputs_preserve_existing_config(self, tmp_path, monkeypatch):
        ...
        user_home.mkdir()
        monkeypatch.setenv("HOME", str(user_home))
        monkeypatch.setattr("plugins.memory.hindsight.get_shay_home", lambda: shay_home)
        ...
        provider = HindsightMemoryProvider()
        provider.save_config(existing_config, str(shay_home))
        ...
        monkeypatch.setattr("shay_cli.memory_setup._curses_select", lambda *args, **kwargs: kwargs.get("default", 0))
        monkeypatch.setattr("shutil.which", lambda name: None)
        monkeypatch.setattr("builtins.input", lambda prompt="": "")
        monkeypatch.setattr("sys.stdin.isatty", lambda: True)
        monkeypatch.setattr("getpass.getpass", lambda prompt="": "")
        ...
        saved = json.loads((shay_home / "hindsight" / "config.json").read_text())
...
class TestSyncTurn:
    def test_sync_turn_retains_metadata_rich_turn(self, provider_with_config):
        ...
        assert item["context"] == "conversation between Shay-Shay and the User"
        ...
        assert item["metadata"]["source"] == "shay"
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["retain_async"] is True
        assert len(call_kwargs["items"]) == 1
        assert call_kwargs["items"][0]["context"] == "conversation between Shay-Shay and the User"
    ...
    def test_resume_creates_new_document(self, tmp_path, monkeypatch):
        ...
        config_path.parent.mkdir(parents=True, exist_ok=True)
        config_path.write_text(json.dumps(config))
        monkeypatch.setattr("plugins.memory.hindsight.get_shay_home", lambda: tmp_path)

        p1 = HindsightMemoryProvider()
        p1.initialize(session_id="resumed-session", shay_home=str(tmp_path), platform="cli")
        ...
        p2.initialize(session_id="resumed-session", shay_home=str(tmp_path), platform="cli")
...
class TestBankIdTemplate:
    def test_sanitize_bank_segment_passthrough(self):
        assert _sanitize_bank_segment("shay") == "shay"
    ...
    def test_resolve_collapses_empty_placeholders(self):
        # When user is empty, "shay-{user}" becomes "shay-" -> trimmed to "shay"
        result = _resolve_bank_id_template(
            "shay-{user}", fallback="default",
    ...
    def test_provider_uses_bank_id_template_from_config(self, tmp_path, monkeypatch):
        ...
        p.initialize(
            ...
            agent_workspace="shay",
        )
        assert p._bank_id == "shay-coder"
        assert p._bank_id_template == "shay-{profile}"
    ...
    def test_provider_template_with_missing_profile_falls_back(self, tmp_path, monkeypatch):
        ...
        p = HindsightMemoryProvider()
        # No agent_identity passed — template renders to "shay-" which collapses to "shay"
        p.initialize(session_id="s1", shay_home=str(tmp_path), platform="cli")
        assert p._bank_id == "shay"
