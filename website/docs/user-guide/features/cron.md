...
Schedule tasks to run automatically with natural language or cron expressions. Shay-Shay exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Shay-Shay itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::warning
Cron-run sessions cannot recursively create more cron jobs. Shay-Shay disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
shay cron create "every 2h" "Check server status"
shay cron create "every 1h" "Summarize new feed items" --skill blogwatcher
shay cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Shay-Shay normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Shay-Shay will use the unified `cronjob` tool internally.
...
shay cron create "every 1d at 09:00" \
...
```bash
shay cron edit <job_id> --schedule "every 4h"
shay cron edit <job_id> --prompt "Use the revised task"
shay cron edit <job_id> --skill blogwatcher --skill maps
shay cron edit <job_id> --add-skill maps
shay cron edit <job_id> --remove-skill blogwatcher
shay cron edit <job_id> --clear-skills
```
...
```bash
shay cron list
shay cron pause <job_id>
shay cron resume <job_id>
shay cron run <job_id>
shay cron remove <job_id>
shay cron status
shay cron tick
```
...
```bash
shay gateway install     # Install as a user service
sudo shay gateway install --system   # Linux: boot-time system service for servers
shay gateway             # Or run in foreground
...
On each tick Shay-Shay:

1. loads jobs from `~/.shay/cron/jobs.json`
...
A file lock at `~/.shay/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.shay/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.shay/cron/output/`), but no message is sent to the delivery target.
...
Or set the `SHAY_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.shay/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Shay-Shay directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Shay-Shay will write the check script to `~/.shay/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.shay/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.shay/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.shay/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.shay/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Shay-Shay reads Job 1's most recent output from `~/.shay/cron/output/{job1_id}/*.md`
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Shay-Shay skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `shay tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `shay tools` cron-platform config wins; otherwise Shay-Shay falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Shay-Shay should even invoke the agent. Emit a final stdout line of the form:
...
Jobs are stored in `~/.shay/cron/jobs.json`. Output from job runs is saved to `~/.shay/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Shay-Shay resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
