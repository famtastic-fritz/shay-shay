"""langfuse — Shay-Shay plugin for Langfuse observability.

Traces Shay-Shay conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Shay-Shay plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``shay plugins enable
observability/langfuse`` or ``shay tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``shay tools`` or ~/.shay/.env):
  SHAY_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  SHAY_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  SHAY_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  SHAY_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  SHAY_LANGFUSE_RELEASE     - release/version tag
  SHAY_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  SHAY_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  SHAY_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("SHAY_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Shay-Shay plugin system —
    ...
    public_key = _env("SHAY_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("SHAY_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    if not (public_key and secret_key):
        _LANGFUSE_CLIENT = _INIT_FAILED
        return None

    base_url = _env("SHAY_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("SHAY_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("SHAY_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("SHAY_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid SHAY_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("SHAY_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Shay-Shay branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Shay-Shay also has a turn-scoped
    ...
    # messages is a list only for legacy Shay-Shay branches that fired
    # pre_llm_call with API messages directly. Current Shay-Shay fires
...
def register(ctx) -> None:
    ...
    # Shay-Shay versions.  pre_api_request / post_api_request fire per API
