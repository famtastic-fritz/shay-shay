...
def backup_env(monkeypatch, tmp_path):
    """Isolate SHAY_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".shay"
    home.mkdir()
    (home / "skills").mkdir()
    ...
    # Reload so get_shay_home picks up the env var fresh.
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd shay_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under SHAY_HOME. Returns the path.
...
def _reload_cron_jobs(home: Path):
    """Reload cron.jobs so its module-level SHAY_DIR picks up the tmp HOME."""
    import shay_constants
    importlib.reload(shay_constants)
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.shay/cron/jobs.json
