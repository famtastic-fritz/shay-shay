...
def _make_runner_with_adapter(session_id: str = None):
    ...
    # module-import time, defeating per-test SHAY_HOME monkeypatches).
...
@pytest.mark.asyncio
async def test_goal_verdict_done_sent_via_adapter_send(shay_home):
    ...
    with patch("shay_cli.goals.judge_goal", return_value=("done", "the feature shipped", False)):
...
@pytest.mark.asyncio
async def test_goal_verdict_continue_enqueues_continuation(shay_home):
    ...
    with patch("shay_cli.goals.judge_goal", return_value=("continue", "still needs work", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_budget_exhausted_sends_pause(shay_home):
    """When the budget is exhausted, a '⏸ Goal paused' message must be sent
    and no further continuation enqueued."""
    runner, adapter, session_entry, src = _make_runner_with_adapter()

    from shay_cli.goals import GoalManager, save_goal
    ...
    with patch("shay_cli.goals.judge_goal", return_value=("continue", "keep going", False)):
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
...
@pytest.mark.asyncio
async def test_goal_verdict_skipped_when_no_active_goal(shay_home):
...
@pytest.mark.asyncio
async def test_goal_verdict_survives_adapter_without_send(shay_home):
    ...
    with patch("shay_cli.goals.judge_goal", return_value=("done", "ok", False)):
        # must not raise
        await runner._post_turn_goal_continuation(
            session_entry=session_entry,
            source=src,
