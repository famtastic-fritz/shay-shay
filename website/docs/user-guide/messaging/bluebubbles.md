...
Connect Shay-Shay to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Shay-Shay and the BlueBubbles server
...
Or set environment variables directly in `~/.shay/.env`:
...
When someone messages your iMessage, Shay-Shay automatically sends them a pairing code. Approve it with:
```bash
shay pairing approve bluebubbles <CODE>
```
Use `shay pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.shay/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.shay/.env`):
```bash
...
Shay-Shay will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Shay-Shay
Shay-Shay → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Shay-Shay sends messages via the BlueBubbles REST API.
...
Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.shay/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Shay-Shay resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `shay logs gateway` for webhook errors (or `shay logs -f` to follow in real-time)
