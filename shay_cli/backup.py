"""
Backup and import commands for shay CLI.

`shay backup` creates a zip archive of the entire ~/.shay/ directory
(excluding the shay-shay repo and transient files).

`shay import` restores from a backup zip, overlaying onto the current
SHAY_HOME root.
"""
...
from shay_constants import get_default_shay_root, get_shay_home, display_shay_home
...
_EXCLUDED_DIRS = {
    "shay-shay",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to shay root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the Shay-Shay home directory."""
    shay_root = get_default_shay_root()

    if not shay_root.is_dir():
        print(f"Error: Shay-Shay home directory not found at {shay_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"shay-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"shay-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_shay_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(shay_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(shay_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(shay_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside shay root
            try:
    ...
    print(f"\nRestore with: shay import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Shay-Shay backup.
    ...
    # Look for telltale files that a shay home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Shay-Shay backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.shay/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a shay dir name
        if prefix in {".shay", "shay"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_shay_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (shay_root / "config.yaml").exists()
        has_env = (shay_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Shay-Shay configuration.")
        ...
        for member in members:
            ...
            target = shay_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(shay_root.resolve())
            except ValueError:
        ...
        profiles_dir = shay_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # shay_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: shay profile list  (after installing shay)")

        # Guidance
        print()
        if not (shay_root / "shay-shay").is_dir():
            print("Note: The shay-shay codebase was not included in the backup.")
            print("  If this is a fresh install, run: shay update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  shay -p {pname} gateway install")

        print("Done. Your Shay-Shay configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and shay backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to SHAY_HOME).
...
# — `shay update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(shay_home: Optional[Path] = None) -> Path:
    home = shay_home or get_shay_home()
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(shay_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(shay_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for shay backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_shay_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, shay_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``shay_root`` to ``out_path``.
...
def _pre_update_backup_dir(shay_home: Optional[Path] = None) -> Path:
    home = shay_home or get_shay_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of SHAY_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<SHAY_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``shay update``) should continue even if the backup fails.
    """
    shay_root = shay_home or get_default_shay_root()
    if not shay_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(shay_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, shay_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `shay claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of SHAY_HOME under ``backups/`` before a
    ``shay claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``shay import <archive>``.  Writes to
    ``<SHAY_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `shay import` and the
