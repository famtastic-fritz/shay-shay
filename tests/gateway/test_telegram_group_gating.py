...
def test_group_messages_can_require_direct_trigger_via_config():
    ...
    assert adapter._should_process_message(_group_message("hi @shay_bot", entities=[_mention_entity("hi @shay_bot")])) is True
    ...
    assert adapter._should_process_message(
        _group_message(
            "/status@shay_bot",
            entities=[_bot_command_entity("/status@shay_bot", "/status@shay_bot")],
        ),
        is_command=True,
    ) is True
...
def test_guest_mode_allows_only_direct_mentions_outside_allowed_chats():
    ...
    mentioned = _group_message(
        "hi @shay_bot",
        chat_id=-201,
        entities=[_mention_entity("hi @shay_bot")],
    )
