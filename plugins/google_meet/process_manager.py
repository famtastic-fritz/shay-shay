...
session-scoped state file under ``$SHAY_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $SHAY_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_shay_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this shay install, leave it first —
    ...
    env = os.environ.copy()
    env["SHAY_MEET_URL"] = url
    env["SHAY_MEET_OUT_DIR"] = str(out)
    env["SHAY_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["SHAY_MEET_HEADED"] = "1"
    if auth_state:
        env["SHAY_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["SHAY_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if SHAY_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["SHAY_MEET_MODE"] = mode
    if realtime_model:
        env["SHAY_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["SHAY_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["SHAY_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["SHAY_MEET_REALTIME_KEY"] = realtime_api_key
