"""Shell completion script generation for shay CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_shay_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Shay-Shay bash completion
# Add to ~/.bashrc:
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef shay
# Shay-Shay zsh completion
# Add to ~/.zshrc:
...
_shay() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_shay_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'shay command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Shay-Shay fish completion",
        "# Add to your config:",
        "#   shay completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __shay_profiles",
        "    echo default",
        "    if test -d $HOME/.shay/profiles",
        "        ls $HOME/.shay/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c shay -f -s p -l profile"
        " -d 'Profile name' -xa '(__shay_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__shay_profiles)' -d 'Profile name'"
                )
