# Contributing to Shay-Shay

Thank you for contributing to Shay-Shay! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Shay-Shay to work everywhere.
...
Bundled skills (in `skills/`) ship with every Shay-Shay install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `shay skills browse` (labeled "official") and install it with `shay skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `shay skills install`.
...
```bash
git clone --recurse-submodules https://github.com/NousResearch/shay-shay.git
cd shay-shay
...
```bash
mkdir -p ~/.shay/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.shay/config.yaml
touch ~/.shay/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.shay/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/shay" ~/.local/bin/shay
...
├── cli.py                    # ShayCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (shay-cli, shay-telegram, etc.)
├── shay_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── shay_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.shay/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (shay-shay.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.shay/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.shay/`)

| Path | Purpose |
|------|---------|
| `~/.shay/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.shay/.env` | API keys and secrets |
| `~/.shay/auth.json` | OAuth credentials (Nous Portal) |
| `~/.shay/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.shay/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.shay/state.db` | Canonical SQLite session/history database for recall and `session_search` |
| `~/.shay/sessions/` | Legacy/auxiliary JSON session artifacts and gateway bookkeeping |
| `~/.shay/cron/` | Scheduled job data |
| `~/.shay/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`shay_state.py`) with full-text search and unique session titles. `~/.shay/sessions/` remains for legacy/auxiliary JSON artifacts and gateway bookkeeping, not as the canonical recall layer.
- **Memory hierarchy reference**: For the current repo-level memory classification, see `docs/shay-memory-hierarchy.md`, `docs/shay-session-artifact-policy.md`, and `docs/shay-db-status.md`.
...
(for example `_SHAY_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.shay`:
...
The user may skip setup and keep loading the skill. Shay-Shay only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `shay setup` or update `~/.shay/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Shay-Shay tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `shay --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Shay-Shay uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.shay/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `shay_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `shay_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Shay-Shay runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the shay wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `shay_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `shay_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `shay_cli/gateway_windows.py::
...
Shay-Shay has terminal access. Security matters.
...
2. **Test manually**: Run `shay` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/NousResearch/shay-shay/issues)
- Include: OS, Python version, Shay-Shay version (`shay version`), full error traceback
