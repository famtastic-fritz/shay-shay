# Shay-Shay Security Policy

This document describes Shay-Shay's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/NousResearch/shay-shay/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Shay-Shay does not operate a bug
bounty program.**
...
- Environment details (`shay version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Shay-Shay access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Shay-Shay's documentation
...
Shay-Shay supports two OS-level isolation postures. They address
...
Shay-Shay supports this in two ways:

- **Shay-Shay's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Shay-Shay's in-process heuristics
...
Shay-Shay filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Shay-Shay
itself. Bugs in Shay-Shay's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Shay-Shay does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Shay-Shay's own documentation, or reasonable
  operator expectations would predict — including cases where
  Shay-Shay has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  shay-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Shay-Shay's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Shay-Shay. Bugs in Shay-Shay
...
- Shay-Shay includes supply-chain guards for MCP server
