"""Tests for the kanban CLI surface (shay_cli.kanban)."""
...
from shay_cli import kanban as kc
...
def test_kanban_in_autocomplete_table():
    from shay_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("SHAY_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("SHAY_KANBAN_BOARD") == "beta"
